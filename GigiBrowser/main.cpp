...
{
    const wchar_t* windowTitle = L"Gigi Browser (v" GIGI_VERSION_WITH_BUILD_NUMBER() " " BUILD_FLAVOR() ")";

    // Create application window
    //ImGui_ImplWin32_EnableDpiAwareness();
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, windowTitle, nullptr };
    wc.hIcon = LoadIcon(GetModuleHandle(nullptr), IDI_APPLICATION);
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, windowTitle, WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    // Turn off ini saving and loading
    ...
    init_info.RTVFormat = g_swapChainFormat;
    ...
    if (!g_browser.Init(g_pd3dDevice, APP_NUM_FRAMES_IN_FLIGHT, g_pd3dSrvDescHeap, APP_SRV_HEAP_SIZE, (int)g_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV), argc, argv))
        return 1;
    ...
    while (!done)
    {
        ...
        g_browser.ProcessPendingTextureLoads(g_pd3dCommandList);
