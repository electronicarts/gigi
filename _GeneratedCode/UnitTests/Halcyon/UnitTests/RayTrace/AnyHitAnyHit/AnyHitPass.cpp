...
namespace halcyon
{
	...
	unsigned int AnyHitPass::m_variable__missindex_Miss = 0;
	unsigned int AnyHitPass::m_variable__chindex_ClosestHit = 0;
	unsigned int AnyHitPass::m_variable__rt_hit_group_count = 0;

void AnyHitPipeline::loadPipeline(ShaderPipelines& pipeline)
{
	...
	pipeline.loadRayTracingMissProgram(getStaticClassId(), MissPipelineId, "Shaders/AnyHit/Miss", emptyShaderSignature);
	pipeline.loadRayTracingHitProgram(getStaticClassId(), ClosestHitPipelineId, "", "Shaders/AnyHit/ClosestHit", hitShaderSignature);
	pipeline.loadRayTracingRayGenProgram(getStaticClassId(), DoRTPipelineId, "Shaders/AnyHit/DoRT", emptyShaderSignature);
}

void AnyHitPipeline::unloadPipeline(ShaderPipelines& pipeline)
{
	if (!pipeline.isRayTracingSupported())
	{
		return;
...
void AnyHitPass::createGlobalResourcesHandles()
{
}

void AnyHitPass::deleteGlobalResourcesHandles()
{
}

void AnyHitPass::createGlobalResources(const IRenderDeviceGroup& deviceGroup, const RenderPassResourceCreateInfo& info)
{
}

void AnyHitPass::importGlobalResources(RenderGraphScope& scope, RenderGraph& renderGraph, const RenderPassResourceImportInfo& info)
{
	auto& d = scope.add<AnyHitPassData>();

}

void AnyHitPass::deleteGlobalResources(const IRenderDeviceGroup& deviceGroup)
{
...
void AnyHitPass::addAnyHitPass(RenderGraph& renderGraph, RenderGraphScope& scope, const ShaderPipelines& pipelines)
{
	auto& settings = scope.get<RenderGraphViewData>().renderSettings.AnyHit;
	auto& dynamicConstants	= scope.get<RenderGraphDynamicConstants>();
	auto& passData = scope.get<AnyHitPassData>();
	...
	renderGraph.addPassCallback("AnyHit InitPass", InitPass);
	m_variable__missindex_Miss = pipelines.getRtMissShaderIndex(AnyHitPipeline::getStaticClassId(), AnyHitPipeline::MissPipelineId);
	m_variable__chindex_ClosestHit = pipelines.getRtHitShaderIndex(AnyHitPipeline::getStaticClassId(), AnyHitPipeline::ClosestHitPipelineId);
	...
	auto& DoRTPass = [&](RenderGraphBuild& build)
	{
		...
		const uint32 rayGenShaderIndex	   = pipelines.getRtRayGenShaderIndex(AnyHitPipeline::getStaticClassId(), AnyHitPipeline::DoRTPipelineId);
		...
		auto buffervertexBuffer = passData.bufferSceneVB = build.read(passData.bufferSceneVB, RenderBindFlags::ShaderResource);
		...
		return [=](RenderGraphRegistry& registry, RenderCommandList& commandList)
		{
			...
			RenderBindingShaderResourceView srvs[] = {
				...
				defaultBufferView(registry, buffervertexBuffer),
				};
	...
	renderGraph.addPassCallback("AnyHit DoRT", DoRTPass);
}
