...
void Mips_CS_CubePipeline::loadPipeline(ShaderPipelines& pipeline)
{
...
void Mips_CS_CubePipeline::unloadPipeline(ShaderPipelines& pipeline)
{
...
void Mips_CS_CubePass::createGlobalResourcesHandles()
{
	m_texture_loadedTexture_0 = RenderSystem::createTexture("Mips_CS_Cube _loadedTexture_0 Texture");
}

void Mips_CS_CubePass::deleteGlobalResourcesHandles()
{
	RenderSystem::destroyResource(m_texture_loadedTexture_0);
}

void Mips_CS_CubePass::createGlobalResources(const IRenderDeviceGroup& deviceGroup, const RenderPassResourceCreateInfo& info)
{
	...
	// Texture: _loadedTexture_0
	{
		...
		{
			HcyVerify(d->createTexture(m_texture_loadedTexture_0, m_texture_loadedTexture_0Desc, nullptr, "Mips_CS_Cube _loadedTexture_0 Texture"));
		}
	}
}

void Mips_CS_CubePass::importGlobalResources(RenderGraphScope& scope, RenderGraph& renderGraph, const RenderPassResourceImportInfo& info)
{
	auto& d = scope.add<Mips_CS_CubePassData>();

	d.texture_loadedTexture_0 = renderGraph.import("Mips_CS_Cube _loadedTexture_0 Texture", m_texture_loadedTexture_0, m_texture_loadedTexture_0Desc);
}

void Mips_CS_CubePass::deleteGlobalResources(const IRenderDeviceGroup& deviceGroup)
{
	for (auto& d : deviceGroup.allDevices())
	{
...
void Mips_CS_CubePass::addMips_CS_CubePass(RenderGraph& renderGraph, RenderGraphScope& scope, const ShaderPipelines& pipelines)
{
	auto& settings = scope.get<RenderGraphViewData>().renderSettings.Mips_CS_Cube;
	auto& dynamicConstants	= scope.get<RenderGraphDynamicConstants>();
	auto& passData = scope.get<Mips_CS_CubePassData>();
	...
	renderGraph.addPassCallback("Mips_CS_Cube InitPass", InitPass);
	...
	auto& Mip0Pass = [&](RenderGraphBuild& build)
	{
		...
		auto pipelineState = pipelines.pipelineState(Mips_CS_CubePipeline::getStaticClassId(), Mips_CS_CubePipeline::Mip0PipelineId);
		...
		return [=](RenderGraphRegistry& registry, RenderCommandList& commandList)
		{
			...
			RenderBindingUnorderedAccessView uavs[] = {
				defaultTextureCubeRwView(registry, textureOutput),
				};
	...
	renderGraph.addPassCallback("Mips_CS_Cube Mip0", Mip0Pass);
	...
	auto& Mip1Pass = [&](RenderGraphBuild& build)
	{
		...
		auto pipelineState = pipelines.pipelineState(Mips_CS_CubePipeline::getStaticClassId(), Mips_CS_CubePipeline::Mip1PipelineId);

		return [=](RenderGraphRegistry& registry, RenderCommandList& commandList)
		{
			RenderBindingUnorderedAccessView uavs[] = {
				defaultTextureCubeRwView(registry, textureInput),
				defaultTextureCubeRwView(registry, textureOutput),
				};
			commandList.dispatch3d(
	...
	renderGraph.addPassCallback("Mips_CS_Cube Mip1", Mip1Pass);
	...
	auto& Mip2Pass = [&](RenderGraphBuild& build)
	{
		...
		auto pipelineState = pipelines.pipelineState(Mips_CS_CubePipeline::getStaticClassId(), Mips_CS_CubePipeline::Mip2PipelineId);
	...
	renderGraph.addPassCallback("Mips_CS_Cube Mip2", Mip2Pass);
	...
	auto& Mip3Pass = [&](RenderGraphBuild& build)
	{
		...
		auto pipelineState = pipelines.pipelineState(Mips_CS_CubePipeline::getStaticClassId(), Mips_CS_CubePipeline::Mip3PipelineId);
	...
	renderGraph.addPassCallback("Mips_CS_Cube Mip3", Mip3Pass);
}
