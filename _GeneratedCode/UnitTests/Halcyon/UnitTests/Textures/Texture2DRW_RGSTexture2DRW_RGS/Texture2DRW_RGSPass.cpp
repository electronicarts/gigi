...
namespace halcyon
{
	...
	unsigned int Texture2DRW_RGSPass::m_variable__chindex_RWCHS = 0;
	unsigned int Texture2DRW_RGSPass::m_variable__missindex_RWMISS = 0;
	unsigned int Texture2DRW_RGSPass::m_variable__rt_hit_group_count = 0;

void Texture2DRW_RGSPipeline::loadPipeline(ShaderPipelines& pipeline)
{
	...
	pipeline.loadRayTracingHitProgram(getStaticClassId(), RWCHSPipelineId, "", "Shaders/Texture2DRW_RGS/RWCHS", hitShaderSignature);
	pipeline.loadRayTracingMissProgram(getStaticClassId(), RWMISSPipelineId, "Shaders/Texture2DRW_RGS/RWMISS", emptyShaderSignature);
	pipeline.loadRayTracingRayGenProgram(getStaticClassId(), RWPipelineId, "Shaders/Texture2DRW_RGS/RW", emptyShaderSignature);
}

void Texture2DRW_RGSPipeline::unloadPipeline(ShaderPipelines& pipeline)
{
	if (!pipeline.isRayTracingSupported())
	{
		return;
...
void Texture2DRW_RGSPass::createGlobalResourcesHandles()
{
	m_texture_loadedTexture_0 = RenderSystem::createTexture("Texture2DRW_RGS _loadedTexture_0 Texture");
}

void Texture2DRW_RGSPass::deleteGlobalResourcesHandles()
{
	RenderSystem::destroyResource(m_texture_loadedTexture_0);
}

void Texture2DRW_RGSPass::createGlobalResources(const IRenderDeviceGroup& deviceGroup, const RenderPassResourceCreateInfo& info)
{
	...
	// Texture: _loadedTexture_0
	{
		...
		{
			HcyVerify(d->createTexture(m_texture_loadedTexture_0, m_texture_loadedTexture_0Desc, nullptr, "Texture2DRW_RGS _loadedTexture_0 Texture"));
		}
	}
}

void Texture2DRW_RGSPass::importGlobalResources(RenderGraphScope& scope, RenderGraph& renderGraph, const RenderPassResourceImportInfo& info)
{
	auto& d = scope.add<Texture2DRW_RGSPassData>();

	d.texture_loadedTexture_0 = renderGraph.import("Texture2DRW_RGS _loadedTexture_0 Texture", m_texture_loadedTexture_0, m_texture_loadedTexture_0Desc);
}

void Texture2DRW_RGSPass::deleteGlobalResources(const IRenderDeviceGroup& deviceGroup)
{
	for (auto& d : deviceGroup.allDevices())
	{
...
void Texture2DRW_RGSPass::addTexture2DRW_RGSPass(RenderGraph& renderGraph, RenderGraphScope& scope, const ShaderPipelines& pipelines)
{
	auto& settings = scope.get<RenderGraphViewData>().renderSettings.Texture2DRW_RGS;
	auto& dynamicConstants	= scope.get<RenderGraphDynamicConstants>();
	auto& passData = scope.get<Texture2DRW_RGSPassData>();
	...
	renderGraph.addPassCallback("Texture2DRW_RGS InitPass", InitPass);
	m_variable__chindex_RWCHS = pipelines.getRtHitShaderIndex(Texture2DRW_RGSPipeline::getStaticClassId(), Texture2DRW_RGSPipeline::RWCHSPipelineId);
	m_variable__missindex_RWMISS = pipelines.getRtMissShaderIndex(Texture2DRW_RGSPipeline::getStaticClassId(), Texture2DRW_RGSPipeline::RWMISSPipelineId);
	...
	auto& RWPass = [&](RenderGraphBuild& build)
	{
		...
		const uint32 rayGenShaderIndex	   = pipelines.getRtRayGenShaderIndex(Texture2DRW_RGSPipeline::getStaticClassId(), Texture2DRW_RGSPipeline::RWPipelineId);
	...
	renderGraph.addPassCallback("Texture2DRW_RGS RW", RWPass);
}
