...
Copy Public/buffertestTechnique.h to Engine/Source/Runtime/Renderer/Public/

Copy Public/buffertestTechnique.cpp to Engine/Source/Runtime/Renderer/Private/

Copy the contents of Shaders to Engine/Shaders/Private/buffertest/
...
struct FTechniqueState_buffertest
{
    // User Variables - feel free to read and write these
    float Var_gain = float(0.5f);  // Overall Volume Adjustment
    float Var_alpha1 = float(1.f);  // Adjusts the contribution of sample n-1
    float Var_alpha2 = float(0.f);  // Adjusts the contribution of sample n-2
};

and put this member variable into FSceneViewState:

FTechniqueState_buffertest TechniqueState_buffertest;
...
Assuming you want your new post processing effect to run first, you can put this chunk of code after that line:

	{
		...
		// View.ViewState->TechniqueState_buffertest.Var_Enabled = true;
...
See buffertestTechnique.cpp for details.
