...
FB_ABI_BEGIN(TwoRayGens_TwoRayGens2_Abi)
FB_ABI_END
...
{
	Struct__TwoRayGens2CB _TwoRayGens2CB;
};
...
#define TraceARay(AccelerationStructure, RayFlags, InstanceInclusionMask, RayContributionToHitGroupIndex, MultiplierForGeometryContributionToHitGroupIndex, MissShaderIndex, Ray, payload) \
	fb::rt::traceRay<TwoRayGens_TwoRayGens2_Abi, FB_RT_FLAGS_UseSharedStack | FB_RT_FLAGS_DefaultStackLds, RayFlags> \
...
FB_DEFINE_MISS_SHADER(TwoRayGens_TwoRayGens2_Abi, Miss2A, defaultLsrt, Payload, payload)
{
	payload.hit = false;
	payload.blueChannelMultiplier = 0.25f;
}

FB_DEFINE_MISS_SHADER(TwoRayGens_TwoRayGens2_Abi, Miss2B, defaultLsrt, Payload, payload)
{
	payload.hit = false;
	payload.blueChannelMultiplier = 1.0f;
}

FB_DEFINE_CLOSEST_HIT_SHADER(TwoRayGens_TwoRayGens2_Abi, ClosestHit2, defaultLsrt, Payload, payload)
{
	payload.hit = true;
