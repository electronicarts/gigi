...
FB_ABI_BEGIN(TwoRayGens_TwoRayGens1_Abi)
FB_ABI_END
...
{
	Struct__TwoRayGens1CB _TwoRayGens1CB;
};
...
#define TraceARay(AccelerationStructure, RayFlags, InstanceInclusionMask, RayContributionToHitGroupIndex, MultiplierForGeometryContributionToHitGroupIndex, MissShaderIndex, Ray, payload) \
	fb::rt::traceRay<TwoRayGens_TwoRayGens1_Abi, FB_RT_FLAGS_UseSharedStack | FB_RT_FLAGS_DefaultStackLds, RayFlags> \
...
FB_DEFINE_MISS_SHADER(TwoRayGens_TwoRayGens1_Abi, Miss1, defaultLsrt, Payload, payload)
{
	payload.hit = false;
}

FB_DEFINE_CLOSEST_HIT_SHADER(TwoRayGens_TwoRayGens1_Abi, ClosestHit1, defaultLsrt, Payload, payload)
{
	payload.hit = true;
}
