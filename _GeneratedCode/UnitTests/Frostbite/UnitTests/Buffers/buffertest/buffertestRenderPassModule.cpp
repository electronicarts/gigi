...
#include "buffertestRenderPassModule.h"
...
namespace
{
	FB_DECLARE_VAR_FLOAT(g_vargain, "buffertest.gain", 0.5f);  // Overall Volume Adjustment
	FB_DECLARE_VAR_FLOAT(g_varalpha1, "buffertest.alpha1", 1.f);  // Adjusts the contribution of sample n-1
	FB_DECLARE_VAR_FLOAT(g_varalpha2, "buffertest.alpha2", 0.f);  // Adjusts the contribution of sample n-2
}

namespace fb
{
ShaderProgramPipelineCache buffertestRenderPassModule::s_pipelineCache;
...
void buffertestRenderPassModule::addPass(FrameGraph& frameGraph, FrameGraphBlackboard& blackboard)
{
	...
	FrameGraphMutableResource bufferOutputTypedStructBuffer;  // An internal buffer used during the filtering process.
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("buffertest.Initialization", [&](RenderPassBuilderBase& builder, PassData& data)
		{
			...
			// An internal buffer used during the filtering process.
			{
				...
				desc.name = "buffertest.OutputTypedBuffer";
			...
			// Buffer: OutputStructuredBuffer
			{
				...
				FrameGraphBufferDesc desc;
				desc.name = "buffertest.OutputStructuredBuffer";
				desc.usage = RenderResourceUsage_Static;
				desc.initialState = FrameGraphBufferDesc::InitialState_Undefined;
				desc.bindFlags = bindFlags;
				desc.memoryFlags = memoryFlags;
			...
			// An internal buffer used during the filtering process.
			{
				...
				FrameGraphBufferDesc desc;
				desc.name = "buffertest.OutputTypedBufferRaw";
				desc.usage = RenderResourceUsage_Static;
				desc.initialState = FrameGraphBufferDesc::InitialState_Undefined;
				desc.bindFlags = bindFlags;
				desc.memoryFlags = memoryFlags;
			...
			// An internal buffer used during the filtering process.
			{
				...
				FrameGraphBufferDesc desc;
				desc.name = "buffertest.OutputTypedStructBuffer";
				desc.usage = RenderResourceUsage_Static;
				desc.initialState = FrameGraphBufferDesc::InitialState_Undefined;
				desc.bindFlags = bindFlags;
				desc.memoryFlags = memoryFlags;
	...
	// Transition resources for the next action
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("buffertest.Transitions", [&](RenderPassBuilderBase& builder, PassData& data)
		{
	...
	// BufferTest compute shader
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("buffertest.BufferTest",
		...
		[=](const PassData& data, const Rc2RenderPassResources& resources, CommandRecorder& commandRecorder)
		{
			const ComputePipelineInfo& info = s_pipelineCache.getComputePipeline(ShaderProgram_buffertest_BufferTest_MainCs, 0);
			...
			binder.computeStage()
				...
				.setUav(0, resources.getUav(data.bufferOutputTypedBuffer))
				.setSrv(1, resources.getSrv(data.bufferInputTypedStructBuffer))
				.setUav(1, resources.getUav(data.bufferOutputTypedStructBuffer))
				.setSrv(2, resources.getSrv(data.bufferInputStructuredBuffer))
				.setUav(2, resources.getUav(data.bufferOutputStructuredBuffer))
				.setSrv(3, resources.getSrv(data.bufferInputTypedBufferRaw))
				.setUav(3, resources.getUav(data.bufferOutputTypedBufferRaw))
...
void buffertestRenderPassModule::onInit(const InitParams& params)
{
...
buffertestRenderPassModule::~buffertestRenderPassModule()
{
