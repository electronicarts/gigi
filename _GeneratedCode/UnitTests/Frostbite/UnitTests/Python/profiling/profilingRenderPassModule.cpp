...
#include "profilingRenderPassModule.h"
...
namespace fb
{
ShaderProgramPipelineCache profilingRenderPassModule::s_pipelineCache;
...
void profilingRenderPassModule::addPass(FrameGraph& frameGraph, FrameGraphBlackboard& blackboard)
{
	...
	// Initialization - create resources internal to the technique
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("profiling.Initialization", [&](RenderPassBuilderBase& builder, PassData& data)
		{
		},
	...
	// Runs the shader
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("profiling.DoSimpleCS",
		...
		[=](const PassData& data, const Rc2RenderPassResources& resources, CommandRecorder& commandRecorder)
		{
			const ComputePipelineInfo& info = s_pipelineCache.getComputePipeline(ShaderProgram_profiling_DoSimpleCS_MainCs, 0);
...
void profilingRenderPassModule::onInit(const InitParams& params)
{
...
profilingRenderPassModule::~profilingRenderPassModule()
{
