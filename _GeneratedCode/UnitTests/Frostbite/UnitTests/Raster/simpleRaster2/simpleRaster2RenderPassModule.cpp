...
#include "simpleRaster2RenderPassModule.h"
...
namespace fb
{
ShaderProgramPipelineCache simpleRaster2RenderPassModule::s_pipelineCache;
...
void simpleRaster2RenderPassModule::addPass(FrameGraph& frameGraph, FrameGraphBlackboard& blackboard)
{
	// TODO: Provide this imported buffer
	FrameGraphResource& bufferVertexBuffer = ;
	// TODO: Describe the vertex buffer format
	eastl::vector<InputElementDesc> bufferVertexBuffer_vbDesc;
	...
	FrameGraphMutableResource textureDepth_Buffer;
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("simpleRaster2.Initialization", [&](RenderPassBuilderBase& builder, PassData& data)
		{
			...
			// Texture: Color_Buffer
			{
				...
				desc.name = "simpleRaster2.Color_Buffer";
			...
			// Texture: Depth_Buffer
			{
				...
				FrameGraphTextureDesc desc;
				desc.name = "simpleRaster2.Depth_Buffer";
				desc.usage = RenderResourceUsage_Static;
				desc.initialState = FrameGraphTextureDesc::InitialState_Undefined;
				desc.type = type;
				desc.bindFlags = bindFlags;
	...
	// Draw Call: Rasterize
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("simpleRaster2.Rasterize",
		[&](RenderPassBuilderBase& builder, PassData& data)
		{
			...
			FrameGraphBufferDesc vbDesc = builder.getBufferDesc(bufferVertexBuffer);
			data.vertexBuffer = builder.read(bufferVertexBuffer);
		...
		[=](const PassData& data, const Rc2RenderPassResources& resources, CommandRecorder& commandRecorder)
		{
			commandRecorder.transitionResource(resources.getBuffer(bufferVertexBuffer), ResourceBindFlags_VertexBuffer);

			RenderPassDesc renderPassDesc;
			renderPassDesc.name = "simpleRaster2.Rasterize";
			...
			inputElementDescs.insert(inputElementDescs.end(), bufferVertexBuffer_vbDesc.begin(), bufferVertexBuffer_vbDesc.end());
			...
			graphicsPipelineCompileInfo.shaderProgram = ShaderProgram_simpleRaster2_Rasterize_VSPS;
			...
			vertexBufferViews[0] = resources.getBuffer(bufferVertexBuffer)->getAsVbv(ViewDesc::buildAsVertexBufferView(0, data.vbStrideBytes, data.vbSizeBytes));
...
void simpleRaster2RenderPassModule::onInit(const InitParams& params)
{
...
simpleRaster2RenderPassModule::~simpleRaster2RenderPassModule()
{
