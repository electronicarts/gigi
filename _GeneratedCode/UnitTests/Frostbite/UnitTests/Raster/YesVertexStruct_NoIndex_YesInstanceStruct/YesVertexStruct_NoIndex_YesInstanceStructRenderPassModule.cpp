...
#include "YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule.h"
...
namespace
{
	FB_DECLARE_VAR_INT(g_varviewMode, "YesVertexStruct_NoIndex_YesInstanceStruct.viewMode", (int)fb::YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule::ViewMode::Normal);
}

namespace fb
{
ShaderProgramPipelineCache YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule::s_pipelineCache;
...
void YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule::addPass(FrameGraph& frameGraph, FrameGraphBlackboard& blackboard)
{
	...
	// TODO: Provide this imported buffer
	FrameGraphResource& bufferInstance_Buffer = ;
	// TODO: Describe the vertex buffer format
	eastl::vector<InputElementDesc> bufferInstance_Buffer_vbDesc;
	...
	FrameGraphMutableResource textureDepth_Buffer;
	{
		...
		frameGraph.addRc2CallbackPass<PassData>("YesVertexStruct_NoIndex_YesInstanceStruct.Initialization", [&](RenderPassBuilderBase& builder, PassData& data)
		{
			...
			// Texture: Color_Buffer
			{
				...
				desc.name = "YesVertexStruct_NoIndex_YesInstanceStruct.Color_Buffer";
			...
			// Texture: Depth_Buffer
			{
				...
				FrameGraphTextureDesc desc;
				desc.name = "YesVertexStruct_NoIndex_YesInstanceStruct.Depth_Buffer";
				desc.usage = RenderResourceUsage_Static;
				desc.initialState = FrameGraphTextureDesc::InitialState_Undefined;
				desc.type = type;
				desc.bindFlags = bindFlags;
	...
	// Draw Call: Rasterize
	{
		struct PassData
		{
			...
			size_t inbStrideBytes;
			size_t inbSizeBytes;
			Struct__VertexShaderCB cb_VertexShaderCB;
			Struct__PixelShaderCB cb_PixelShaderCB;
		};

		frameGraph.addRc2CallbackPass<PassData>("YesVertexStruct_NoIndex_YesInstanceStruct.Rasterize",
		[&](RenderPassBuilderBase& builder, PassData& data)
		{
			...
			FrameGraphBufferDesc inbDesc = builder.getBufferDesc(bufferInstance_Buffer);
			data.instanceBuffer = builder.read(bufferInstance_Buffer);
			data.inbStrideBytes = inbDesc.structByteStride > 0 ? inbDesc.structByteStride : getFormatBytesPerElement(convertRenderFormat(inbDesc.format));
			data.inbSizeBytes = inbDesc.size;

			uint32_t vbCount = vbDesc.size / (uint32_t)data.vbStrideBytes;
			data.vertexCountPerInstance = vbCount;

			uint32_t inbCount = inbDesc.size / (uint32_t)data.inbStrideBytes;
			data.instanceCount = inbCount;
		},
		[=](const PassData& data, const Rc2RenderPassResources& resources, CommandRecorder& commandRecorder)
		{
			commandRecorder.transitionResource(resources.getBuffer(bufferVertex_Buffer), ResourceBindFlags_VertexBuffer);
			commandRecorder.transitionResource(resources.getBuffer(bufferInstance_Buffer), ResourceBindFlags_VertexBuffer);

			RenderPassDesc renderPassDesc;
			renderPassDesc.name = "YesVertexStruct_NoIndex_YesInstanceStruct.Rasterize";
			...
			inputElementDescs.insert(inputElementDescs.end(), bufferInstance_Buffer_vbDesc.begin(), bufferInstance_Buffer_vbDesc.end());
			...
			graphicsPipelineCompileInfo.shaderProgram = ShaderProgram_YesVertexStruct_NoIndex_YesInstanceStruct_Rasterize_VSPS;
			...
			// Vertex Buffers
			ResourceView vertexBufferViews[2];
			...
			vertexBufferViews[1] = resources.getBuffer(bufferInstance_Buffer)->getAsVbv(ViewDesc::buildAsVertexBufferView(0, data.inbStrideBytes, data.inbSizeBytes));
			commandRecorder.setVertexBuffers(2, vertexBufferViews);
	...
	// bufferInstance_Buffer  TODO: Put this buffer into the black board (Optional)
}

void YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule::onInit(const InitParams& params)
{
...
YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule::~YesVertexStruct_NoIndex_YesInstanceStructRenderPassModule()
{
