...
class class_Texture2DArrayRW_RGS
{
...
@binding(5) @group(0) var _loadedTexture_0 : texture_2d_array<f32>;

@binding(4) @group(0) var importedTextureReadOnly : texture_storage_2d_array</*(importedTextureReadOnly_format)*/, read>;

@binding(2) @group(0) var importedColor : texture_2d_array<f32>;

@binding(0) @group(0) var nodeTexture : texture_storage_2d_array</*(nodeTexture_format)*/, write>;

@binding(1) @group(0) var importedTexture : texture_storage_2d_array</*(importedTexture_format)*/, write>;
...
fn rgsmain(@builtin(global_invocation_id) DTid_0 : vec3<u32>)
{
    ...
    var loadedTexturePx_0 : vec3<f32> = (textureLoad((_loadedTexture_0), ((_S1)).xy, i32(((_S1)).z), ((_S1)).w)).xyz;
    var _S2 : vec3<i32> = vec3<i32>(px_0);
    var _S3 : vec4<f32> = (textureLoad((importedTextureReadOnly), ((_S2)).xy, i32(((_S2)).z)));
    var importedTexturePx_0 : vec3<f32> = _S3.xyz;
    var importedColorPx_0 : vec3<f32> = (textureLoad((importedColor), ((_S1)).xy, i32(((_S1)).z), ((_S1)).w)).xyz;
    textureStore((nodeTexture), ((px_0)).xy, i32(((px_0)).z), (vec4<f32>(loadedTexturePx_0 * importedTexturePx_0 * importedColorPx_0, 1.0f)));
    textureStore((importedTexture), ((px_0)).xy, i32(((px_0)).z), (vec4<f32>((loadedTexturePx_0 + importedTexturePx_0) / vec3<f32>(2.0f) * importedColorPx_0, 1.0f)));
    return;
}
    ...
    // Handle (re)creation of texture NodeTexture
    {
        ...
        if (this.texture_NodeTexture === null)
        {
            ...
            this.texture_NodeTexture = device.createTexture({
                label: "texture Texture2DArrayRW_RGS.NodeTexture",
    ...
    // Handle (re)creation of texture RW_nodeTexture_ReadOnly
    {
        ...
        if (this.texture_RW_nodeTexture_ReadOnly === null)
        {
            ...
            this.texture_RW_nodeTexture_ReadOnly = device.createTexture({
                label: "texture Texture2DArrayRW_RGS.RW_nodeTexture_ReadOnly",
    ...
    // Handle (re)creation of texture RW_importedTexture_ReadOnly
    {
        ...
        if (this.texture_RW_importedTexture_ReadOnly === null)
        {
            ...
            this.texture_RW_importedTexture_ReadOnly = device.createTexture({
                label: "texture Texture2DArrayRW_RGS.RW_importedTexture_ReadOnly",
    ...
    // (Re)create raygen shader RW
    {
        ...
        [
            ...
            },
            {
                ...
                binding: 3,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_RW_nodeTexture_ReadOnly_format), viewDimension: "2d-array" }
            },
            {
                // importedTextureReadOnly
                binding: 4,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_RW_importedTexture_ReadOnly_format), viewDimension: "2d-array" }
            },
            {
                // _loadedTexture_0
                binding: 5,
        ...
        if (this.ShaderModule_RayGen_RW === null || newHash !== this.Hash_RayGen_RW)
        {
            ...
            let shaderCode = class_Texture2DArrayRW_RGS.ShaderCode_RW_RWRGS;
...
{
    encoder.pushDebugGroup("Texture2DArrayRW_RGS.NodeTexture");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.NodeTexture"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.ImportedTexture");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.ImportedTexture"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.ImportedColor");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.ImportedColor"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.RW_nodeTexture_ReadOnly");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.RW_nodeTexture_ReadOnly"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.Copy_RW_nodeTexture");
    ...
    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.Copy_RW_nodeTexture"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.RW_importedTexture_ReadOnly");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.RW_importedTexture_ReadOnly"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.Copy_RW_importedTexture");
    ...
    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.Copy_RW_importedTexture"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS._loadedTexture_0");

    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS._loadedTexture_0"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS._RWRGSCB");

    // Create constant buffer _RWRGSCB
    if (this.constantBuffer__RWRGSCB === null)
    {
        this.constantBuffer__RWRGSCB = device.createBuffer({
            label: "Texture2DArrayRW_RGS._RWRGSCB",
    ...
    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS._RWRGSCB"

    encoder.pushDebugGroup("Texture2DArrayRW_RGS.RW");
        ...
        const bindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                },
                {
                    // nodeTextureReadOnly
                    binding: 3,
                    resource: this.texture_RW_nodeTexture_ReadOnly.createView({ dimension: "2d-array", mipLevelCount: 1, baseMipLevel: 0 })
                },
                {
                    // importedTextureReadOnly
                    binding: 4,
                    resource: this.texture_RW_importedTexture_ReadOnly.createView({ dimension: "2d-array", mipLevelCount: 1, baseMipLevel: 0 })
                },
                {
                    // _loadedTexture_0
                    binding: 5,
    ...
    encoder.popDebugGroup(); // "Texture2DArrayRW_RGS.RW"
...
var Texture2DArrayRW_RGS = new class_Texture2DArrayRW_RGS;

export default Texture2DArrayRW_RGS;
