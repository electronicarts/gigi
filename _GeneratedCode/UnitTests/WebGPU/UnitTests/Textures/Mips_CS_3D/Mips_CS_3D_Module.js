...
@binding(0) @group(0) var Output : texture_storage_3d</*(Output_format)*/, write>;
...
fn main(@builtin(global_invocation_id) DTid_0 : vec3<u32>)
{
    ...
    dims_0[i32(0)] = _S2;
    dims_0[i32(1)] = _S3;
    var readPos_0 : vec2<u32> = (px_0.xy + vec2<u32>(u32(0.6180340051651001f * f32(dims_0.x)), u32(0.6180340051651001f * f32(dims_0.y))) * vec2<u32>(px_0.z)) % dims_0;
    var _S4 : vec3<i32> = vec3<i32>(vec3<u32>(readPos_0, u32(0)));
    textureStore((Output), (px_0), (vec4<f32>(LinearToSRGB_0((textureLoad((_loadedTexture_0), ((_S4)).xy, ((_S4)).z)).xyz), 1.0f)));
    return;
}
...
@binding(0) @group(0) var Input : texture_storage_3d</*(Input_format)*/, write>;

@binding(2) @group(0) var InputReadOnly : texture_storage_3d</*(InputReadOnly_format)*/, read>;

@binding(1) @group(0) var Output : texture_storage_3d</*(Output_format)*/, write>;
...
fn main(@builtin(global_invocation_id) DTid_0 : vec3<u32>)
{
    var px_0 : vec3<u32> = DTid_0.xyz;
    var dims_0 : vec3<u32>;
    var _S3 : u32 = dims_0[i32(0)];
    var _S4 : u32 = dims_0[i32(1)];
    var _S5 : u32 = dims_0[i32(2)];
    {var dim = textureDimensions((Input));((_S3)) = dim.x;((_S4)) = dim.y;((_S5)) = dim.z;};
    dims_0[i32(0)] = _S3;
    dims_0[i32(1)] = _S4;
    dims_0[i32(2)] = _S5;
    var _S6 : vec3<u32> = px_0 * vec3<u32>(u32(2));
    var _S7 : vec3<u32> = _S6 % dims_0;
    var _S8 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S7))));
    var _S9 : vec3<f32> = SRGBToLinear_0(_S8.xyz);
    var _S10 : vec3<u32> = (_S6 + vec3<u32>(u32(0), u32(0), u32(1))) % dims_0;
    var _S11 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S10))));
    var _S12 : vec3<f32> = _S9 + SRGBToLinear_0(_S11.xyz);
    var _S13 : vec3<u32> = (_S6 + vec3<u32>(u32(0), u32(1), u32(0))) % dims_0;
    var _S14 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S13))));
    var _S15 : vec3<f32> = _S12 + SRGBToLinear_0(_S14.xyz);
    var _S16 : vec3<u32> = (_S6 + vec3<u32>(u32(0), u32(1), u32(1))) % dims_0;
    var _S17 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S16))));
    var _S18 : vec3<f32> = _S15 + SRGBToLinear_0(_S17.xyz);
    var _S19 : vec3<u32> = (_S6 + vec3<u32>(u32(1), u32(0), u32(0))) % dims_0;
    var _S20 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S19))));
    var _S21 : vec3<f32> = _S18 + SRGBToLinear_0(_S20.xyz);
    var _S22 : vec3<u32> = (_S6 + vec3<u32>(u32(1), u32(0), u32(1))) % dims_0;
    var _S23 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S22))));
    var _S24 : vec3<f32> = _S21 + SRGBToLinear_0(_S23.xyz);
    var _S25 : vec3<u32> = (_S6 + vec3<u32>(u32(1), u32(1), u32(0))) % dims_0;
    var _S26 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S25))));
    var _S27 : vec3<f32> = _S24 + SRGBToLinear_0(_S26.xyz);
    var _S28 : vec3<u32> = (_S6 + vec3<u32>(u32(1), u32(1), u32(1))) % dims_0;
    var _S29 : vec4<f32> = (textureLoad((InputReadOnly), (vec3<i32>(_S28))));
    textureStore((Output), (px_0), (vec4<f32>(LinearToSRGB_0((_S27 + SRGBToLinear_0(_S29.xyz)) / vec3<f32>(8.0f)), 1.0f)));
    return;
}
...
variable_RenderSize = [ 256, 256, 8 ];
variableDefault_RenderSize = [ 256, 256, 8 ];
...
async Init(device, encoder, useBlockingAPIs)
{
    // Handle (re)creation of texture Output
    {
        ...
        if (this.texture_Output === null)
        {
            ...
            this.texture_Output = device.createTexture({
                label: "texture Mips_CS_3D.Output",
    ...
    // Handle (re)creation of texture Mip0_Output_ReadOnly
    {
        ...
        if (this.texture_Mip0_Output_ReadOnly === null)
        {
            ...
            this.texture_Mip0_Output_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip0_Output_ReadOnly",
    ...
    // Handle (re)creation of texture Mip1_Input_ReadOnly
    {
        ...
        if (this.texture_Mip1_Input_ReadOnly === null)
        {
            ...
            this.texture_Mip1_Input_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip1_Input_ReadOnly",
    ...
    // Handle (re)creation of texture Mip1_Output_ReadOnly
    {
        ...
        if (this.texture_Mip1_Output_ReadOnly === null)
        {
            ...
            this.texture_Mip1_Output_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip1_Output_ReadOnly",
    ...
    // Handle (re)creation of texture Mip2_Input_ReadOnly
    {
        ...
        if (this.texture_Mip2_Input_ReadOnly === null)
        {
            ...
            this.texture_Mip2_Input_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip2_Input_ReadOnly",
    ...
    // Handle (re)creation of texture Mip2_Output_ReadOnly
    {
        ...
        if (this.texture_Mip2_Output_ReadOnly === null)
        {
            ...
            this.texture_Mip2_Output_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip2_Output_ReadOnly",
    ...
    // Handle (re)creation of texture Mip3_Input_ReadOnly
    {
        ...
        if (this.texture_Mip3_Input_ReadOnly === null)
        {
            ...
            this.texture_Mip3_Input_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip3_Input_ReadOnly",
    ...
    // Handle (re)creation of texture Mip3_Output_ReadOnly
    {
        ...
        if (this.texture_Mip3_Output_ReadOnly === null)
        {
            ...
            this.texture_Mip3_Output_ReadOnly = device.createTexture({
                label: "texture Mips_CS_3D.Mip3_Output_ReadOnly",
    ...
    // (Re)create compute shader Mip0
    {
        ...
        [
            {
                ...
                storageTexture : { access: "write-only", format: Shared.GetNonSRGBFormat(this.texture_Output_format), viewDimension: "3d" }
            },
            {
                // OutputReadOnly
                binding: 1,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip0_Output_ReadOnly_format), viewDimension: "3d" }
            },
            {
                // _loadedTexture_0
                binding: 2,
        ...
        if (this.ShaderModule_Compute_Mip0 === null || newHash !== this.Hash_Compute_Mip0)
        {
            ...
            let shaderCode = class_Mips_CS_3D.ShaderCode_Mip0_Mip0CS_0;
    ...
    // (Re)create compute shader Mip1
    {
        ...
        [
            ...
            },
            {
                ...
                binding: 2,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip1_Input_ReadOnly_format), viewDimension: "3d" }
            },
            {
                // OutputReadOnly
                binding: 3,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip1_Output_ReadOnly_format), viewDimension: "3d" }
            },
        ];
        ...
        if (this.ShaderModule_Compute_Mip1 === null || newHash !== this.Hash_Compute_Mip1)
        {
            ...
            let shaderCode = class_Mips_CS_3D.ShaderCode_Mip1_MipNCS_0;
    ...
    // (Re)create compute shader Mip2
    {
        ...
        [
            ...
            },
            {
                ...
                binding: 2,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip2_Input_ReadOnly_format), viewDimension: "3d" }
            },
            {
                // OutputReadOnly
                binding: 3,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip2_Output_ReadOnly_format), viewDimension: "3d" }
            },
        ];
        ...
        if (this.ShaderModule_Compute_Mip2 === null || newHash !== this.Hash_Compute_Mip2)
        {
            ...
            let shaderCode = class_Mips_CS_3D.ShaderCode_Mip2_MipNCS_1;
            shaderCode = shaderCode.replace("/*(Input_format)*/", Shared.GetNonSRGBFormat(this.texture_Output_format));
            shaderCode = shaderCode.replace("/*(Output_format)*/", Shared.GetNonSRGBFormat(this.texture_Output_format));
    ...
    // (Re)create compute shader Mip3
    {
        ...
        [
            ...
            },
            {
                ...
                binding: 2,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip3_Input_ReadOnly_format), viewDimension: "3d" }
            },
            {
                // OutputReadOnly
                binding: 3,
                visibility: GPUShaderStage.COMPUTE,
                storageTexture : { access: "read-only", format: Shared.GetNonSRGBFormat(this.texture_Mip3_Output_ReadOnly_format), viewDimension: "3d" }
            },
        ];
        ...
        if (this.ShaderModule_Compute_Mip3 === null || newHash !== this.Hash_Compute_Mip3)
        {
            ...
            let shaderCode = class_Mips_CS_3D.ShaderCode_Mip3_MipNCS_2;
            shaderCode = shaderCode.replace("/*(Input_format)*/", Shared.GetNonSRGBFormat(this.texture_Output_format));
            shaderCode = shaderCode.replace("/*(Output_format)*/", Shared.GetNonSRGBFormat(this.texture_Output_format));
...
{
    encoder.pushDebugGroup("Mips_CS_3D.Output");

    encoder.popDebugGroup(); // "Mips_CS_3D.Output"

    encoder.pushDebugGroup("Mips_CS_3D.Mip0_Output_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip0_Output_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip0_Output");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip0_Output"

    encoder.pushDebugGroup("Mips_CS_3D.Mip1_Input_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip1_Input_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Mip1_Output_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip1_Output_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Mip2_Input_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip2_Input_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Mip2_Output_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip2_Output_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Mip3_Input_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip3_Input_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D.Mip3_Output_ReadOnly");

    encoder.popDebugGroup(); // "Mips_CS_3D.Mip3_Output_ReadOnly"

    encoder.pushDebugGroup("Mips_CS_3D._loadedTexture_0");

    encoder.popDebugGroup(); // "Mips_CS_3D._loadedTexture_0"

    encoder.pushDebugGroup("Mips_CS_3D.Mip0");
            ...
            entries: [
                {
                    // Output
                    binding: 0,
                    resource: this.texture_Output.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 0 })
                },
                {
                    // OutputReadOnly
                    binding: 1,
                    resource: this.texture_Mip0_Output_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 0 })
                },
                {
                    // _loadedTexture_0
                    binding: 2,
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Mip0"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip1_Input");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip1_Input"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip1_Output");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip1_Output"

    encoder.pushDebugGroup("Mips_CS_3D.Mip1");
    ...
    // Run compute shader Mip1
    {
        const bindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                },
                {
                    // Output
                    binding: 1,
                    resource: this.texture_Output.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 1 })
                },
                {
                    // InputReadOnly
                    binding: 2,
                    resource: this.texture_Mip1_Input_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 0 })
                },
                {
                    // OutputReadOnly
                    binding: 3,
                    resource: this.texture_Mip1_Output_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 1 })
                },
            ]
        });
        ...
        const baseDispatchSize = [ this.variable_RenderSize[0], this.variable_RenderSize[1], this.variable_RenderSize[2] ];
        const dispatchSize = [
            Math.floor((Math.floor(((parseInt(baseDispatchSize[0]) + 0) * 1) / 2) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[1]) + 0) * 1) / 2) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[2]) + 0) * 1) / 2) + 0 + 4 - 1) / 4)
        ];
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Mip1"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip2_Input");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip2_Input"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip2_Output");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip2_Output"

    encoder.pushDebugGroup("Mips_CS_3D.Mip2");
    ...
    // Run compute shader Mip2
    {
        const bindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                },
                {
                    // Output
                    binding: 1,
                    resource: this.texture_Output.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 2 })
                },
                {
                    // InputReadOnly
                    binding: 2,
                    resource: this.texture_Mip2_Input_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 1 })
                },
                {
                    // OutputReadOnly
                    binding: 3,
                    resource: this.texture_Mip2_Output_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 2 })
                },
            ]
        });
        ...
        const baseDispatchSize = [ this.variable_RenderSize[0], this.variable_RenderSize[1], this.variable_RenderSize[2] ];
        const dispatchSize = [
            Math.floor((Math.floor(((parseInt(baseDispatchSize[0]) + 0) * 1) / 4) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[1]) + 0) * 1) / 4) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[2]) + 0) * 1) / 4) + 0 + 4 - 1) / 4)
        ];
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Mip2"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip3_Input");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip3_Input"

    encoder.pushDebugGroup("Mips_CS_3D.Copy_Mip3_Output");
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Copy_Mip3_Output"

    encoder.pushDebugGroup("Mips_CS_3D.Mip3");
    ...
    // Run compute shader Mip3
    {
        const bindGroup = device.createBindGroup({
            ...
            entries: [
                ...
                },
                {
                    // Output
                    binding: 1,
                    resource: this.texture_Output.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 3 })
                },
                {
                    // InputReadOnly
                    binding: 2,
                    resource: this.texture_Mip3_Input_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 2 })
                },
                {
                    // OutputReadOnly
                    binding: 3,
                    resource: this.texture_Mip3_Output_ReadOnly.createView({ dimension: "3d", mipLevelCount: 1, baseMipLevel: 3 })
                },
            ]
        });
        ...
        const baseDispatchSize = [ this.variable_RenderSize[0], this.variable_RenderSize[1], this.variable_RenderSize[2] ];
        const dispatchSize = [
            Math.floor((Math.floor(((parseInt(baseDispatchSize[0]) + 0) * 1) / 8) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[1]) + 0) * 1) / 8) + 0 + 4 - 1) / 4),
            Math.floor((Math.floor(((parseInt(baseDispatchSize[2]) + 0) * 1) / 8) + 0 + 4 - 1) / 4)
        ];
    ...
    encoder.popDebugGroup(); // "Mips_CS_3D.Mip3"
...
var Mips_CS_3D = new class_Mips_CS_3D;

export default Mips_CS_3D;
