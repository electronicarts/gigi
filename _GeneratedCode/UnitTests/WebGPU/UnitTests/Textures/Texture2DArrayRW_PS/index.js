...
numExecutions = await test_Textures_Texture2DArrayRW_PS.Init(device, Texture2DArrayRW_PS);
...
for (let frameIndex = 0; frameIndex < numExecutions; ++frameIndex)
{
    ...
    if (!await Texture2DArrayRW_PS.Execute(device, encoder, true))
        Shared.LogError("Could not execute Texture2DArrayRW_PS");

    // Do post technique execution work
    test_Textures_Texture2DArrayRW_PS.NodeOnly_PostExecute(device, encoder, Texture2DArrayRW_PS, frameIndex);
    ...
    // Do validation work
    await test_Textures_Texture2DArrayRW_PS.NodeOnly_Validate(Texture2DArrayRW_PS, frameIndex);
}
