...
@binding(2) @group(0) var importedColor : texture_2d<f32>;

@binding(0) @group(0) var nodeTexture : texture_storage_2d</*(nodeTexture_format)*/, write>;

@binding(1) @group(0) var importedTexture : texture_storage_2d</*(importedTexture_format)*/, write>;
...
fn rgsmain(@builtin(global_invocation_id) DTid_0 : vec3<u32>)
{
    ...
    var loadedTexturePx_0 : vec3<f32> = (textureLoad((_loadedTexture_0), ((_S1)).xy, ((_S1)).z)).xyz;
    var _S2 : vec4<f32> = (textureLoad((importedTextureReadOnly), (vec2<i32>(px_0))));
    var importedTexturePx_0 : vec3<f32> = _S2.xyz;
    var importedColorPx_0 : vec3<f32> = (textureLoad((importedColor), ((_S1)).xy, ((_S1)).z)).xyz;
...
async Init(device, encoder, useBlockingAPIs)
{
    // Handle (re)creation of texture NodeTexture
    {
        ...
        if (this.texture_NodeTexture === null)
        {
            ...
            this.texture_NodeTexture = device.createTexture({
                label: "texture Texture2DRW_RGS.NodeTexture",
    ...
    // Handle (re)creation of texture RW_nodeTexture_ReadOnly
    {
        ...
        if (this.texture_RW_nodeTexture_ReadOnly === null)
        {
            ...
            this.texture_RW_nodeTexture_ReadOnly = device.createTexture({
                label: "texture Texture2DRW_RGS.RW_nodeTexture_ReadOnly",
    ...
    // Handle (re)creation of texture RW_importedTexture_ReadOnly
    {
        ...
        if (this.texture_RW_importedTexture_ReadOnly === null)
        {
            ...
            this.texture_RW_importedTexture_ReadOnly = device.createTexture({
                label: "texture Texture2DRW_RGS.RW_importedTexture_ReadOnly",
    ...
    // (Re)create raygen shader RW
    {
        ...
        if (this.ShaderModule_RayGen_RW === null || newHash !== this.Hash_RayGen_RW)
        {
            ...
            let shaderCode = class_Texture2DRW_RGS.ShaderCode_RW_RWRGS;
...
{
    encoder.pushDebugGroup("Texture2DRW_RGS.NodeTexture");

    encoder.popDebugGroup(); // "Texture2DRW_RGS.NodeTexture"

    encoder.pushDebugGroup("Texture2DRW_RGS.ImportedTexture");

    encoder.popDebugGroup(); // "Texture2DRW_RGS.ImportedTexture"

    encoder.pushDebugGroup("Texture2DRW_RGS.ImportedColor");

    encoder.popDebugGroup(); // "Texture2DRW_RGS.ImportedColor"

    encoder.pushDebugGroup("Texture2DRW_RGS.RW_nodeTexture_ReadOnly");

    encoder.popDebugGroup(); // "Texture2DRW_RGS.RW_nodeTexture_ReadOnly"

    encoder.pushDebugGroup("Texture2DRW_RGS.Copy_RW_nodeTexture");
    ...
    encoder.popDebugGroup(); // "Texture2DRW_RGS.Copy_RW_nodeTexture"

    encoder.pushDebugGroup("Texture2DRW_RGS.RW_importedTexture_ReadOnly");

    encoder.popDebugGroup(); // "Texture2DRW_RGS.RW_importedTexture_ReadOnly"

    encoder.pushDebugGroup("Texture2DRW_RGS.Copy_RW_importedTexture");
    ...
    encoder.popDebugGroup(); // "Texture2DRW_RGS.Copy_RW_importedTexture"

    encoder.pushDebugGroup("Texture2DRW_RGS._loadedTexture_0");

    encoder.popDebugGroup(); // "Texture2DRW_RGS._loadedTexture_0"

    encoder.pushDebugGroup("Texture2DRW_RGS._RWRGSCB");

    // Create constant buffer _RWRGSCB
    if (this.constantBuffer__RWRGSCB === null)
    {
        this.constantBuffer__RWRGSCB = device.createBuffer({
            label: "Texture2DRW_RGS._RWRGSCB",
    ...
    encoder.popDebugGroup(); // "Texture2DRW_RGS._RWRGSCB"

    encoder.pushDebugGroup("Texture2DRW_RGS.RW");
    ...
    encoder.popDebugGroup(); // "Texture2DRW_RGS.RW"
...
var Texture2DRW_RGS = new class_Texture2DRW_RGS;

export default Texture2DRW_RGS;
