import { test_Raster_Stencil } from "../../../UnitTestLogic.js";
...
import Stencil from "./Stencil_Module.js"
...
numExecutions = await test_Raster_Stencil.Init(device, Stencil);

for (let frameIndex = 0; frameIndex < numExecutions; ++frameIndex)
{
    // Run the technique
    const encoder = device.createCommandEncoder();
    test_Raster_Stencil.PreExecute(device, encoder, Stencil, frameIndex);
    if (!await Stencil.Execute(device, encoder, true))
        Shared.LogError("Could not execute Stencil");

    // Do post technique execution work
    test_Raster_Stencil.NodeOnly_PostExecute(device, encoder, Stencil, frameIndex);
    ...
    // Do validation work
    await test_Raster_Stencil.NodeOnly_Validate(Stencil, frameIndex);
}
