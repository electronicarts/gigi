...
fn csmain(@builtin(global_invocation_id) DTid_0 : vec3<u32>)
{
    ...
    var _S2 : f32 = Scene[_S1]._padding0_0;
    var _S3 : vec3<f32> = Scene[_S1].Position_0;
    var _S4 : f32 = Scene[_S1]._padding1_0;
    var vertex_0 : Struct_VertexBuffer_0;
    vertex_0.Color_0 = Scene[_S1].Color_0;
    ...
    Scene[_S1].Position_0 = _S5.Position_0;
    Scene[_S1]._padding1_0 = _S5._padding1_0;
    return;
}
...
ShaderModule_Compute_ModifyScene = null;
BindGroupLayout_Compute_ModifyScene = null;
PipelineLayout_Compute_ModifyScene = null;
Pipeline_Compute_ModifyScene = null;
...
async Init(device, encoder, useBlockingAPIs)
{
    // Handle (re)creation of texture Texture
    {
        ...
        if (this.texture_Texture === null)
        {
            ...
            this.texture_Texture = device.createTexture({
                label: "texture simpleRTDynamic.Texture",
    ...
    // Handle (re)creation of texture DoRT_g_texture_ReadOnly
    {
        ...
        if (this.texture_DoRT_g_texture_ReadOnly === null)
        {
            ...
            this.texture_DoRT_g_texture_ReadOnly = device.createTexture({
                label: "texture simpleRTDynamic.DoRT_g_texture_ReadOnly",
    ...
    // (Re)create compute shader ModifyScene
    {
        ...
        if (this.ShaderModule_Compute_ModifyScene === null || newHash !== this.Hash_Compute_ModifyScene)
        {
            ...
            this.BindGroupLayout_Compute_ModifyScene = device.createBindGroupLayout({
                label: "Compute Bind Group Layout ModifyScene",
                entries: bindGroupEntries
            });

            this.PipelineLayout_Compute_ModifyScene = device.createPipelineLayout({
                label: "Compute Pipeline Layout ModifyScene",
                bindGroupLayouts: [this.BindGroupLayout_Compute_ModifyScene],
            });

            if (useBlockingAPIs)
            {
                this.Pipeline_Compute_ModifyScene = device.createComputePipeline({
                    label: "Compute Pipeline ModifyScene",
                    layout: this.PipelineLayout_Compute_ModifyScene,
                    compute: {
                        module: this.ShaderModule_Compute_ModifyScene,
                        entryPoint: "csmain",
                    }
                });
            }
            ...
            {
                this.loadingPromises.add("ModifyScene");
                ...
                }).then( handle => { this.Pipeline_Compute_ModifyScene = handle; this.loadingPromises.delete("ModifyScene"); } );
            }
        }
    }
    ...
    // (Re)create raygen shader DoRT
    {
        ...
        if (this.ShaderModule_RayGen_DoRT === null || newHash !== this.Hash_RayGen_DoRT)
        {
            ...
            let shaderCode = this.buffer_Scene_isBVH
                ? class_simpleRTDynamic.ShaderCode_DoRT_SimpleRTRayGen_BVH
                : class_simpleRTDynamic.ShaderCode_DoRT_SimpleRTRayGen_NonBVH
            ;
...
{
    encoder.pushDebugGroup("simpleRTDynamic.Texture");

    encoder.popDebugGroup(); // "simpleRTDynamic.Texture"

    encoder.pushDebugGroup("simpleRTDynamic.Scene");

    encoder.popDebugGroup(); // "simpleRTDynamic.Scene"

    encoder.pushDebugGroup("simpleRTDynamic.DoRT_g_texture_ReadOnly");

    encoder.popDebugGroup(); // "simpleRTDynamic.DoRT_g_texture_ReadOnly"

    encoder.pushDebugGroup("simpleRTDynamic.Copy_DoRT_g_texture");
    ...
    encoder.popDebugGroup(); // "simpleRTDynamic.Copy_DoRT_g_texture"

    encoder.pushDebugGroup("simpleRTDynamic._SimpleRTRayGenCB");
    ...
    if (this.constantBuffer__SimpleRTRayGenCB === null)
    {
        this.constantBuffer__SimpleRTRayGenCB = device.createBuffer({
            label: "simpleRTDynamic._SimpleRTRayGenCB",
    ...
    // Run compute shader ModifyScene
    {
        const bindGroup = device.createBindGroup({
            label: "Compute Bind Group ModifyScene",
            layout: this.BindGroupLayout_Compute_ModifyScene,
            entries: [
                {
                    // Scene
                    binding: 0,
        ...
        const baseDispatchSize = [this.buffer_Scene_count, 1, 1];
        const dispatchSize = [
        ...
        if (this.Pipeline_Compute_ModifyScene !== null)
        {
            const computePass = encoder.beginComputePass();
                computePass.setPipeline(this.Pipeline_Compute_ModifyScene);
    ...
    encoder.popDebugGroup(); // "simpleRTDynamic.ModifyScene"

    encoder.pushDebugGroup("simpleRTDynamic.DoRT");
    ...
    encoder.popDebugGroup(); // "simpleRTDynamic.DoRT"
...
var simpleRTDynamic = new class_simpleRTDynamic;

export default simpleRTDynamic;
