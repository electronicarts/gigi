...
void UnitTestImpl(UnitTestContext& testContext, ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12Utils::ReadbackHelper& readbackHelper, buffertest_viewOffsetCS::Context* context, UnitTestEvent event)
{
    // Create buffers and put them into the context as inputs, as appropriate
    if (testContext.IsFirstPreExecute(event))
    {
        // Load, create and set buffer_InputStructuredBuffer
        {
            ...
            buffertest_viewOffsetCS::Struct_TestStruct inputStructuredBufferData;
            bool result = DX12Utils::ParseCSV::ForEachValue(file.GetBytes(), true,
                [&inputStructuredBufferData] (int tokenIndex, const char* token)
                {
                    // skip empty tokens, caused by trailing commas
    ...
    if (testContext.IsFirstPostExecute(event))
    {
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_OutputTypedBuffer, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\Buffers\\buffertest_viewOffsetCS\\0.bin");
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_OutputTypedStructBuffer, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\Buffers\\buffertest_viewOffsetCS\\1.bin");
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_OutputStructuredBuffer, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\Buffers\\buffertest_viewOffsetCS\\2.bin");
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_OutputTypedBufferRaw, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\Buffers\\buffertest_viewOffsetCS\\3.bin");
    }
}
...
void UnitTestImpl(UnitTestContext& testContext, ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12Utils::ReadbackHelper& readbackHelper, bufferViewDescriptorTable::Context* context, UnitTestEvent event)
{
    if (testContext.IsFirstPostExecute(event))
    {
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_TheBuffer, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\Buffers\\bufferViewDescriptorTable\\0.bin");
    }
}

void UnitTestImpl(UnitTestContext& testContext, ID3D12Device* device, ID3D12GraphicsCommandList* commandList, DX12Utils::ReadbackHelper& readbackHelper, CopyPartialBuffer::Context* context, UnitTestEvent event)
{
    if (testContext.IsFirstPostExecute(event))
    {
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_Output, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\CopyResource\\CopyPartialBuffer\\0.bin");
        testContext.VerifyReadbackBinary(device, commandList, context->m_output.buffer_Input, D3D12_RESOURCE_STATE_COPY_DEST, 0, 0, "..\\..\\..\\Techniques\\UnitTests\\_GoldImages\\CopyResource\\CopyPartialBuffer\\1.bin");
    }
}
