...
namespace buffertest
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // BufferTest compute shader
        {
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/BufferTest.hlsl", "Main", "cs_6_1", defines,
               ContextInternal::computeShader_BufferTest_rootSig, &ContextInternal::computeShader_BufferTest_pso, c_debugShaders, (c_debugNames ? L"BufferTest" : nullptr), Context::LogFn))
                return false;
        }
