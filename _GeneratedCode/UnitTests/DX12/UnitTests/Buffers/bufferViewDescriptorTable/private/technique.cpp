...
namespace bufferViewDescriptorTable
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Write1_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Write1_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_Write2_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Write2_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Init
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_Init_rootSig, (c_debugNames ? L"Init" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "bufferViewDescriptorTable_Init.hlsl";
        ...
        // Compute Shader: Write1
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_Write1_rootSig, (c_debugNames ? L"Write1" : nullptr), Context::LogFn))
                return false;
            ...
            if (c_debugShaders) shaderCompilationInfo.flags |= ShaderCompilationFlags::Debug;
            shaderCompilationInfo.defines.emplace_back("WRITE_VALUE","1.0f");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchMultiply","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchDivide","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPreAdd","uint3(0,0,0)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPostAdd","uint3(0,0,0)");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Write1_rootSig, &ContextInternal::computeShader_Write1_pso, Context::LogFn))
                return false;
        }

        // Compute Shader: Write2
        {
            D3D12_STATIC_SAMPLER_DESC* samplers = nullptr;
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_Write2_rootSig, (c_debugNames ? L"Write2" : nullptr), Context::LogFn))
                return false;
            ...
            if (c_debugShaders) shaderCompilationInfo.flags |= ShaderCompilationFlags::Debug;
            shaderCompilationInfo.defines.emplace_back("WRITE_VALUE","2.0f");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchMultiply","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchDivide","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPreAdd","uint3(0,0,0)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPostAdd","uint3(0,0,0)");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Write2_rootSig, &ContextInternal::computeShader_Write2_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Write1_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Write1_pso);
            ContextInternal::computeShader_Write1_pso = nullptr;
        }

        if(ContextInternal::computeShader_Write1_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Write1_rootSig);
            ContextInternal::computeShader_Write1_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Write2_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Write2_pso);
            ContextInternal::computeShader_Write2_pso = nullptr;
        }

        if(ContextInternal::computeShader_Write2_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Write2_rootSig);
            ContextInternal::computeShader_Write2_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Write1
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Write2
    ...
    Context::~Context()
    {
        ...
        if(m_output.buffer_TheBuffer)
        {
            s_delayedRelease.Add(m_output.buffer_TheBuffer);
            m_output.buffer_TheBuffer = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("bufferViewDescriptorTable", commandList, 4);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"bufferViewDescriptorTable Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (3+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"bufferViewDescriptorTable Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].UAV.pResource = context->m_output.buffer_TheBuffer;
        ...
        // Compute Shader: Init
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Init", commandList, 2);
            ...
            unsigned int baseDispatchSize[3] = { context->m_output.buffer_TheBuffer_count, 1, 1 };
        ...
        // Compute Shader: Write1
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Write1", commandList, 1);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Write1_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Write1_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.buffer_TheBuffer, context->m_output.buffer_TheBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_TheBuffer_stride, context->m_output.buffer_TheBuffer_count, 0, 4, 4, false }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Write1";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Compute Shader: Write2
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Write2", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Write2_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Write2_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.buffer_TheBuffer, context->m_output.buffer_TheBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_TheBuffer_stride, context->m_output.buffer_TheBuffer_count, 0, 0, 4, false }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Write2";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // TheBuffer
        {
            ...
            if(!m_output.buffer_TheBuffer ||
               m_output.buffer_TheBuffer_count != desiredCount ||
               m_output.buffer_TheBuffer_format != desiredFormat ||
               m_output.buffer_TheBuffer_stride != desiredStride)
            {
                dirty = true;
                if(m_output.buffer_TheBuffer)
                    s_delayedRelease.Add(m_output.buffer_TheBuffer);
                ...
                m_output.buffer_TheBuffer = DX12Utils::CreateBuffer(device, desiredSize, m_output.c_buffer_TheBuffer_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"TheBuffer" : nullptr), Context::LogFn);
                m_output.buffer_TheBuffer_count = desiredCount;
                m_output.buffer_TheBuffer_format = desiredFormat;
                m_output.buffer_TheBuffer_stride = desiredStride;
            }
