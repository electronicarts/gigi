...
namespace bufferViewDescriptorTable
{
    ...
    struct Context
    {
        ...
        {
            return "bufferViewDescriptorTable";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"bufferViewDescriptorTable";
        }
        ...
        struct ContextOutput
        {
            ...
            ID3D12Resource* buffer_TheBuffer = nullptr;
            DXGI_FORMAT buffer_TheBuffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_TheBuffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_TheBuffer_count = 0; // How many items there are
            const D3D12_RESOURCE_STATES c_buffer_TheBuffer_endingState = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;

            static const D3D12_RESOURCE_FLAGS c_buffer_TheBuffer_flags =  D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS; // Flags the buffer needs to have been created with
        };
