...
namespace StructuredBuffer
{
    ...
    Context::~Context()
    {
        ...
        // _csmainCB
        if (m_internal.constantBuffer__csmainCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__csmainCB);
            m_internal.constantBuffer__csmainCB = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Compute Shader: csmain
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_input.buffer_buff, context->m_input.buffer_buff_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_input.buffer_buff_stride, context->m_input.buffer_buff_count },
                { context->m_internal.constantBuffer__csmainCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _csmainCB
        if (m_internal.constantBuffer__csmainCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__csmainCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_csmainCB" : nullptr), Context::LogFn);
        }
