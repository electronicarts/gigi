...
namespace StructuredBuffer
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_csmain_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_csmain_rootSig = nullptr;
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_csmain_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_csmain_pso);
            ContextInternal::computeShader_csmain_pso = nullptr;
        }

        if(ContextInternal::computeShader_csmain_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_csmain_rootSig);
            ContextInternal::computeShader_csmain_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: csmain
    ...
    Context::~Context()
    {
        ...
        // _csmain_0CB
        if (m_internal.constantBuffer__csmain_0CB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__csmain_0CB);
            m_internal.constantBuffer__csmain_0CB = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("StructuredBuffer", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"StructuredBuffer Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"StructuredBuffer Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        if (!context->m_input.buffer_buff)
        {
            Context::LogFn(LogLevel::Error, "StructuredBuffer: Imported buffer \"buff\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_buff_state != D3D12_RESOURCE_STATE_UNORDERED_ACCESS)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_buff;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_buff_state;
            ...
            else
            {
                ...
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].UAV.pResource = context->m_input.buffer_buff;
                barrierCount++;
            }
        ...
        {
            context->m_internal.constantBuffer__csmain_0CB_cpu.frameDeltaTime = context->m_input.variable_frameDeltaTime;
            context->m_internal.constantBuffer__csmain_0CB_cpu.frameIndex = context->m_input.variable_frameIndex;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__csmain_0CB, context->m_internal.constantBuffer__csmain_0CB_cpu, Context::LogFn);
        }

        // Compute Shader: csmain
        {
            ScopedPerfEvent scopedPerf("Compute Shader: csmain", commandList, 1);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_csmain_rootSig);
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_internal.constantBuffer__csmain_0CB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false }
            };
            ...
            unsigned int baseDispatchSize[3] = { context->m_input.buffer_buff_count, 1, 1 };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "csmain";
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_buff_state != D3D12_RESOURCE_STATE_UNORDERED_ACCESS)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_buff_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
            else
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _csmain_0CB
        if (m_internal.constantBuffer__csmain_0CB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__csmain_0CB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_csmain_0CB" : nullptr), Context::LogFn);
        }
