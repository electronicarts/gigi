...
namespace StructuredBuffer
{
    ...
    struct Context
    {
        ...
        struct ContextInput
        {
            ...
            unsigned int buffer_buff_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_buff_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_buff_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_buff_flags =  D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS; // Flags the buffer needs to have been created with
        };
