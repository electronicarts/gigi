...
namespace buffertest_viewOffsetCS
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("buffertest_viewOffsetCS", commandList, 10);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"buffertest_viewOffsetCS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"buffertest_viewOffsetCS Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "buffertest_viewOffsetCS: Imported buffer \"InputTypedBuffer\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_InputStructuredBuffer)
        {
            Context::LogFn(LogLevel::Error, "buffertest_viewOffsetCS: Imported buffer \"InputStructuredBuffer\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_InputTypedBufferRaw)
        {
            Context::LogFn(LogLevel::Error, "buffertest_viewOffsetCS: Imported buffer \"InputTypedBufferRaw\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_InputTypedStructBuffer)
        {
            Context::LogFn(LogLevel::Error, "buffertest_viewOffsetCS: Imported buffer \"InputTypedStructBuffer\" is null.\n");
            return;
        }
        ...
        // BufferTest compute shader
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_output.buffer_OutputTypedBuffer, context->m_output.buffer_OutputTypedBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_OutputTypedBuffer_stride, context->m_output.buffer_OutputTypedBuffer_count, 0, 1, 0, false },
                ...
                { context->m_output.buffer_OutputTypedStructBuffer, context->m_output.buffer_OutputTypedStructBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_OutputTypedStructBuffer_stride, context->m_output.buffer_OutputTypedStructBuffer_count, 0, 4, 0, true },
                ...
                { context->m_output.buffer_OutputStructuredBuffer, context->m_output.buffer_OutputStructuredBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_OutputStructuredBuffer_stride, context->m_output.buffer_OutputStructuredBuffer_count, 0, 1, 0, false },
                ...
                { context->m_output.buffer_OutputTypedBufferRaw, context->m_output.buffer_OutputTypedBufferRaw_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, true, context->m_output.buffer_OutputTypedBufferRaw_stride, context->m_output.buffer_OutputTypedBufferRaw_count, 0, 16, 0, true },
