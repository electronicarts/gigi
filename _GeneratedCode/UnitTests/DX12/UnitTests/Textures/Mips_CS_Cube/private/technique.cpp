...
namespace Mips_CS_Cube
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Mip0
        {
            ...
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Mips_CS_Cube_Mip0CS.hlsl";
        ...
        // Compute Shader: Mip1
        {
            ...
            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Mips_CS_Cube_MipNCS.hlsl";
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Mips_CS_Cube", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Mips_CS_Cube Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (4+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Mips_CS_Cube Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Compute Shader: Mip0
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 0, 0, 0, false },
            ...
            unsigned int baseDispatchSize[3] = { (unsigned int)context->m_input.variable_RenderSize[0], (unsigned int)context->m_input.variable_RenderSize[1], 1 };
        ...
        // Compute Shader: Mip1
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 0, 0, 0, false },
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 1, 0, 0, false }
            };
            ...
            unsigned int dispatchSize[3] = {
                ...
                (((baseDispatchSize[1] + 0) * 1) / 2 + 0 + 8 - 1) / 8,
                (((baseDispatchSize[2] + 0) * 1) / 1 + 0 + 1 - 1) / 1
            };
        ...
        // Compute Shader: Mip2
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 1, 0, 0, false },
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 2, 0, 0, false }
            };
            ...
            unsigned int dispatchSize[3] = {
                (((baseDispatchSize[0] + 0) * 1) / 4 + 0 + 8 - 1) / 8,
                (((baseDispatchSize[1] + 0) * 1) / 4 + 0 + 8 - 1) / 8,
                (((baseDispatchSize[2] + 0) * 1) / 1 + 0 + 1 - 1) / 1
            };
        ...
        // Compute Shader: Mip3
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 2, 0, 0, false },
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 3, 0, 0, false }
            };
            ...
            unsigned int dispatchSize[3] = {
                (((baseDispatchSize[0] + 0) * 1) / 8 + 0 + 8 - 1) / 8,
                (((baseDispatchSize[1] + 0) * 1) / 8 + 0 + 8 - 1) / 8,
                (((baseDispatchSize[2] + 0) * 1) / 1 + 0 + 1 - 1) / 1
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Output
        {
               ...
               m_output.texture_Output_format != desiredFormat)
            {
                ...
                m_output.texture_Output = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Output_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::TextureCube, (c_debugNames ? L"Output" : nullptr), Context::LogFn);
                m_output.texture_Output_size[0] = desiredSize[0];
                m_output.texture_Output_size[1] = desiredSize[1];
                m_output.texture_Output_size[2] = desiredSize[2];
                m_output.texture_Output_numMips = desiredNumMips;
