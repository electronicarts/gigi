...
namespace Mips_VSPS_2D
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Mips_VSPS_2D", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Mips_VSPS_2D Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Mips_VSPS_2D Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Draw Call: Node_1
        {
            ScopedPerfEvent scopedPerf("Draw Call: Node_1", commandList, 1);
            ...
            int renderWidth = context->m_internal.texture_Output_size[0];
            int renderHeight = context->m_internal.texture_Output_size[1];
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Output
        {
               ...
               m_internal.texture_Output_format != desiredFormat)
            {
                ...
                m_internal.texture_Output = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_internal.texture_Output_flags, D3D12_RESOURCE_STATE_RENDER_TARGET, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Output" : nullptr), Context::LogFn);
                m_internal.texture_Output_size[0] = desiredSize[0];
                m_internal.texture_Output_size[1] = desiredSize[1];
                m_internal.texture_Output_size[2] = desiredSize[2];
                m_internal.texture_Output_numMips = desiredNumMips;
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Node_1_pso || !m_internal.drawCall_Node_1_rootSig)
        {
            ...
            // MipTexVS
            rangesVertex[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Mips_VSPS_2D_VS.hlsl", "main", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Mips_VSPS_2D_PS.hlsl", "main", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            ...
            psoDesc.RTVFormats[0] = m_internal.texture_Output_format;
