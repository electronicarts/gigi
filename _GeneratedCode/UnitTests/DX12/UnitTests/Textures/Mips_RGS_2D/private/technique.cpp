...
namespace Mips_RGS_2D
{
    ...
    ID3D12StateObject* ContextInternal::rayShader_RayGen_rtso = nullptr;
    ID3D12RootSignature* ContextInternal::rayShader_RayGen_rootSig = nullptr;
    ID3D12Resource* ContextInternal::rayShader_RayGen_shaderTableRayGen = nullptr;
    unsigned int    ContextInternal::rayShader_RayGen_shaderTableRayGenSize = 0;
    ID3D12Resource* ContextInternal::rayShader_RayGen_shaderTableMiss = nullptr;
    unsigned int    ContextInternal::rayShader_RayGen_shaderTableMissSize = 0;
    ID3D12Resource* ContextInternal::rayShader_RayGen_shaderTableHitGroup = nullptr;
    unsigned int    ContextInternal::rayShader_RayGen_shaderTableHitGroupSize = 0;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Ray Shader: RayGen
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::rayShader_RayGen_rootSig, (c_debugNames ? L"RayGen" : nullptr), Context::LogFn))
                return false;
            ...
            // Compile shaders
            std::vector<unsigned char> shaderCode[2];

            // Compile RTMiss : Mips_RGS_2D_RGS.hlsl Miss()
            shaderCode[0] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Mips_RGS_2D_RGS.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[0].empty())
                return false;

            // Compile RTRayGen : Mips_RGS_2D_RGS.hlsl main()
            shaderCode[1] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Mips_RGS_2D_RGS.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            ...
            // DXIL Library for RTMiss : Mips_RGS_2D_RGS.hlsl Miss()
            {
            ...
            // DXIL Library for RTRayGen : Mips_RGS_2D_RGS.hlsl main()
            {
                static D3D12_EXPORT_DESC exportDesc;
                exportDesc.Name = L"main_1";
                exportDesc.ExportToRename = L"main";
                exportDesc.Flags = D3D12_EXPORT_FLAG_NONE;
            ...
            // Hit group: EmptyHG
            {
                ...
                subObjects[2].pDesc = &hitGroupDesc;
            }
            ...
            subObjects[3].pDesc = &payloadDesc;

            // Associate payload with shaders
            const WCHAR* shaderExports[] = { L"Miss_0", L"main_1" };

            D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION shaderPayloadAssociation = {};
            shaderPayloadAssociation.NumExports = 2;
            shaderPayloadAssociation.pExports = shaderExports;
            shaderPayloadAssociation.pSubobjectToAssociate = &subObjects[3];

            subObjects[4].Type = D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION;
            subObjects[4].pDesc = &shaderPayloadAssociation;
            ...
            subObjects[6].pDesc = &ContextInternal::rayShader_RayGen_rootSig;
            if (FAILED(dxrDevice->CreateStateObject(&soDesc, IID_PPV_ARGS(&ContextInternal::rayShader_RayGen_rtso))))
                return false;

            if (c_debugNames)
                ContextInternal::rayShader_RayGen_rtso->SetName(L"RayGen state object");

            // Create the shader tables
            {
                ID3D12StateObjectProperties* soprops = nullptr;
                if(FAILED(ContextInternal::rayShader_RayGen_rtso->QueryInterface(IID_PPV_ARGS(&soprops))))
                    return false;

                // make the ray gen shader table and fill it out
                {
                    ContextInternal::rayShader_RayGen_shaderTableRayGenSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(1 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_RayGen_shaderTableRayGen = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_RayGen_shaderTableRayGenSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"RayGen shader table ray gen" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_RayGen_shaderTableRayGen->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"main_1"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_RayGen_shaderTableRayGen->Unmap(0, nullptr);
                }

                // make the miss shader table and fill it out
                {
                    ContextInternal::rayShader_RayGen_shaderTableMissSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(1 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_RayGen_shaderTableMiss = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_RayGen_shaderTableMissSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"RayGen shader table miss" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_RayGen_shaderTableMiss->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"Miss_0"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_RayGen_shaderTableMiss->Unmap(0, nullptr);
                }

                // make the hit group shader table and fill it out
                {
                    ContextInternal::rayShader_RayGen_shaderTableHitGroupSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(1 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_RayGen_shaderTableHitGroup = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_RayGen_shaderTableHitGroupSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"RayGen shader table hit group" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_RayGen_shaderTableHitGroup->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"hitgroup0"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_RayGen_shaderTableHitGroup->Unmap(0, nullptr);
                }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::rayShader_RayGen_rtso)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_RayGen_rtso);
            ContextInternal::rayShader_RayGen_rtso = nullptr;
        }

        if(ContextInternal::rayShader_RayGen_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_RayGen_rootSig);
            ContextInternal::rayShader_RayGen_rootSig = nullptr;
        }

        if(ContextInternal::rayShader_RayGen_shaderTableRayGen)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_RayGen_shaderTableRayGen);
            ContextInternal::rayShader_RayGen_shaderTableRayGen = nullptr;
        }

        if(ContextInternal::rayShader_RayGen_shaderTableMiss)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_RayGen_shaderTableMiss);
            ContextInternal::rayShader_RayGen_shaderTableMiss = nullptr;
        }

        if(ContextInternal::rayShader_RayGen_shaderTableHitGroup)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_RayGen_shaderTableHitGroup);
            ContextInternal::rayShader_RayGen_shaderTableHitGroup = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // ray shader: RayGen
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_MipTex)
        {
            s_delayedRelease.Add(m_output.texture_MipTex);
            m_output.texture_MipTex = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Mips_RGS_2D", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Mips_RGS_2D Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Mips_RGS_2D Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        if (!context->m_input.buffer_VertexBuffer_blas)
        {
            Context::LogFn(LogLevel::Error, "Mips_RGS_2D: Imported buffer \"VertexBuffer\" needs a BLAS/TLAS. Use CreateManagedTLAS to create them.\n");
            return;
        }

        if (!context->m_input.buffer_VertexBuffer)
        {
            Context::LogFn(LogLevel::Error, "Mips_RGS_2D: Imported buffer \"VertexBuffer\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_VertexBuffer_state != D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE)
            {
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].UAV.pResource = context->m_output.texture_MipTex;
        ...
        // Ray Shader: RayGen
        {
            ScopedPerfEvent scopedPerf("Ray Shader: RayGen", commandList, 0);
            ...
            commandList->SetComputeRootSignature(ContextInternal::rayShader_RayGen_rootSig);
            ...
            unsigned int baseDispatchSize[3] = {
                context->m_output.texture_MipTex_size[0],
                context->m_output.texture_MipTex_size[1],
                context->m_output.texture_MipTex_size[2]
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "RayGen";
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // MipTex
        {
            ...
            if(!m_output.texture_MipTex ||
               m_output.texture_MipTex_size[0] != desiredSize[0] ||
               m_output.texture_MipTex_size[1] != desiredSize[1] ||
               m_output.texture_MipTex_size[2] != desiredSize[2] ||
               m_output.texture_MipTex_numMips != desiredNumMips ||
               m_output.texture_MipTex_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_MipTex)
                    s_delayedRelease.Add(m_output.texture_MipTex);

                m_output.texture_MipTex = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_MipTex_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"MipTex" : nullptr), Context::LogFn);
                m_output.texture_MipTex_size[0] = desiredSize[0];
                m_output.texture_MipTex_size[1] = desiredSize[1];
                m_output.texture_MipTex_size[2] = desiredSize[2];
                m_output.texture_MipTex_numMips = desiredNumMips;
