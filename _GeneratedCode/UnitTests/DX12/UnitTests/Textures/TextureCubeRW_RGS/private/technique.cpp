...
namespace TextureCubeRW_RGS
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Ray Shader: RW
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 4, samplers, 1, &ContextInternal::rayShader_RW_rootSig, (c_debugNames ? L"RW" : nullptr), Context::LogFn))
                return false;
            ...
            // Compile RTClosestHit : TextureCubeRW_CHS_MISS.hlsl chsmain()
            shaderCode[0] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_CHS_MISS.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[0].empty())
                return false;

            // Compile RTMiss : TextureCubeRW_CHS_MISS.hlsl missmain()
            shaderCode[1] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_CHS_MISS.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[1].empty())
                return false;

            // Compile RTRayGen : TextureCubeRW_RGS.hlsl rgsmain()
            shaderCode[2] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_RGS.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            ...
            // DXIL Library for RTClosestHit : TextureCubeRW_CHS_MISS.hlsl chsmain()
            {
            ...
            // DXIL Library for RTMiss : TextureCubeRW_CHS_MISS.hlsl missmain()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            // DXIL Library for RTRayGen : TextureCubeRW_RGS.hlsl rgsmain()
            {
                static D3D12_EXPORT_DESC exportDesc;
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("TextureCubeRW_RGS", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"TextureCubeRW_RGS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"TextureCubeRW_RGS Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_RGS: Imported texture \"ImportedTexture\" is null.\n");
            return;
        }

        if (!context->m_input.texture_ImportedColor)
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_RGS: Imported texture \"ImportedColor\" is null.\n");
            return;
        }
        ...
        // Ray Shader: RW
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_NodeTexture, context->m_output.texture_NodeTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6 },
                { context->m_input.texture_ImportedTexture, context->m_input.texture_ImportedTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::TextureCube, false, 0, 6 },
                { context->m_input.texture_ImportedColor, context->m_input.texture_ImportedColor_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::TextureCube, false, 0, 6 },
                { context->m_internal.texture__loadedTexture_0, context->m_internal.texture__loadedTexture_0_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::TextureCube, false, 0, 6 }
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // NodeTexture
        {
               ...
               m_output.texture_NodeTexture_format != desiredFormat)
            {
                ...
                m_output.texture_NodeTexture = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_NodeTexture_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::TextureCube, (c_debugNames ? L"NodeTexture" : nullptr), Context::LogFn);
                m_output.texture_NodeTexture_size[0] = desiredSize[0];
                m_output.texture_NodeTexture_size[1] = desiredSize[1];
                m_output.texture_NodeTexture_size[2] = desiredSize[2];
                m_output.texture_NodeTexture_format = desiredFormat;
        ...
        // _loadedTexture_0
        {
            if (!m_internal.texture__loadedTexture_0)
            {
                ...
                while(1)
                {
                    ...
                    sprintf_s(indexedFileName, "%lsassets/cube%s.png", s_techniqueLocation.c_str(), c_cubeMapNames[textureIndex]);
                ...
                m_internal.texture__loadedTexture_0 = DX12Utils::CreateTexture(device, size, DXGI_FORMAT_R8G8B8A8_UNORM, m_internal.texture__loadedTexture_0_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::TextureCube, (c_debugNames ? L"_loadedTexture_0" : nullptr), Context::LogFn);
