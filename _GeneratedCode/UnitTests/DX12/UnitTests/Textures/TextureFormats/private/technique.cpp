...
namespace TextureFormats
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: ComputeShader
        {
            ...
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "TextureFormats.hlsl";
            shaderCompilationInfo.entryPoint = "csmain";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "ComputeShader" : "");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_ComputeShader_rootSig, &ContextInternal::computeShader_ComputeShader_pso, Context::LogFn))
                return false;
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Compute Shader: ComputeShader
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_input.texture_Texture, context->m_input.texture_Texture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false }
            };
