...
namespace TextureCubeRW_PS
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("TextureCubeRW_PS", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"TextureCubeRW_PS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"TextureCubeRW_PS Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_PS: Imported texture \"ImportedTexture\" is null.\n");
            return;
        }

        if (!context->m_input.texture_ImportedColor)
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_PS: Imported texture \"ImportedColor\" is null.\n");
            return;
        }

        if (!context->m_input.texture_Color)
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_PS: Imported texture \"Color\" is null.\n");
            return;
        }

        if (context->m_input.texture_Color_rtv == -1)
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_PS: Imported texture \"Color\" needs an RTV. Use CreateManagedRTV to create one.\n");
            return;
        }
        ...
        // Draw Call: Node_1
        {
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->m_input.texture_Color_rtv),
            };
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Node_1_pso || !m_internal.drawCall_Node_1_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_VS.hlsl", "vsmain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_PS.hlsl", "psmain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
