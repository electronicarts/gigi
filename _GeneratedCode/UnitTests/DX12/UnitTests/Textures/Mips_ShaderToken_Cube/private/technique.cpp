...
namespace Mips_ShaderToken_Cube
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: MakeOutput
        {
            ...
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Mips_ShaderToken_Cube_CS.hlsl";
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Mips_ShaderToken_Cube", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Mips_ShaderToken_Cube Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Mips_ShaderToken_Cube Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Compute Shader: MakeOutput
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_internal.texture__loadedTexture_0, context->m_internal.texture__loadedTexture_0_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::TextureCube, false, 0, 6, 0, 0, 0, false }
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _loadedTexture_0
        {
            if (!m_internal.texture__loadedTexture_0)
            {
                ...
                m_internal.texture__loadedTexture_0 = DX12Utils::CreateTexture(device, size, desiredNumMips, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, m_internal.texture__loadedTexture_0_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::TextureCube, (c_debugNames ? L"_loadedTexture_0" : nullptr), Context::LogFn);
