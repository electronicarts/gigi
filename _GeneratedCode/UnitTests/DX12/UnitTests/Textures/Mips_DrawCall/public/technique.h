...
namespace Mips_DrawCall
{
    ...
    struct Context
    {
        ...
        {
            return "Mips_DrawCall";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"Mips_DrawCall";
        }
        ...
        struct ContextInput
        {
            ...
            ID3D12Resource* buffer_SphereVB = nullptr;
            DXGI_FORMAT buffer_SphereVB_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_SphereVB_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_SphereVB_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_SphereVB_state = D3D12_RESOURCE_STATE_COMMON;
            std::vector<D3D12_INPUT_ELEMENT_DESC> buffer_SphereVB_vertexInputLayout;

            static const D3D12_RESOURCE_FLAGS c_buffer_SphereVB_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
        ...
        struct ContextOutput
        {
            ...
            ID3D12Resource* texture_Final_Color_Buffer = nullptr;
            unsigned int texture_Final_Color_Buffer_size[3] = { 0, 0, 0 };
            unsigned int texture_Final_Color_Buffer_numMips = 0;
            DXGI_FORMAT texture_Final_Color_Buffer_format = DXGI_FORMAT_UNKNOWN;
            static const D3D12_RESOURCE_FLAGS texture_Final_Color_Buffer_flags =  D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
            const D3D12_RESOURCE_STATES c_texture_Final_Color_Buffer_endingState = D3D12_RESOURCE_STATE_RENDER_TARGET;

            ID3D12Resource* texture_Final_Depth_Buffer = nullptr;
            unsigned int texture_Final_Depth_Buffer_size[3] = { 0, 0, 0 };
            unsigned int texture_Final_Depth_Buffer_numMips = 0;
            DXGI_FORMAT texture_Final_Depth_Buffer_format = DXGI_FORMAT_UNKNOWN;
            static const D3D12_RESOURCE_FLAGS texture_Final_Depth_Buffer_flags =  D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
            const D3D12_RESOURCE_STATES c_texture_Final_Depth_Buffer_endingState = D3D12_RESOURCE_STATE_DEPTH_WRITE;
        };
