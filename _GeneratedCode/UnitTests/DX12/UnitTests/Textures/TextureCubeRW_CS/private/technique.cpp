...
namespace TextureCubeRW_CS
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: RW
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 4, samplers, 1, &ContextInternal::computeShader_RW_rootSig, (c_debugNames ? L"RW" : nullptr), Context::LogFn))
                return false;

            D3D_SHADER_MACRO defines[] = {
                { "__GigiDispatchMultiply", "uint3(1,1,0)" },
                { "__GigiDispatchDivide", "uint3(1,1,1)" },
                { "__GigiDispatchPreAdd", "uint3(0,0,0)" },
                { "__GigiDispatchPostAdd", "uint3(0,0,1)" },
                { nullptr, nullptr }
            };

            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/TextureCubeRW_CS.hlsl", "csmain", "cs_6_1", defines,
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("TextureCubeRW_CS", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"TextureCubeRW_CS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"TextureCubeRW_CS Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_CS: Imported texture \"ImportedTexture\" is null.\n");
            return;
        }

        if (!context->m_input.texture_ImportedColor)
        {
            Context::LogFn(LogLevel::Error, "TextureCubeRW_CS: Imported texture \"ImportedColor\" is null.\n");
            return;
        }
        ...
        // Compute Shader: RW
        {
            ...
            unsigned int dispatchSize[3] = {
                ...
                (((baseDispatchSize[2] + 0) * 0) / 1 + 1 + 1 - 1) / 1
            };
