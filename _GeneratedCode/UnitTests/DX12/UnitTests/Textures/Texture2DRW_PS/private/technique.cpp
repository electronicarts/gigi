...
namespace Texture2DRW_PS
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Draw Call: RW
        {
            ...
            {
                { context->m_output.texture_NodeTexture, context->m_output.texture_NodeTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_input.texture_ImportedTexture, context->m_input.texture_ImportedTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_input.texture_ImportedColor, context->m_input.texture_ImportedColor_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_RW_pso || !m_internal.drawCall_RW_rootSig)
        {
            ...
            shaderCompilationInfoVS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Texture2DRW_VS.hlsl";
            shaderCompilationInfoVS.entryPoint = "vsmain";
            shaderCompilationInfoVS.shaderModel = "vs_6_1";
            shaderCompilationInfoVS.debugName = (c_debugNames ? "RW" : "");
            ...
            shaderCompilationInfoPS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Texture2DRW_PS.hlsl";
            shaderCompilationInfoPS.entryPoint = "psmain";
            shaderCompilationInfoPS.shaderModel = "ps_6_1";
            shaderCompilationInfoPS.debugName = (c_debugNames ? "RW" : "");
