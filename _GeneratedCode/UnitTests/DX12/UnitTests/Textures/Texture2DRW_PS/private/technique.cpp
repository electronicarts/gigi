...
namespace Texture2DRW_PS
{
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // Draw Call: RW
    ...
    Context::~Context()
    {
        ...
        if(m_internal.drawCall_RW_pso)
        {
            s_delayedRelease.Add(m_internal.drawCall_RW_pso);
            m_internal.drawCall_RW_pso = nullptr;
        }

        if(m_internal.drawCall_RW_rootSig)
        {
            s_delayedRelease.Add(m_internal.drawCall_RW_rootSig);
            m_internal.drawCall_RW_rootSig = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Texture2DRW_PS", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Texture2DRW_PS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Texture2DRW_PS Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "Texture2DRW_PS: Imported texture \"ImportedTexture\" is null.\n");
            return;
        }

        if (!context->m_input.texture_ImportedColor)
        {
            Context::LogFn(LogLevel::Error, "Texture2DRW_PS: Imported texture \"ImportedColor\" is null.\n");
            return;
        }

        if (!context->m_input.texture_Color)
        {
            Context::LogFn(LogLevel::Error, "Texture2DRW_PS: Imported texture \"Color\" is null.\n");
            return;
        }
        ...
        // Draw Call: RW
        {
            ScopedPerfEvent scopedPerf("Draw Call: RW", commandList, 3);
            ...
            commandList->SetGraphicsRootSignature(context->m_internal.drawCall_RW_rootSig);
            commandList->SetPipelineState(context->m_internal.drawCall_RW_pso);

            DX12Utils::ResourceDescriptor descriptorsPS[] =
            {
                { context->m_output.texture_NodeTexture, context->m_output.texture_NodeTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0 },
                { context->m_input.texture_ImportedTexture, context->m_input.texture_ImportedTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0 },
                { context->m_input.texture_ImportedColor, context->m_input.texture_ImportedColor_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0 },
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_input.texture_Color, context->m_input.texture_Color_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "Texture2DRW_PS.Color"))
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // NodeTexture
        {
               ...
               m_output.texture_NodeTexture_format != desiredFormat)
            {
                ...
                m_output.texture_NodeTexture = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_NodeTexture_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"NodeTexture" : nullptr), Context::LogFn);
                m_output.texture_NodeTexture_size[0] = desiredSize[0];
                m_output.texture_NodeTexture_size[1] = desiredSize[1];
                m_output.texture_NodeTexture_size[2] = desiredSize[2];
                m_output.texture_NodeTexture_numMips = desiredNumMips;
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_RW_pso || !m_internal.drawCall_RW_rootSig)
        {
            ...
            hr = device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&m_internal.drawCall_RW_rootSig));
            if (FAILED(hr))
            {
                const char* errorMsg = (error ? (const char*)error->GetBufferPointer() : nullptr);
                if (errorMsg)
            ...
            if (c_debugNames)
                m_internal.drawCall_RW_rootSig->SetName(L"RW");

            D3D_SHADER_MACRO* definesVS = nullptr;

            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Texture2DRW_VS.hlsl", "vsmain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/Texture2DRW_PS.hlsl", "psmain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            ...
            psoDesc.pRootSignature = m_internal.drawCall_RW_rootSig;
            ...
            hr = device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_internal.drawCall_RW_pso));

            if (FAILED(hr))
            {
                Context::LogFn(LogLevel::Error, "Could not create PSO for RW");
                return false;
            }

            // name the PSO for debuggers
            if (c_debugNames)
                m_internal.drawCall_RW_pso->SetName(L"RW");
        }
        return true;
    }
};
