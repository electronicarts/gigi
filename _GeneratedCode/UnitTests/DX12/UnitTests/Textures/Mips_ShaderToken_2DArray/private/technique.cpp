...
namespace Mips_ShaderToken_2DArray
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_MakeOutput_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_MakeOutput_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: MakeOutput
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_MakeOutput_rootSig, (c_debugNames ? L"MakeOutput" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Mips_ShaderToken_2DArray_CS.hlsl";
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "MakeOutput" : "");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_MakeOutput_rootSig, &ContextInternal::computeShader_MakeOutput_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_MakeOutput_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_MakeOutput_pso);
            ContextInternal::computeShader_MakeOutput_pso = nullptr;
        }

        if(ContextInternal::computeShader_MakeOutput_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_MakeOutput_rootSig);
            ContextInternal::computeShader_MakeOutput_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: MakeOutput
    ...
    Context::~Context()
    {
        ...
        if(m_internal.texture_Output)
        {
            s_delayedRelease.Add(m_internal.texture_Output);
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Mips_ShaderToken_2DArray", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Mips_ShaderToken_2DArray Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Mips_ShaderToken_2DArray Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].UAV.pResource = context->m_internal.texture_Output;
        ...
        // Compute Shader: MakeOutput
        {
            ScopedPerfEvent scopedPerf("Compute Shader: MakeOutput", commandList, 1);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_MakeOutput_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_MakeOutput_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_internal.texture_Output, context->m_internal.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_internal.texture__loadedTexture_0, context->m_internal.texture__loadedTexture_0_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture2DArray, false, 0, context->m_internal.texture__loadedTexture_0_size[2], 0, 0, 0, false }
            };
            ...
            unsigned int baseDispatchSize[3] = {
                context->m_internal.texture_Output_size[0],
                context->m_internal.texture_Output_size[1],
                context->m_internal.texture_Output_size[2]
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "MakeOutput";
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Output
        {
            ...
            if(!m_internal.texture_Output ||
               m_internal.texture_Output_size[0] != desiredSize[0] ||
               m_internal.texture_Output_size[1] != desiredSize[1] ||
               m_internal.texture_Output_size[2] != desiredSize[2] ||
               m_internal.texture_Output_numMips != desiredNumMips ||
               m_internal.texture_Output_format != desiredFormat)
            {
                dirty = true;
                if(m_internal.texture_Output)
                    s_delayedRelease.Add(m_internal.texture_Output);

                m_internal.texture_Output = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_internal.texture_Output_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Output" : nullptr), Context::LogFn);
                m_internal.texture_Output_size[0] = desiredSize[0];
                m_internal.texture_Output_size[1] = desiredSize[1];
                m_internal.texture_Output_size[2] = desiredSize[2];
                m_internal.texture_Output_numMips = desiredNumMips;
        ...
        // _loadedTexture_0
        {
            if (!m_internal.texture__loadedTexture_0)
            {
                ...
                m_internal.texture__loadedTexture_0 = DX12Utils::CreateTexture(device, size, desiredNumMips, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB, m_internal.texture__loadedTexture_0_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::Texture2DArray, (c_debugNames ? L"_loadedTexture_0" : nullptr), Context::LogFn);
