...
namespace Texture2DArrayRW_PS
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Draw Call: Node_1
        {
            ...
            {
                { context->m_output.texture_NodeTexture, context->m_output.texture_NodeTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2DArray, false, 0, context->m_output.texture_NodeTexture_size[2], 0, 0, 0, false },
                { context->m_input.texture_ImportedTexture, context->m_input.texture_ImportedTexture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2DArray, false, 0, context->m_input.texture_ImportedTexture_size[2], 0, 0, 0, false },
                { context->m_input.texture_ImportedColor, context->m_input.texture_ImportedColor_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Texture2DArray, false, 0, context->m_input.texture_ImportedColor_size[2], 0, 0, 0, false },
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_input.texture_Color, context->m_input.texture_Color_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "Texture2DArrayRW_PS.Color"))
            };
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Node_1_pso || !m_internal.drawCall_Node_1_rootSig)
        {
            ...
            shaderCompilationInfoVS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Texture2DArrayRW_VS.hlsl";
            ...
            shaderCompilationInfoPS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "Texture2DArrayRW_PS.hlsl";
