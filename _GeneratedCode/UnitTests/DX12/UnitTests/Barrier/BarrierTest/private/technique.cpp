...
namespace BarrierTest
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Draw_Left_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Draw_Left_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_Draw_Right_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Draw_Right_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_After_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_After_rootSig = nullptr;
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Draw_Left_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Draw_Left_pso);
            ContextInternal::computeShader_Draw_Left_pso = nullptr;
        }

        if(ContextInternal::computeShader_Draw_Left_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Draw_Left_rootSig);
            ContextInternal::computeShader_Draw_Left_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Draw_Right_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Draw_Right_pso);
            ContextInternal::computeShader_Draw_Right_pso = nullptr;
        }

        if(ContextInternal::computeShader_Draw_Right_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Draw_Right_rootSig);
            ContextInternal::computeShader_Draw_Right_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_After_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_After_pso);
            ContextInternal::computeShader_After_pso = nullptr;
        }

        if(ContextInternal::computeShader_After_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_After_rootSig);
            ContextInternal::computeShader_After_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Draw_Left
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Draw_Right
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: After
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("BarrierTest", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"BarrierTest Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (4+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"BarrierTest Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Compute Shader: Draw_Left
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Draw_Left", commandList, 0);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Draw_Left_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Draw_Left_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Output, context->m_output.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 }
            };
            ...
            unsigned int dispatchSize[3] = {
                (((baseDispatchSize[0] + 0) * 1) / 2 + 0 + 8 - 1) / 8,
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Draw_Left";
        ...
        // Compute Shader: Draw_Right
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Draw_Right", commandList, 1);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Draw_Right_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Draw_Right_pso);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Draw_Right";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Compute Shader: After
        {
            ScopedPerfEvent scopedPerf("Compute Shader: After", commandList, 4);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_After_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_After_pso);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "After";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Output
        {
               ...
               m_output.texture_Output_format != desiredFormat)
            {
                ...
                m_output.texture_Output = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Output_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Output" : nullptr), Context::LogFn);
                m_output.texture_Output_size[0] = desiredSize[0];
                m_output.texture_Output_size[1] = desiredSize[1];
                m_output.texture_Output_size[2] = desiredSize[2];
                m_output.texture_Output_format = desiredFormat;
