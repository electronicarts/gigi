...
namespace VariableAliases
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Set_Red_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Set_Red_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_Set_Green_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Set_Green_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_Set_Blue_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Set_Blue_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Set_Red
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_Set_Red_rootSig, (c_debugNames ? L"Set_Red" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "VariableAliases_SetChannel.hlsl";
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "Set_Red" : "");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Set_Red_rootSig, &ContextInternal::computeShader_Set_Red_pso, Context::LogFn))
                return false;
        }
        ...
        // Compute Shader: Set_Green
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_Set_Green_rootSig, (c_debugNames ? L"Set_Green" : nullptr), Context::LogFn))
                return false;
            ...
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "Set_Green" : "");
            if (c_debugShaders) shaderCompilationInfo.flags |= ShaderCompilationFlags::Debug;
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchMultiply","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchDivide","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPreAdd","uint3(0,0,0)");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Set_Green_rootSig, &ContextInternal::computeShader_Set_Green_pso, Context::LogFn))
                return false;
        }
        ...
        // Compute Shader: Set_Blue
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_Set_Blue_rootSig, (c_debugNames ? L"Set_Blue" : nullptr), Context::LogFn))
                return false;
            ...
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "Set_Blue" : "");
            if (c_debugShaders) shaderCompilationInfo.flags |= ShaderCompilationFlags::Debug;
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchMultiply","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchDivide","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPreAdd","uint3(0,0,0)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPostAdd","uint3(0,0,0)");
            shaderCompilationInfo.defines.emplace_back("__GIGI_AlIAS_VARIABLE_CONST_Value","0.75");

            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Set_Blue_rootSig, &ContextInternal::computeShader_Set_Blue_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Set_Red_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Red_pso);
            ContextInternal::computeShader_Set_Red_pso = nullptr;
        }

        if(ContextInternal::computeShader_Set_Red_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Red_rootSig);
            ContextInternal::computeShader_Set_Red_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Set_Green_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Green_pso);
            ContextInternal::computeShader_Set_Green_pso = nullptr;
        }

        if(ContextInternal::computeShader_Set_Green_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Green_rootSig);
            ContextInternal::computeShader_Set_Green_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Set_Blue_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Blue_pso);
            ContextInternal::computeShader_Set_Blue_pso = nullptr;
        }

        if(ContextInternal::computeShader_Set_Blue_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Set_Blue_rootSig);
            ContextInternal::computeShader_Set_Blue_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Set_Red
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Set_Green
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Set_Blue
    ...
    Context::~Context()
    {
        ...
        // _SetChannel_0CB_0
        if (m_internal.constantBuffer__SetChannel_0CB_0)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__SetChannel_0CB_0);
            m_internal.constantBuffer__SetChannel_0CB_0 = nullptr;
        }

        // _SetChannel_0CB_1
        if (m_internal.constantBuffer__SetChannel_0CB_1)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__SetChannel_0CB_1);
            m_internal.constantBuffer__SetChannel_0CB_1 = nullptr;
        }

        // _SetChannel_1CB_0
        if (m_internal.constantBuffer__SetChannel_1CB_0)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__SetChannel_1CB_0);
            m_internal.constantBuffer__SetChannel_1CB_0 = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("VariableAliases", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"VariableAliases Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (3+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"VariableAliases Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            context->m_internal.constantBuffer__SetChannel_0CB_0_cpu._alias_Channel = context->m_input.variable_Node1Channel;
            context->m_internal.constantBuffer__SetChannel_0CB_0_cpu._alias_Value = context->m_input.variable_Node1Value;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__SetChannel_0CB_0, context->m_internal.constantBuffer__SetChannel_0CB_0_cpu, Context::LogFn);
        }
        ...
        // Compute Shader: Set_Red
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Set_Red", commandList, 1);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Set_Red_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Set_Red_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Color, context->m_output.texture_Color_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_internal.constantBuffer__SetChannel_0CB_0, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Set_Red";
        ...
        {
            context->m_internal.constantBuffer__SetChannel_0CB_1_cpu._alias_Channel = context->m_input.variable_Node2Channel;
            context->m_internal.constantBuffer__SetChannel_0CB_1_cpu._alias_Value = context->m_input.variable_Node2Value;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__SetChannel_0CB_1, context->m_internal.constantBuffer__SetChannel_0CB_1_cpu, Context::LogFn);
        }
        ...
        // Compute Shader: Set_Green
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Set_Green", commandList, 2);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Set_Green_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Set_Green_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Color, context->m_output.texture_Color_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_internal.constantBuffer__SetChannel_0CB_1, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Set_Green";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        {
            context->m_internal.constantBuffer__SetChannel_1CB_0_cpu._alias_Channel = context->m_input.variable_Node3Channel;
            context->m_internal.constantBuffer__SetChannel_1CB_0_cpu._alias_Value = context->m_input.variable_Node3Value;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__SetChannel_1CB_0, context->m_internal.constantBuffer__SetChannel_1CB_0_cpu, Context::LogFn);
        }
        ...
        // Compute Shader: Set_Blue
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Set_Blue", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Set_Blue_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Set_Blue_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Color, context->m_output.texture_Color_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0, 0, 0, 0, false },
                { context->m_internal.constantBuffer__SetChannel_1CB_0, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Set_Blue";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _SetChannel_0CB_0
        if (m_internal.constantBuffer__SetChannel_0CB_0 == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__SetChannel_0CB_0 = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_SetChannel_0CB_0" : nullptr), Context::LogFn);
        }

        // _SetChannel_0CB_1
        if (m_internal.constantBuffer__SetChannel_0CB_1 == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__SetChannel_0CB_1 = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_SetChannel_0CB_1" : nullptr), Context::LogFn);
        }

        // _SetChannel_1CB_0
        if (m_internal.constantBuffer__SetChannel_1CB_0 == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__SetChannel_1CB_0 = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_SetChannel_1CB_0" : nullptr), Context::LogFn);
        }
