...
namespace IndirectDispatch
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Do_Indirect_Dispatch_1_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Do_Indirect_Dispatch_1_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_Do_Indirect_Dispatch_2_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Do_Indirect_Dispatch_2_rootSig = nullptr;
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_pso)
        {
            ...
            ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_pso = nullptr;
        }

        if(ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_rootSig);
            ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Do_Indirect_Dispatch_1_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Do_Indirect_Dispatch_1_pso);
            ContextInternal::computeShader_Do_Indirect_Dispatch_1_pso = nullptr;
        }

        if(ContextInternal::computeShader_Do_Indirect_Dispatch_1_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Do_Indirect_Dispatch_1_rootSig);
            ContextInternal::computeShader_Do_Indirect_Dispatch_1_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_Do_Indirect_Dispatch_2_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Do_Indirect_Dispatch_2_pso);
            ContextInternal::computeShader_Do_Indirect_Dispatch_2_pso = nullptr;
        }

        if(ContextInternal::computeShader_Do_Indirect_Dispatch_2_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Do_Indirect_Dispatch_2_rootSig);
            ContextInternal::computeShader_Do_Indirect_Dispatch_2_rootSig = nullptr;
        }
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_Render_Target)
        {
            s_delayedRelease.Add(m_output.texture_Render_Target);
            m_output.texture_Render_Target = nullptr;
        }

        if(m_internal.buffer_Indirect_Dispatch_Count)
        {
            s_delayedRelease.Add(m_internal.buffer_Indirect_Dispatch_Count);
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_internal.buffer_Indirect_Dispatch_Count;
        ...
        // Compute Shader: Fill_Indirect_Dispatch_Count
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Fill_Indirect_Dispatch_Count", commandList, 2);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Fill_Indirect_Dispatch_Count_rootSig);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Fill_Indirect_Dispatch_Count";
        ...
        // Transition resources for the next action
        {
            ...
            barriers[1].Transition.pResource = context->m_internal.buffer_Indirect_Dispatch_Count;
        ...
        // Compute Shader: Do_Indirect_Dispatch_1
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Do_Indirect_Dispatch_1", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Do_Indirect_Dispatch_1_rootSig);
            ...
            int executeIndirectOffset = 1;

            commandList->ExecuteIndirect(
                ContextInternal::s_commandSignatureDispatch,
                1,
                context->m_internal.buffer_Indirect_Dispatch_Count,
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Do_Indirect_Dispatch_1";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Compute Shader: Do_Indirect_Dispatch_2
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Do_Indirect_Dispatch_2", commandList, 4);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Do_Indirect_Dispatch_2_rootSig);
            ...
            int executeIndirectOffset = context->m_input.variable_Second_Dispatch_Offset;
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Do_Indirect_Dispatch_2";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Render_Target
        {
            ...
            if(!m_output.texture_Render_Target ||
               m_output.texture_Render_Target_size[0] != desiredSize[0] ||
               m_output.texture_Render_Target_size[1] != desiredSize[1] ||
               m_output.texture_Render_Target_size[2] != desiredSize[2] ||
               m_output.texture_Render_Target_numMips != desiredNumMips ||
               m_output.texture_Render_Target_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_Render_Target)
                    s_delayedRelease.Add(m_output.texture_Render_Target);

                m_output.texture_Render_Target = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Render_Target_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Render_Target" : nullptr), Context::LogFn);
                m_output.texture_Render_Target_size[0] = desiredSize[0];
                m_output.texture_Render_Target_size[1] = desiredSize[1];
                m_output.texture_Render_Target_size[2] = desiredSize[2];
                m_output.texture_Render_Target_numMips = desiredNumMips;
        ...
        // Indirect_Dispatch_Count
        {
            ...
            unsigned int desiredCount = ((baseCount + 0 ) * 12) / 1 + 0;
            DXGI_FORMAT desiredFormat = DXGI_FORMAT_R32_UINT;
            unsigned int desiredStride = 0;

            if(!m_internal.buffer_Indirect_Dispatch_Count ||
               m_internal.buffer_Indirect_Dispatch_Count_count != desiredCount ||
               m_internal.buffer_Indirect_Dispatch_Count_format != desiredFormat ||
               m_internal.buffer_Indirect_Dispatch_Count_stride != desiredStride)
            {
                dirty = true;
                if(m_internal.buffer_Indirect_Dispatch_Count)
                    s_delayedRelease.Add(m_internal.buffer_Indirect_Dispatch_Count);
                ...
                m_internal.buffer_Indirect_Dispatch_Count = DX12Utils::CreateBuffer(device, desiredSize, m_internal.c_buffer_Indirect_Dispatch_Count_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Indirect_Dispatch_Count" : nullptr), Context::LogFn);
                m_internal.buffer_Indirect_Dispatch_Count_count = desiredCount;
                m_internal.buffer_Indirect_Dispatch_Count_format = desiredFormat;
                m_internal.buffer_Indirect_Dispatch_Count_stride = desiredStride;
            }
