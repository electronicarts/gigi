...
namespace ReadbackSequence
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Node_1_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Node_1_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Node_1
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_Node_1_rootSig, (c_debugNames ? L"Node_1" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/ReadbackSequence.hlsl", "csmain", "cs_6_1", defines,
               ContextInternal::computeShader_Node_1_rootSig, &ContextInternal::computeShader_Node_1_pso, c_debugShaders, (c_debugNames ? L"Node_1" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Node_1_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Node_1_pso);
            ContextInternal::computeShader_Node_1_pso = nullptr;
        }

        if(ContextInternal::computeShader_Node_1_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Node_1_rootSig);
            ContextInternal::computeShader_Node_1_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Node_1
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("ReadbackSequence", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"ReadbackSequence Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"ReadbackSequence Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "ReadbackSequence: Imported texture \"Output\" is null.\n");
            return;
        }
        ...
        // Compute Shader: Node_1
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Node_1", commandList, 0);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Node_1_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Node_1_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_input.texture_Output, context->m_input.texture_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 },
                { context->m_internal.constantBuffer__ReadbackSequenceCSCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
