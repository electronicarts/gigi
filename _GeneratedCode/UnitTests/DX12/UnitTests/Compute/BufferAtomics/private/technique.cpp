...
namespace BufferAtomics
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_ClearBuffer_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_ClearBuffer_rootSig = nullptr;

    ID3D12PipelineState* ContextInternal::computeShader_RunShader_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_RunShader_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: ClearBuffer
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_ClearBuffer_rootSig, (c_debugNames ? L"ClearBuffer" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "BufferAtomicsClear.hlsl";
            shaderCompilationInfo.entryPoint = "csmain";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "ClearBuffer" : "");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_ClearBuffer_rootSig, &ContextInternal::computeShader_ClearBuffer_pso, Context::LogFn))
                return false;
        }
        ...
        // Compute Shader: RunShader
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_RunShader_rootSig, (c_debugNames ? L"RunShader" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "BufferAtomics.hlsl";
            shaderCompilationInfo.entryPoint = "csmain";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "RunShader" : "");
            if (c_debugShaders) shaderCompilationInfo.flags |= ShaderCompilationFlags::Debug;
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchMultiply","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchDivide","uint3(1,1,1)");
            shaderCompilationInfo.defines.emplace_back("__GigiDispatchPreAdd","uint3(0,0,0)");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_RunShader_rootSig, &ContextInternal::computeShader_RunShader_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_ClearBuffer_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_ClearBuffer_pso);
            ContextInternal::computeShader_ClearBuffer_pso = nullptr;
        }

        if(ContextInternal::computeShader_ClearBuffer_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_ClearBuffer_rootSig);
            ContextInternal::computeShader_ClearBuffer_rootSig = nullptr;
        }

        if(ContextInternal::computeShader_RunShader_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_RunShader_pso);
            ContextInternal::computeShader_RunShader_pso = nullptr;
        }

        if(ContextInternal::computeShader_RunShader_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_RunShader_rootSig);
            ContextInternal::computeShader_RunShader_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: ClearBuffer
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: RunShader
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("BufferAtomics", commandList, 3);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"BufferAtomics Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"BufferAtomics Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Compute Shader: ClearBuffer
        {
            ScopedPerfEvent scopedPerf("Compute Shader: ClearBuffer", commandList, 2);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_ClearBuffer_rootSig);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "ClearBuffer";
        ...
        // Compute Shader: RunShader
        {
            ScopedPerfEvent scopedPerf("Compute Shader: RunShader", commandList, 0);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_RunShader_rootSig);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "RunShader";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // TheBuffer
        {
            ...
            unsigned int desiredCount = ((baseCount + 0 ) * 4) / 1 + 0;
