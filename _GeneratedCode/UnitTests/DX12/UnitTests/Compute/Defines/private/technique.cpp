...
namespace Defines
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_WriteOutput_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_WriteOutput_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: WriteOutput
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_WriteOutput_rootSig, (c_debugNames ? L"WriteOutput" : nullptr), Context::LogFn))
                return false;
            ...
            shaderCompilationInfo.defines.emplace_back("RED","0.75");
            shaderCompilationInfo.defines.emplace_back("GREEN","0.5");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_WriteOutput_rootSig, &ContextInternal::computeShader_WriteOutput_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_WriteOutput_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_WriteOutput_pso);
            ContextInternal::computeShader_WriteOutput_pso = nullptr;
        }

        if(ContextInternal::computeShader_WriteOutput_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_WriteOutput_rootSig);
            ContextInternal::computeShader_WriteOutput_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: WriteOutput
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("Defines", commandList, 2);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"Defines Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"Defines Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Compute Shader: WriteOutput
        {
            ScopedPerfEvent scopedPerf("Compute Shader: WriteOutput", commandList, 0);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_WriteOutput_rootSig);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "WriteOutput";
