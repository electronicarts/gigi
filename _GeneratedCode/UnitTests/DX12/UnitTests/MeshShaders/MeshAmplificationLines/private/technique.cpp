...
namespace MeshAmplificationLines
{
    ...
    Context::~Context()
    {
        ...
        // _AmplificationShaderCB
        if (m_internal.constantBuffer__AmplificationShaderCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__AmplificationShaderCB);
            m_internal.constantBuffer__AmplificationShaderCB = nullptr;
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("MeshAmplificationLines", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"MeshAmplificationLines Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"MeshAmplificationLines Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "MeshAmplificationLines: Imported buffer \"VertexBuffer\" is null.\n");
            return;
        }
        ...
        {
            context->m_internal.constantBuffer__MeshShaderCB_cpu.AmplificationSpacing = context->m_input.variable_AmplificationSpacing;
        ...
        // Shader Constants: _AmplificationShaderCB
        {
            context->m_internal.constantBuffer__AmplificationShaderCB_cpu.AmplificationCount = context->m_input.variable_AmplificationCount;
            context->m_internal.constantBuffer__AmplificationShaderCB_cpu.Offset = context->m_input.variable_Offset;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__AmplificationShaderCB, context->m_internal.constantBuffer__AmplificationShaderCB_cpu, Context::LogFn);
        }
        ...
        // Draw Call: Rasterize
        {
            ...
            DX12Utils::ResourceDescriptor descriptorsAS[] =
            {
                ...
                { context->m_internal.constantBuffer__AmplificationShaderCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false },
            };
            D3D12_GPU_DESCRIPTOR_HANDLE descriptorTableAS = GetDescriptorTable(device, s_srvHeap, descriptorsAS, 2, Context::LogFn);
            commandList->SetGraphicsRootDescriptorTable(0, descriptorTableAS);
            ...
            commandList->SetGraphicsRootDescriptorTable(1, descriptorTableMS);
            ...
            // Clear Color_Buffer
            {
                 ...
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "MeshAmplificationLines.Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for MeshAmplificationLines.Color_Buffer");
            ...
            // Clear Depth_Buffer
            {
                 ...
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "MeshAmplificationLines.Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for MeshAmplificationLines.Depth_Buffer");
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "MeshAmplificationLines.Color_Buffer"))
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "MeshAmplificationLines.Depth_Buffer"));
            ...
            unsigned int baseDispatchSize[3] = { (unsigned int)context->m_input.variable_AmplificationCount, 1, 1 };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _AmplificationShaderCB
        if (m_internal.constantBuffer__AmplificationShaderCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__AmplificationShaderCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_AmplificationShaderCB" : nullptr), Context::LogFn);
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            D3D12_DESCRIPTOR_RANGE rangesAmplification[2];

            // AmpVB
            rangesAmplification[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
            rangesAmplification[0].NumDescriptors = 1;
            rangesAmplification[0].BaseShaderRegister = 0;
            rangesAmplification[0].RegisterSpace = 0;
            rangesAmplification[0].OffsetInDescriptorsFromTableStart = 0;

            // _AmplificationShaderCB
            rangesAmplification[1].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
            rangesAmplification[1].NumDescriptors = 1;
            rangesAmplification[1].BaseShaderRegister = 0;
            rangesAmplification[1].RegisterSpace = 0;
            rangesAmplification[1].OffsetInDescriptorsFromTableStart = 1;
            ...
            rootParams[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_AMPLIFICATION;
            rootParams[0].DescriptorTable.NumDescriptorRanges = 2;
            rootParams[0].DescriptorTable.pDescriptorRanges = rangesAmplification;
            ...
            rootParams[1].DescriptorTable.pDescriptorRanges = rangesMesh;
            ...
            shaderCompilationInfoPS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "MeshAmplificationLines_PS.hlsl";
            ...
            shaderCompilationInfoAS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "MeshAmplificationLines_AS.hlsl";
            shaderCompilationInfoAS.entryPoint = "ASMain";
            shaderCompilationInfoAS.shaderModel = "as_6_5";
            shaderCompilationInfoAS.debugName = (c_debugNames ? "Rasterize" : "");
            if (c_debugShaders) shaderCompilationInfoAS.flags |= ShaderCompilationFlags::Debug;

            std::vector<unsigned char> byteCodeAS = DX12Utils::CompileShaderToByteCode_DXC(shaderCompilationInfoAS, Context::LogFn);
            if (byteCodeAS.size() == 0)
                return false;

            ShaderCompilationInfo shaderCompilationInfoMS;
            shaderCompilationInfoMS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "MeshAmplificationLines_MS.hlsl";
