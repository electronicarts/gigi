...
namespace MeshAmplification
{
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/MeshAmplification_PS.hlsl", "PSMain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            if (byteCodePS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesAS = nullptr;

            std::vector<unsigned char> byteCodeAS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/MeshAmplification_AS.hlsl", "ASMain", "as_6_5", definesAS, c_debugShaders, Context::LogFn);
            if (byteCodeAS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesMS = nullptr;

            std::vector<unsigned char> byteCodeMS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/MeshAmplification_MS.hlsl", "MSMain", "ms_6_5", definesMS, c_debugShaders, Context::LogFn);
