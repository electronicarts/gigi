...
namespace TwoRayGens
{
    ...
    ID3D12StateObject* ContextInternal::rayShader_DoRT1_rtso = nullptr;
    ID3D12RootSignature* ContextInternal::rayShader_DoRT1_rootSig = nullptr;
    ID3D12Resource* ContextInternal::rayShader_DoRT1_shaderTableRayGen = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT1_shaderTableRayGenSize = 0;
    ID3D12Resource* ContextInternal::rayShader_DoRT1_shaderTableMiss = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT1_shaderTableMissSize = 0;
    ID3D12Resource* ContextInternal::rayShader_DoRT1_shaderTableHitGroup = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT1_shaderTableHitGroupSize = 0;

    ID3D12StateObject* ContextInternal::rayShader_DoRT2_rtso = nullptr;
    ID3D12RootSignature* ContextInternal::rayShader_DoRT2_rootSig = nullptr;
    ID3D12Resource* ContextInternal::rayShader_DoRT2_shaderTableRayGen = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT2_shaderTableRayGenSize = 0;
    ID3D12Resource* ContextInternal::rayShader_DoRT2_shaderTableMiss = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT2_shaderTableMissSize = 0;
    ID3D12Resource* ContextInternal::rayShader_DoRT2_shaderTableHitGroup = nullptr;
    unsigned int    ContextInternal::rayShader_DoRT2_shaderTableHitGroupSize = 0;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Ray Shader: DoRT1
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 3, samplers, 0, &ContextInternal::rayShader_DoRT1_rootSig, (c_debugNames ? L"DoRT1" : nullptr), Context::LogFn))
                return false;

            D3D_SHADER_MACRO defines[] = {
                { "MAX_RECURSION_DEPTH", "3" },
                { "RT_HIT_GROUP_COUNT", "2" },
                { nullptr, nullptr }
            };
            ...
            // Compile RTMiss : TwoRayGens1.hlsl Miss1()
            shaderCode[0] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens1.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[0].empty())
                return false;

            // Compile RTClosestHit : TwoRayGens1.hlsl ClosestHit1()
            shaderCode[1] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens1.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[1].empty())
                return false;

            // Compile RTMiss : TwoRayGens2.hlsl Miss2()
            shaderCode[2] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens2.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[2].empty())
                return false;

            // Compile RTClosestHit : TwoRayGens2.hlsl ClosestHit2()
            shaderCode[3] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens2.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[3].empty())
                return false;

            // Compile RTRayGen : TwoRayGens1.hlsl RayGen1()
            shaderCode[4] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens1.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            ...
            soDesc.NumSubobjects = 11;
            soDesc.pSubobjects = subObjects;

            // DXIL Library for RTMiss : TwoRayGens1.hlsl Miss1()
            {
            ...
            // DXIL Library for RTClosestHit : TwoRayGens1.hlsl ClosestHit1()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            // DXIL Library for RTMiss : TwoRayGens2.hlsl Miss2()
            {
                static D3D12_EXPORT_DESC exportDesc;
                exportDesc.Name = L"Miss2_2";
                exportDesc.ExportToRename = L"Miss2";
                exportDesc.Flags = D3D12_EXPORT_FLAG_NONE;
            ...
            // DXIL Library for RTClosestHit : TwoRayGens2.hlsl ClosestHit2()
            {
                static D3D12_EXPORT_DESC exportDesc;
                exportDesc.Name = L"ClosestHit2_3";
            ...
            // DXIL Library for RTRayGen : TwoRayGens1.hlsl RayGen1()
            {
                static D3D12_EXPORT_DESC exportDesc;
                exportDesc.Name = L"RayGen1_4";
            ...
            // Hit group: HitGroup2
            {
                ...
                hitGroupDesc.AnyHitShaderImport = nullptr;
                hitGroupDesc.ClosestHitShaderImport = L"ClosestHit2_3";
                hitGroupDesc.IntersectionShaderImport = nullptr;
                hitGroupDesc.Type = D3D12_HIT_GROUP_TYPE_TRIANGLES;
                subObjects[6].Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP;
                subObjects[6].pDesc = &hitGroupDesc;
            }
            ...
            subObjects[7].Type = D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG;
            subObjects[7].pDesc = &payloadDesc;

            // Associate payload with shaders
            const WCHAR* shaderExports[] = { L"Miss1_0", L"ClosestHit1_1", L"Miss2_2", L"ClosestHit2_3", L"RayGen1_4" };

            D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION shaderPayloadAssociation = {};
            shaderPayloadAssociation.NumExports = 5;
            shaderPayloadAssociation.pExports = shaderExports;
            shaderPayloadAssociation.pSubobjectToAssociate = &subObjects[7];

            subObjects[8].Type = D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION;
            subObjects[8].pDesc = &shaderPayloadAssociation;

            // Pipeline Config
            D3D12_RAYTRACING_PIPELINE_CONFIG pipelineConfig;
            pipelineConfig.MaxTraceRecursionDepth = 3;

            subObjects[9].Type = D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_PIPELINE_CONFIG;
            subObjects[9].pDesc = &pipelineConfig;

            // Global Root Signature
            subObjects[10].Type = D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE;
            subObjects[10].pDesc = &ContextInternal::rayShader_DoRT1_rootSig;
            if (FAILED(dxrDevice->CreateStateObject(&soDesc, IID_PPV_ARGS(&ContextInternal::rayShader_DoRT1_rtso))))
                return false;

            if (c_debugNames)
                ContextInternal::rayShader_DoRT1_rtso->SetName(L"DoRT1 state object");

            // Create the shader tables
            {
                ID3D12StateObjectProperties* soprops = nullptr;
                if(FAILED(ContextInternal::rayShader_DoRT1_rtso->QueryInterface(IID_PPV_ARGS(&soprops))))
                    return false;

                // make the ray gen shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT1_shaderTableRayGenSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(1 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT1_shaderTableRayGen = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT1_shaderTableRayGenSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT1 shader table ray gen" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT1_shaderTableRayGen->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"RayGen1_4"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_DoRT1_shaderTableRayGen->Unmap(0, nullptr);
                }

                // make the miss shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT1_shaderTableMissSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(2 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT1_shaderTableMiss = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT1_shaderTableMissSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT1 shader table miss" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT1_shaderTableMiss->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"Miss1_0"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;
                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"Miss2_2"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_DoRT1_shaderTableMiss->Unmap(0, nullptr);
                }

                // make the hit group shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT1_shaderTableHitGroupSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(2 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT1_shaderTableHitGroup = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT1_shaderTableHitGroupSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT1 shader table hit group" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT1_shaderTableHitGroup->Map(0, &readRange, (void**)&shaderTableBytes);
                    ...
                    ContextInternal::rayShader_DoRT1_shaderTableHitGroup->Unmap(0, nullptr);
                }
        ...
        // Ray Shader: DoRT2
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 3, samplers, 0, &ContextInternal::rayShader_DoRT2_rootSig, (c_debugNames ? L"DoRT2" : nullptr), Context::LogFn))
                return false;
            ...
            // Compile RTRayGen : TwoRayGens2.hlsl RayGen2()
            shaderCode[4] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/TwoRayGens2.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[4].empty())
                return false;
            ...
            // DXIL Library for RTRayGen : TwoRayGens2.hlsl RayGen2()
            {
                static D3D12_EXPORT_DESC exportDesc;
                exportDesc.Name = L"RayGen2_4";
            ...
            // Associate payload with shaders
            const WCHAR* shaderExports[] = { L"Miss1_0", L"ClosestHit1_1", L"Miss2_2", L"ClosestHit2_3", L"RayGen2_4" };
            ...
            // Global Root Signature
            subObjects[10].Type = D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE;
            subObjects[10].pDesc = &ContextInternal::rayShader_DoRT2_rootSig;
            if (FAILED(dxrDevice->CreateStateObject(&soDesc, IID_PPV_ARGS(&ContextInternal::rayShader_DoRT2_rtso))))
                return false;

            if (c_debugNames)
                ContextInternal::rayShader_DoRT2_rtso->SetName(L"DoRT2 state object");

            // Create the shader tables
            {
                ID3D12StateObjectProperties* soprops = nullptr;
                if(FAILED(ContextInternal::rayShader_DoRT2_rtso->QueryInterface(IID_PPV_ARGS(&soprops))))
                    return false;

                // make the ray gen shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT2_shaderTableRayGenSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(1 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT2_shaderTableRayGen = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT2_shaderTableRayGenSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT2 shader table ray gen" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT2_shaderTableRayGen->Map(0, &readRange, (void**)&shaderTableBytes);

                    memcpy(shaderTableBytes, soprops->GetShaderIdentifier(L"RayGen2_4"), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
                    shaderTableBytes += D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT;

                    ContextInternal::rayShader_DoRT2_shaderTableRayGen->Unmap(0, nullptr);
                }

                // make the miss shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT2_shaderTableMissSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(2 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT2_shaderTableMiss = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT2_shaderTableMissSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT2 shader table miss" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT2_shaderTableMiss->Map(0, &readRange, (void**)&shaderTableBytes);
                    ...
                    ContextInternal::rayShader_DoRT2_shaderTableMiss->Unmap(0, nullptr);
                }

                // make the hit group shader table and fill it out
                {
                    ContextInternal::rayShader_DoRT2_shaderTableHitGroupSize = ALIGN(D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT, (unsigned int)(2 * D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT));
                    ContextInternal::rayShader_DoRT2_shaderTableHitGroup = DX12Utils::CreateBuffer(device, ContextInternal::rayShader_DoRT2_shaderTableHitGroupSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD, (c_debugNames ? L"DoRT2 shader table hit group" : nullptr), nullptr);

                    unsigned char* shaderTableBytes = nullptr;
                    D3D12_RANGE readRange = { 0, 0 };
                    ContextInternal::rayShader_DoRT2_shaderTableHitGroup->Map(0, &readRange, (void**)&shaderTableBytes);
                    ...
                    ContextInternal::rayShader_DoRT2_shaderTableHitGroup->Unmap(0, nullptr);
                }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::rayShader_DoRT1_rtso)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT1_rtso);
            ContextInternal::rayShader_DoRT1_rtso = nullptr;
        }

        if(ContextInternal::rayShader_DoRT1_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT1_rootSig);
            ContextInternal::rayShader_DoRT1_rootSig = nullptr;
        }

        if(ContextInternal::rayShader_DoRT1_shaderTableRayGen)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT1_shaderTableRayGen);
            ContextInternal::rayShader_DoRT1_shaderTableRayGen = nullptr;
        }

        if(ContextInternal::rayShader_DoRT1_shaderTableMiss)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT1_shaderTableMiss);
            ContextInternal::rayShader_DoRT1_shaderTableMiss = nullptr;
        }

        if(ContextInternal::rayShader_DoRT1_shaderTableHitGroup)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT1_shaderTableHitGroup);
            ContextInternal::rayShader_DoRT1_shaderTableHitGroup = nullptr;
        }

        if(ContextInternal::rayShader_DoRT2_rtso)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT2_rtso);
            ContextInternal::rayShader_DoRT2_rtso = nullptr;
        }

        if(ContextInternal::rayShader_DoRT2_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT2_rootSig);
            ContextInternal::rayShader_DoRT2_rootSig = nullptr;
        }

        if(ContextInternal::rayShader_DoRT2_shaderTableRayGen)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT2_shaderTableRayGen);
            ContextInternal::rayShader_DoRT2_shaderTableRayGen = nullptr;
        }

        if(ContextInternal::rayShader_DoRT2_shaderTableMiss)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT2_shaderTableMiss);
            ContextInternal::rayShader_DoRT2_shaderTableMiss = nullptr;
        }

        if(ContextInternal::rayShader_DoRT2_shaderTableHitGroup)
        {
            s_delayedRelease.Add(ContextInternal::rayShader_DoRT2_shaderTableHitGroup);
            ContextInternal::rayShader_DoRT2_shaderTableHitGroup = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // ray shader: DoRT1
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // ray shader: DoRT2
    ...
    Context::~Context()
    {
        ...
        // _TwoRayGens1CB
        if (m_internal.constantBuffer__TwoRayGens1CB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__TwoRayGens1CB);
            m_internal.constantBuffer__TwoRayGens1CB = nullptr;
        }

        // _TwoRayGens2CB
        if (m_internal.constantBuffer__TwoRayGens2CB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__TwoRayGens2CB);
            m_internal.constantBuffer__TwoRayGens2CB = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("TwoRayGens", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"TwoRayGens Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"TwoRayGens Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "TwoRayGens: Imported buffer \"Scene\" needs a BLAS/TLAS. Use CreateManagedTLAS to create them.\n");
            return;
        }

        if (!context->m_input.buffer_Scene)
        {
            Context::LogFn(LogLevel::Error, "TwoRayGens: Imported buffer \"Scene\" is null.\n");
            return;
        }
        ...
        {
            context->m_internal.constantBuffer__TwoRayGens1CB_cpu.cameraPos = context->m_input.variable_cameraPos;
            context->m_internal.constantBuffer__TwoRayGens1CB_cpu.clipToWorld = context->m_input.variable_clipToWorld;
            context->m_internal.constantBuffer__TwoRayGens1CB_cpu.depthNearPlane = context->m_input.variable_depthNearPlane;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__TwoRayGens1CB, context->m_internal.constantBuffer__TwoRayGens1CB_cpu, Context::LogFn);
        }
        ...
        // Ray Shader: DoRT1
        {
            ScopedPerfEvent scopedPerf("Ray Shader: DoRT1", commandList, 2);
            ...
            commandList->SetComputeRootSignature(ContextInternal::rayShader_DoRT1_rootSig);
            dxrCommandList->SetPipelineState1(ContextInternal::rayShader_DoRT1_rtso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Texture, context->m_output.texture_Texture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 },
                { context->m_input.buffer_Scene, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::RTScene, false, context->m_input.buffer_Scene_tlasSize, 1 },
                { context->m_internal.constantBuffer__TwoRayGens1CB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
            ...
            dispatchDesc.RayGenerationShaderRecord.StartAddress = ContextInternal::rayShader_DoRT1_shaderTableRayGen->GetGPUVirtualAddress();
            dispatchDesc.RayGenerationShaderRecord.SizeInBytes = ContextInternal::rayShader_DoRT1_shaderTableRayGenSize;
            dispatchDesc.MissShaderTable.StartAddress = ContextInternal::rayShader_DoRT1_shaderTableMiss->GetGPUVirtualAddress();
            dispatchDesc.MissShaderTable.SizeInBytes = ContextInternal::rayShader_DoRT1_shaderTableMissSize;
            dispatchDesc.MissShaderTable.StrideInBytes = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
            dispatchDesc.HitGroupTable.StartAddress = ContextInternal::rayShader_DoRT1_shaderTableHitGroup->GetGPUVirtualAddress();
            dispatchDesc.HitGroupTable.SizeInBytes = ContextInternal::rayShader_DoRT1_shaderTableHitGroupSize;
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DoRT1";
        ...
        {
            context->m_internal.constantBuffer__TwoRayGens2CB_cpu.cameraPos = context->m_input.variable_cameraPos;
            context->m_internal.constantBuffer__TwoRayGens2CB_cpu.clipToWorld = context->m_input.variable_clipToWorld;
            context->m_internal.constantBuffer__TwoRayGens2CB_cpu.depthNearPlane = context->m_input.variable_depthNearPlane;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__TwoRayGens2CB, context->m_internal.constantBuffer__TwoRayGens2CB_cpu, Context::LogFn);
        }
        ...
        // Ray Shader: DoRT2
        {
            ScopedPerfEvent scopedPerf("Ray Shader: DoRT2", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::rayShader_DoRT2_rootSig);
            dxrCommandList->SetPipelineState1(ContextInternal::rayShader_DoRT2_rtso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Texture, context->m_output.texture_Texture_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 },
                { context->m_input.buffer_Scene, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::RTScene, false, context->m_input.buffer_Scene_tlasSize, 1 },
                { context->m_internal.constantBuffer__TwoRayGens2CB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
            ...
            dispatchDesc.Height = ((baseDispatchSize[1] + 0) * 1) / 1 + 0;
            dispatchDesc.Depth = ((baseDispatchSize[2] + 0) * 1) / 1 + 0;
            dispatchDesc.RayGenerationShaderRecord.StartAddress = ContextInternal::rayShader_DoRT2_shaderTableRayGen->GetGPUVirtualAddress();
            dispatchDesc.RayGenerationShaderRecord.SizeInBytes = ContextInternal::rayShader_DoRT2_shaderTableRayGenSize;
            dispatchDesc.MissShaderTable.StartAddress = ContextInternal::rayShader_DoRT2_shaderTableMiss->GetGPUVirtualAddress();
            dispatchDesc.MissShaderTable.SizeInBytes = ContextInternal::rayShader_DoRT2_shaderTableMissSize;
            dispatchDesc.MissShaderTable.StrideInBytes = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
            dispatchDesc.HitGroupTable.StartAddress = ContextInternal::rayShader_DoRT2_shaderTableHitGroup->GetGPUVirtualAddress();
            dispatchDesc.HitGroupTable.SizeInBytes = ContextInternal::rayShader_DoRT2_shaderTableHitGroupSize;
            dispatchDesc.HitGroupTable.StrideInBytes = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DoRT2";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // The texture that is rendered to
        {
               ...
               m_output.texture_Texture_format != desiredFormat)
            {
                ...
                m_output.texture_Texture = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Texture_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Texture" : nullptr), Context::LogFn);
                m_output.texture_Texture_size[0] = desiredSize[0];
                m_output.texture_Texture_size[1] = desiredSize[1];
                m_output.texture_Texture_size[2] = desiredSize[2];
                m_output.texture_Texture_format = desiredFormat;
        ...
        // _TwoRayGens1CB
        if (m_internal.constantBuffer__TwoRayGens1CB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__TwoRayGens1CB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_TwoRayGens1CB" : nullptr), Context::LogFn);
        }

        // _TwoRayGens2CB
        if (m_internal.constantBuffer__TwoRayGens2CB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__TwoRayGens2CB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_TwoRayGens2CB" : nullptr), Context::LogFn);
        }
