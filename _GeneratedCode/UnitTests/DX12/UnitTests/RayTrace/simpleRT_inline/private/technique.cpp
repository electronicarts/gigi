...
namespace simpleRT_inline
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_DoRT_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_DoRT_rootSig = nullptr;
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_DoRT_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_DoRT_pso);
            ContextInternal::computeShader_DoRT_pso = nullptr;
        }

        if(ContextInternal::computeShader_DoRT_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_DoRT_rootSig);
            ContextInternal::computeShader_DoRT_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        {
            m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: DoRT
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("simpleRT_inline", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"simpleRT_inline Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"simpleRT_inline Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "simpleRT_inline: Imported buffer \"Scene\" needs a BLAS/TLAS. Use CreateManagedTLAS to create them.\n");
            return;
        }

        if (!context->m_input.buffer_Scene)
        {
            Context::LogFn(LogLevel::Error, "simpleRT_inline: Imported buffer \"Scene\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_Scene_VB)
        {
            Context::LogFn(LogLevel::Error, "simpleRT_inline: Imported buffer \"Scene_VB\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_Scene_VB_state != D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Scene_VB;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_Scene_VB_state;
        ...
        {
            ScopedPerfEvent scopedPerf("Compute Shader: DoRT", commandList, 2);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_DoRT_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_DoRT_pso);
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_input.buffer_Scene_VB, context->m_input.buffer_Scene_VB_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Buffer, false, context->m_input.buffer_Scene_VB_stride, context->m_input.buffer_Scene_VB_count },
                { context->m_internal.constantBuffer__SimpleRTCSCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_Scene_VB_state != D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_Scene_VB_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
