...
namespace simpleRT_inline
{
    ...
    struct Context
    {
        ...
        {
            return "simpleRT_inline";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"simpleRT_inline";
        }
        ...
        struct ContextInput
        {
            ...
            ID3D12Resource* buffer_Scene_VB = nullptr;
            DXGI_FORMAT buffer_Scene_VB_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Scene_VB_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Scene_VB_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_Scene_VB_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_Scene_VB_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
