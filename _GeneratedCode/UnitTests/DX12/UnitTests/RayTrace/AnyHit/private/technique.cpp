...
namespace AnyHit
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Ray Shader: DoRT
        {
            ...
            // DXIL Library for RTMiss : AnyHit.hlsl Miss()
            {
            ...
            // DXIL Library for RTClosestHit : AnyHit.hlsl ClosestHit()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            // DXIL Library for RTAnyHit : AnyHit.hlsl AnyHit()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            // DXIL Library for RTRayGen : AnyHit.hlsl RayGen()
            {
                static D3D12_EXPORT_DESC exportDesc;
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("AnyHit", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"AnyHit Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"AnyHit Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "AnyHit: Imported buffer \"Scene\" needs a BLAS/TLAS. Use CreateManagedTLAS to create them.\n");
            return;
        }

        if (!context->m_input.buffer_Scene)
        {
            Context::LogFn(LogLevel::Error, "AnyHit: Imported buffer \"Scene\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_SceneVB)
        {
            Context::LogFn(LogLevel::Error, "AnyHit: Imported buffer \"SceneVB\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_SceneVB_state != D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_SceneVB;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_SceneVB_state;
        ...
        // Ray Shader: DoRT
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_input.buffer_SceneVB, context->m_input.buffer_SceneVB_format, DX12Utils::AccessType::SRV, DX12Utils::ResourceType::Buffer, false, context->m_input.buffer_SceneVB_stride, context->m_input.buffer_SceneVB_count },
                { context->m_internal.constantBuffer__RayGenCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_SceneVB_state != D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_SceneVB_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
