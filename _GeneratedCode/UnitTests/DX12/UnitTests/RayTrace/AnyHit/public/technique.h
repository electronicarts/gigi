...
namespace AnyHit
{
    ...
    struct Context
    {
        ...
        struct ContextInput
        {
            ...
            ID3D12Resource* buffer_SceneVB = nullptr;
            DXGI_FORMAT buffer_SceneVB_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_SceneVB_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_SceneVB_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_SceneVB_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_SceneVB_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
