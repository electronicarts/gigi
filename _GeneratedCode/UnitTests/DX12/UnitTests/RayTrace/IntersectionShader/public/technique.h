...
namespace IntersectionShader
{
    ...
    struct Context
    {
        ...
        struct ContextInput
        {
            ...
            ID3D12Resource* buffer_AABBs = nullptr;
            DXGI_FORMAT buffer_AABBs_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_AABBs_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_AABBs_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_AABBs_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_AABBs_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with

            // TLAS and BLAS information. Required for raytracing. Fill out using CreateManagedTLAS().
            // The resource itself is the tlas.
            unsigned int buffer_AABBs_tlasSize = 0;
            ID3D12Resource* buffer_AABBs_blas = nullptr;
            unsigned int buffer_AABBs_blasSize = 0;

            ID3D12Resource* buffer_AABBsSRV = nullptr;
            DXGI_FORMAT buffer_AABBsSRV_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_AABBsSRV_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_AABBsSRV_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_AABBsSRV_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_AABBsSRV_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
