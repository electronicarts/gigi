...
namespace simpleRT
{
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Ray Shader: DoRT
        {
            ...
            // Compile RTMiss : simpleRT.hlsl Miss()
            shaderCode[0] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/simpleRT.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[0].empty())
                return false;

            // Compile RTClosestHit : simpleRT.hlsl ClosestHit()
            shaderCode[1] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/simpleRT.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            if (shaderCode[1].empty())
                return false;

            // Compile RTRayGen : simpleRT.hlsl RayGen()
            shaderCode[2] = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/simpleRT.hlsl", "", "lib_6_3", defines, c_debugShaders, Context::LogFn);
            ...
            // DXIL Library for RTMiss : simpleRT.hlsl Miss()
            {
            ...
            // DXIL Library for RTClosestHit : simpleRT.hlsl ClosestHit()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            // DXIL Library for RTRayGen : simpleRT.hlsl RayGen()
            {
                static D3D12_EXPORT_DESC exportDesc;
            ...
            subObjects[7].pDesc = &ContextInternal::rayShader_DoRT_rootSig;
    ...
    Context::~Context()
    {
        ...
        // _SimpleRTRayGenCB
        if (m_internal.constantBuffer__SimpleRTRayGenCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__SimpleRTRayGenCB);
            m_internal.constantBuffer__SimpleRTRayGenCB = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("simpleRT", commandList, 4);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"simpleRT Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"simpleRT Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "simpleRT: Imported buffer \"Scene\" needs a BLAS/TLAS. Use CreateManagedTLAS to create them.\n");
            return;
        }

        if (!context->m_input.buffer_Scene)
        {
            Context::LogFn(LogLevel::Error, "simpleRT: Imported buffer \"Scene\" is null.\n");
            return;
        }
        ...
        {
            context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu.cameraPos = context->m_input.variable_cameraPos;
            context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu.clipToWorld = context->m_input.variable_clipToWorld;
            context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu.depthNearPlane = context->m_input.variable_depthNearPlane;
            context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu.hitColor = context->m_input.variable_hitColor;
            context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu.missColor = context->m_input.variable_missColor;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__SimpleRTRayGenCB, context->m_internal.constantBuffer__SimpleRTRayGenCB_cpu, Context::LogFn);
        }
        ...
        if(context->m_input.variable_enabled)
        {
            ...
            DX12Utils::ResourceDescriptor descriptors[] = {
                ...
                { context->m_internal.constantBuffer__SimpleRTRayGenCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 }
            };
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // The texture that is rendered to
        {
            ...
            unsigned int desiredSize[3] = {
                ((baseSize[0] + 0) * 1280) / 1 + 0,
                ((baseSize[1] + 0) * 720) / 1 + 0,
                ((baseSize[2] + 0) * 1) / 1 + 0
            };
        ...
        // _SimpleRTRayGenCB
        if (m_internal.constantBuffer__SimpleRTRayGenCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__SimpleRTRayGenCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_SimpleRTRayGenCB" : nullptr), Context::LogFn);
        }
