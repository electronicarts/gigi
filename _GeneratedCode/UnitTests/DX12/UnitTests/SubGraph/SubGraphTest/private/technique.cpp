...
namespace SubGraphTest
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Copy Resource: Inner_Copy_Import_To_Export
        {
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Inner_Copy_Import_To_Export";
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Inner_Exported_Tex
        {
            ...
            if(!m_output.texture_Inner_Exported_Tex ||
               ...
               m_output.texture_Inner_Exported_Tex_numMips != desiredNumMips ||
               m_output.texture_Inner_Exported_Tex_format != desiredFormat)
            {
                ...
                m_output.texture_Inner_Exported_Tex = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Inner_Exported_Tex_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Inner_Exported_Tex" : nullptr), Context::LogFn);
                m_output.texture_Inner_Exported_Tex_size[0] = desiredSize[0];
                m_output.texture_Inner_Exported_Tex_size[1] = desiredSize[1];
                m_output.texture_Inner_Exported_Tex_size[2] = desiredSize[2];
                m_output.texture_Inner_Exported_Tex_numMips = desiredNumMips;
