...
namespace SubGraphTest
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Swap_Colors_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Swap_Colors_rootSig = nullptr;
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Swap_Colors_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Swap_Colors_pso);
            ContextInternal::computeShader_Swap_Colors_pso = nullptr;
        }

        if(ContextInternal::computeShader_Swap_Colors_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Swap_Colors_rootSig);
            ContextInternal::computeShader_Swap_Colors_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Inner_Copy_Import_To_Export
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Swap_Colors
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_Inner_Exported_Tex)
        {
            s_delayedRelease.Add(m_output.texture_Inner_Exported_Tex);
            m_output.texture_Inner_Exported_Tex = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("SubGraphTest", commandList, 4);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"SubGraphTest Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"SubGraphTest Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        if (!context->m_input.texture_Test)
        {
            Context::LogFn(LogLevel::Error, "SubGraphTest: Imported texture \"Test\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.texture_Test_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.texture_Test;
                barriers[barrierCount].Transition.StateBefore = context->m_input.texture_Test_state;
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_output.texture_Inner_Exported_Tex;
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Inner_Copy_Import_To_Export", commandList, 3);
            ...
            if(context->m_profile)
            {
                context->m_profileData[s_timerIndex].m_label = "Inner_Copy_Import_To_Export";
        ...
        // Compute Shader: Swap_Colors
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Swap_Colors", commandList, 1);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Swap_Colors_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Swap_Colors_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.texture_Inner_Exported_Tex, context->m_output.texture_Inner_Exported_Tex_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 }
            };
            ...
            unsigned int baseDispatchSize[3] = {
                context->m_input.texture_Test_size[0],
                context->m_input.texture_Test_size[1],
                context->m_input.texture_Test_size[2]
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Swap_Colors";
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.texture_Test_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.texture_Test_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Inner_Exported_Tex
        {
            unsigned int baseSize[3] = {
                m_input.texture_Test_size[0],
                m_input.texture_Test_size[1],
                m_input.texture_Test_size[2]
            };
            ...
            DXGI_FORMAT desiredFormat = m_input.texture_Test_format;

            if(!m_output.texture_Inner_Exported_Tex ||
               m_output.texture_Inner_Exported_Tex_size[0] != desiredSize[0] ||
               m_output.texture_Inner_Exported_Tex_size[1] != desiredSize[1] ||
               m_output.texture_Inner_Exported_Tex_size[2] != desiredSize[2] ||
               m_output.texture_Inner_Exported_Tex_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_Inner_Exported_Tex)
                    s_delayedRelease.Add(m_output.texture_Inner_Exported_Tex);

                m_output.texture_Inner_Exported_Tex = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Inner_Exported_Tex_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Inner_Exported_Tex" : nullptr), Context::LogFn);
                m_output.texture_Inner_Exported_Tex_size[0] = desiredSize[0];
                m_output.texture_Inner_Exported_Tex_size[1] = desiredSize[1];
                m_output.texture_Inner_Exported_Tex_size[2] = desiredSize[2];
                m_output.texture_Inner_Exported_Tex_format = desiredFormat;
