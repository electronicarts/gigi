...
namespace SubInSub
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Inner1_Inner2_Rotate_Colors
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 3, samplers, 0, &ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig, (c_debugNames ? L"Inner1_Inner2_Rotate_Colors" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/SubInSubInner2/SubInSubInner2_CS.hlsl", "main", "cs_6_1", defines,
               ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig, &ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_pso, c_debugShaders, (c_debugNames ? L"Inner1_Inner2_Rotate_Colors" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_pso);
            ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_pso = nullptr;
        }

        if(ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig);
            ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Inner1_Inner2_Rotate_Colors
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Inner1_Inner2_Copy_Back
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_Inner1_Inner2_Output)
        {
            s_delayedRelease.Add(m_output.texture_Inner1_Inner2_Output);
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("SubInSub", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"SubInSub Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"SubInSub Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "SubInSub: Imported texture \"Input\" is null.\n");
            return;
        }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_output.texture_Inner1_Inner2_Output;
        ...
        // Compute Shader: Inner1_Inner2_Rotate_Colors
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Inner1_Inner2_Rotate_Colors", commandList, 1);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Inner1_Inner2_Rotate_Colors_rootSig);
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Inner1_Inner2_Rotate_Colors";
        ...
        // Transition resources for the next action
        {
            ...
            barriers[1].Transition.pResource = context->m_output.texture_Inner1_Inner2_Output;
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Inner1_Inner2_Copy_Back", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Inner1_Inner2_Output
        {
            ...
            if(!m_output.texture_Inner1_Inner2_Output ||
               m_output.texture_Inner1_Inner2_Output_size[0] != desiredSize[0] ||
               m_output.texture_Inner1_Inner2_Output_size[1] != desiredSize[1] ||
               m_output.texture_Inner1_Inner2_Output_size[2] != desiredSize[2] ||
               m_output.texture_Inner1_Inner2_Output_numMips != desiredNumMips ||
               m_output.texture_Inner1_Inner2_Output_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_Inner1_Inner2_Output)
                    s_delayedRelease.Add(m_output.texture_Inner1_Inner2_Output);

                m_output.texture_Inner1_Inner2_Output = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Inner1_Inner2_Output_flags, D3D12_RESOURCE_STATE_COPY_SOURCE, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Inner1_Inner2_Output" : nullptr), Context::LogFn);
                m_output.texture_Inner1_Inner2_Output_size[0] = desiredSize[0];
                m_output.texture_Inner1_Inner2_Output_size[1] = desiredSize[1];
                m_output.texture_Inner1_Inner2_Output_size[2] = desiredSize[2];
                m_output.texture_Inner1_Inner2_Output_numMips = desiredNumMips;
