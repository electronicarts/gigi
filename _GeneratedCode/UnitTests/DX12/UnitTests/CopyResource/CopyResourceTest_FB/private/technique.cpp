...
namespace CopyResourceTest_FB
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("CopyResourceTest_FB", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"CopyResourceTest_FB Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"CopyResourceTest_FB Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "CopyResourceTest_FB: Imported texture \"Source_Texture\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_Source_Buffer)
        {
            Context::LogFn(LogLevel::Error, "CopyResourceTest_FB: Imported buffer \"Source_Buffer\" is null.\n");
            return;
        }
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Texture_To_Texture", commandList, 4);
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Buffer_To_Buffer", commandList, 5);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
