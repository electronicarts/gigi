...
namespace CopyPartialBuffer
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_Init_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_Init_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: Init
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 2, samplers, 0, &ContextInternal::computeShader_Init_rootSig, (c_debugNames ? L"Init" : nullptr), Context::LogFn))
                return false;

            ShaderCompilationInfo shaderCompilationInfo;
            shaderCompilationInfo.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "CopyPartialBuffer_Init.hlsl";
            shaderCompilationInfo.entryPoint = "main";
            shaderCompilationInfo.shaderModel = "cs_6_1";
            shaderCompilationInfo.debugName = (c_debugNames ? "Init" : "");
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, shaderCompilationInfo,
               ContextInternal::computeShader_Init_rootSig, &ContextInternal::computeShader_Init_pso, Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_Init_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Init_pso);
            ContextInternal::computeShader_Init_pso = nullptr;
        }

        if(ContextInternal::computeShader_Init_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_Init_rootSig);
            ContextInternal::computeShader_Init_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: Init
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Partial_Copy
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Copy_Partial_Copy_source_0
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Partial_Self_Copy
    ...
    Context::~Context()
    {
        ...
        if(m_output.buffer_Input)
        {
            s_delayedRelease.Add(m_output.buffer_Input);
            m_output.buffer_Input = nullptr;
        }

        if(m_output.buffer_Output)
        {
            s_delayedRelease.Add(m_output.buffer_Output);
            m_output.buffer_Output = nullptr;
        }

        if(m_internal.buffer_Partial_Copy_source_Copy_0)
        {
            s_delayedRelease.Add(m_internal.buffer_Partial_Copy_source_Copy_0);
            m_internal.buffer_Partial_Copy_source_Copy_0 = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("CopyPartialBuffer", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"CopyPartialBuffer Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (4+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"CopyPartialBuffer Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_output.buffer_Input;
            ...
            barriers[1].Transition.pResource = context->m_output.buffer_Output;
        ...
        // Compute Shader: Init
        {
            ScopedPerfEvent scopedPerf("Compute Shader: Init", commandList, 1);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_Init_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_Init_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_output.buffer_Input, context->m_output.buffer_Input_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_Input_stride, context->m_output.buffer_Input_count, 0, 0, 0, false },
                { context->m_output.buffer_Output, context->m_output.buffer_Output_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_output.buffer_Output_stride, context->m_output.buffer_Output_count, 0, 0, 0, false }
            };
            ...
            unsigned int baseDispatchSize[3] = { context->m_output.buffer_Input_count, 1, 1 };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Init";
        ...
        // Copy Resource: Partial_Copy
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Partial_Copy", commandList, 3);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            int srcCopyableBytes = context->m_output.buffer_Input->GetDesc().Width - 12;
            int destCopyableBytes = context->m_output.buffer_Output->GetDesc().Width - 8;
            int copySize = min(srcCopyableBytes, destCopyableBytes);
            copySize = min(copySize, 40);
            commandList->CopyBufferRegion(context->m_output.buffer_Output, 8, context->m_output.buffer_Input, 12, copySize);

            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Partial_Copy";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            barriers[0].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[0].Transition.pResource = context->m_internal.buffer_Partial_Copy_source_Copy_0;
        ...
        // Copy Resource: Copy_Partial_Copy_source_0
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Partial_Copy_source_0", commandList, 6);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            // Copy buffer to buffer. Copy as much as we can, within the specified parameters.
            int srcCopyableBytes = context->m_output.buffer_Input->GetDesc().Width - 0;
            int destCopyableBytes = context->m_internal.buffer_Partial_Copy_source_Copy_0->GetDesc().Width - 0;
            int copySize = min(srcCopyableBytes, destCopyableBytes);
            commandList->CopyBufferRegion(context->m_internal.buffer_Partial_Copy_source_Copy_0, 0, context->m_output.buffer_Input, 0, copySize);

            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Copy_Partial_Copy_source_0";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            barriers[1].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[1].Transition.pResource = context->m_internal.buffer_Partial_Copy_source_Copy_0;
            barriers[1].Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
        ...
        // Copy Resource: Partial_Self_Copy
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Partial_Self_Copy", commandList, 4);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            // Copy buffer to buffer. Copy as much as we can, within the specified parameters.
            int srcCopyableBytes = context->m_internal.buffer_Partial_Copy_source_Copy_0->GetDesc().Width - 16;
            int destCopyableBytes = context->m_output.buffer_Input->GetDesc().Width - 0;
            int copySize = min(srcCopyableBytes, destCopyableBytes);
            copySize = min(copySize, 16);
            commandList->CopyBufferRegion(context->m_output.buffer_Input, 0, context->m_internal.buffer_Partial_Copy_source_Copy_0, 16, copySize);

            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Partial_Self_Copy";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Input
        {
            ...
            if(!m_output.buffer_Input ||
               m_output.buffer_Input_count != desiredCount ||
               m_output.buffer_Input_format != desiredFormat ||
               m_output.buffer_Input_stride != desiredStride)
            {
                dirty = true;
                if(m_output.buffer_Input)
                    s_delayedRelease.Add(m_output.buffer_Input);
                ...
                m_output.buffer_Input = DX12Utils::CreateBuffer(device, desiredSize, m_output.c_buffer_Input_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Input" : nullptr), Context::LogFn);
                m_output.buffer_Input_count = desiredCount;
                m_output.buffer_Input_format = desiredFormat;
                m_output.buffer_Input_stride = desiredStride;
            }
        ...
        // Output
        {
            ...
            if(!m_output.buffer_Output ||
               m_output.buffer_Output_count != desiredCount ||
               m_output.buffer_Output_format != desiredFormat ||
               m_output.buffer_Output_stride != desiredStride)
            {
                dirty = true;
                if(m_output.buffer_Output)
                    s_delayedRelease.Add(m_output.buffer_Output);
                ...
                m_output.buffer_Output = DX12Utils::CreateBuffer(device, desiredSize, m_output.c_buffer_Output_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Output" : nullptr), Context::LogFn);
                m_output.buffer_Output_count = desiredCount;
                m_output.buffer_Output_format = desiredFormat;
                m_output.buffer_Output_stride = desiredStride;
            }
        ...
        // Partial_Copy_source_Copy_0
        {
            ...
            if(!m_internal.buffer_Partial_Copy_source_Copy_0 ||
               m_internal.buffer_Partial_Copy_source_Copy_0_count != desiredCount ||
               m_internal.buffer_Partial_Copy_source_Copy_0_format != desiredFormat ||
               m_internal.buffer_Partial_Copy_source_Copy_0_stride != desiredStride)
            {
                dirty = true;
                if(m_internal.buffer_Partial_Copy_source_Copy_0)
                    s_delayedRelease.Add(m_internal.buffer_Partial_Copy_source_Copy_0);
                ...
                m_internal.buffer_Partial_Copy_source_Copy_0 = DX12Utils::CreateBuffer(device, desiredSize, m_internal.c_buffer_Partial_Copy_source_Copy_0_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Partial_Copy_source_Copy_0" : nullptr), Context::LogFn);
                m_internal.buffer_Partial_Copy_source_Copy_0_count = desiredCount;
                m_internal.buffer_Partial_Copy_source_Copy_0_format = desiredFormat;
                m_internal.buffer_Partial_Copy_source_Copy_0_stride = desiredStride;
            }
