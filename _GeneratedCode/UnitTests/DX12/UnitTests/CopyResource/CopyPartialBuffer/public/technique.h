...
namespace CopyPartialBuffer
{
    ...
    struct Context
    {
        ...
        {
            return "CopyPartialBuffer";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"CopyPartialBuffer";
        }
        ...
        struct ContextOutput
        {
            ...
            ID3D12Resource* buffer_Input = nullptr;
            DXGI_FORMAT buffer_Input_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Input_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Input_count = 0; // How many items there are
            const D3D12_RESOURCE_STATES c_buffer_Input_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            static const D3D12_RESOURCE_FLAGS c_buffer_Input_flags =  D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS; // Flags the buffer needs to have been created with

            ID3D12Resource* buffer_Output = nullptr;
            DXGI_FORMAT buffer_Output_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Output_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Output_count = 0; // How many items there are
            const D3D12_RESOURCE_STATES c_buffer_Output_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            static const D3D12_RESOURCE_FLAGS c_buffer_Output_flags =  D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS; // Flags the buffer needs to have been created with
        };
