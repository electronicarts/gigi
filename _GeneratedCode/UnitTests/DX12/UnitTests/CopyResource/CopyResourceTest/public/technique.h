...
namespace CopyResourceTest
{
    ...
    struct Context
    {
        ...
        {
            return "CopyResourceTest";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"CopyResourceTest";
        }
        ...
        struct ContextInput
        {
            ...
            ID3D12Resource* texture_Source_Texture = nullptr;
            unsigned int texture_Source_Texture_size[3] = { 0, 0, 0 };
            DXGI_FORMAT texture_Source_Texture_format = DXGI_FORMAT_UNKNOWN;
            static const D3D12_RESOURCE_FLAGS texture_Source_Texture_flags =  D3D12_RESOURCE_FLAG_NONE;
            D3D12_RESOURCE_STATES texture_Source_Texture_state = D3D12_RESOURCE_STATE_COMMON;

            ID3D12Resource* buffer_Source_Buffer = nullptr;
            DXGI_FORMAT buffer_Source_Buffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Source_Buffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Source_Buffer_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_Source_Buffer_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_Source_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
        ...
        struct ContextOutput
        {
            ...
            ID3D12Resource* texture_Texture_From_Texture = nullptr;
            unsigned int texture_Texture_From_Texture_size[3] = { 0, 0, 0 };
            DXGI_FORMAT texture_Texture_From_Texture_format = DXGI_FORMAT_UNKNOWN;
            static const D3D12_RESOURCE_FLAGS texture_Texture_From_Texture_flags =  D3D12_RESOURCE_FLAG_NONE;
            const D3D12_RESOURCE_STATES c_texture_Texture_From_Texture_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            ID3D12Resource* texture_Texture_From_Buffer = nullptr;
            unsigned int texture_Texture_From_Buffer_size[3] = { 0, 0, 0 };
            DXGI_FORMAT texture_Texture_From_Buffer_format = DXGI_FORMAT_UNKNOWN;
            static const D3D12_RESOURCE_FLAGS texture_Texture_From_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE;
            const D3D12_RESOURCE_STATES c_texture_Texture_From_Buffer_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            ID3D12Resource* buffer_Buffer_From_Texture = nullptr;
            DXGI_FORMAT buffer_Buffer_From_Texture_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Buffer_From_Texture_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Buffer_From_Texture_count = 0; // How many items there are
            const D3D12_RESOURCE_STATES c_buffer_Buffer_From_Texture_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            static const D3D12_RESOURCE_FLAGS c_buffer_Buffer_From_Texture_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with

            ID3D12Resource* buffer_Buffer_From_Buffer = nullptr;
            DXGI_FORMAT buffer_Buffer_From_Buffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Buffer_From_Buffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Buffer_From_Buffer_count = 0; // How many items there are
            const D3D12_RESOURCE_STATES c_buffer_Buffer_From_Buffer_endingState = D3D12_RESOURCE_STATE_COPY_DEST;

            static const D3D12_RESOURCE_FLAGS c_buffer_Buffer_From_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
        ...
        ID3D12Resource* CreateManagedTexture(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, D3D12_RESOURCE_FLAGS flags, DXGI_FORMAT format, const unsigned int size[3], DX12Utils::ResourceType resourceType, const void* initialData, const wchar_t* debugName, D3D12_RESOURCE_STATES desiredState = D3D12_RESOURCE_STATE_COPY_DEST);
        ID3D12Resource* CreateManagedTextureAndClear(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, D3D12_RESOURCE_FLAGS flags, DXGI_FORMAT format, const unsigned int size[3], DX12Utils::ResourceType resourceType, void* clearValue, size_t clearValueSize, const wchar_t* debugName, D3D12_RESOURCE_STATES desiredState = D3D12_RESOURCE_STATE_COPY_DEST);
        ...
        bool CreateManagedRTV(ID3D12Device* device, ID3D12Resource* resource, DXGI_FORMAT format, D3D12_RTV_DIMENSION dimension, int sliceIndex, int& rtvIndex, const char* debugText);
        bool CreateManagedDSV(ID3D12Device* device, ID3D12Resource* resource, DXGI_FORMAT format, D3D12_DSV_DIMENSION dimension, int sliceIndex, int& dsvIndex, const char* debugText);
