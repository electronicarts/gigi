...
namespace CopyResourceTest
{
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Copy_Texture_To_Texture
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Copy_Texture_To_Buffer
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Copy_Buffer_To_Texture
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: Copy_Buffer_To_Buffer
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_Texture_From_Texture)
        {
            s_delayedRelease.Add(m_output.texture_Texture_From_Texture);
            m_output.texture_Texture_From_Texture = nullptr;
        }

        if(m_output.texture_Texture_From_Buffer)
        {
            s_delayedRelease.Add(m_output.texture_Texture_From_Buffer);
            m_output.texture_Texture_From_Buffer = nullptr;
        }

        if(m_output.buffer_Buffer_From_Texture)
        {
            s_delayedRelease.Add(m_output.buffer_Buffer_From_Texture);
            m_output.buffer_Buffer_From_Texture = nullptr;
        }

        if(m_output.buffer_Buffer_From_Buffer)
        {
            s_delayedRelease.Add(m_output.buffer_Buffer_From_Buffer);
            m_output.buffer_Buffer_From_Buffer = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("CopyResourceTest", commandList, 10);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"CopyResourceTest Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (4+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"CopyResourceTest Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        if (!context->m_input.texture_Source_Texture)
        {
            Context::LogFn(LogLevel::Error, "CopyResourceTest: Imported texture \"Source_Texture\" is null.\n");
            return;
        }

        if (!context->m_input.buffer_Source_Buffer)
        {
            Context::LogFn(LogLevel::Error, "CopyResourceTest: Imported buffer \"Source_Buffer\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.texture_Source_Texture_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.texture_Source_Texture;
                barriers[barrierCount].Transition.StateBefore = context->m_input.texture_Source_Texture_state;
                barriers[barrierCount].Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }

            if(context->m_input.buffer_Source_Buffer_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Source_Buffer;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_Source_Buffer_state;
                barriers[barrierCount].Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Texture_To_Texture", commandList, 6);
        ...
        // Copy Resource: Copy_Texture_To_Buffer
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Texture_To_Buffer", commandList, 7);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            D3D12_RESOURCE_DESC srcResourceDesc = context->m_input.texture_Source_Texture->GetDesc();
            ...
            srcLoc.pResource = context->m_input.texture_Source_Texture;
            srcLoc.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
            srcLoc.SubresourceIndex = 0;

            D3D12_TEXTURE_COPY_LOCATION destLoc = {};
            destLoc.pResource = context->m_output.buffer_Buffer_From_Texture;
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Copy_Texture_To_Buffer";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }

        // Copy Resource: Copy_Buffer_To_Texture
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Buffer_To_Texture", commandList, 8);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            D3D12_RESOURCE_DESC destResourceDesc = context->m_output.texture_Texture_From_Buffer->GetDesc();
            ...
            D3D12_TEXTURE_COPY_LOCATION destLoc = {};
            destLoc.pResource = context->m_output.texture_Texture_From_Buffer;
            destLoc.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
            destLoc.SubresourceIndex = 0;

            D3D12_TEXTURE_COPY_LOCATION srcLoc = {};
            srcLoc.pResource = context->m_input.buffer_Source_Buffer;
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Copy_Buffer_To_Texture";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        {
            ScopedPerfEvent scopedPerf("Copy Resource: Copy_Buffer_To_Buffer", commandList, 9);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            int srcCopyableBytes = context->m_input.buffer_Source_Buffer->GetDesc().Width - 0;
            int destCopyableBytes = context->m_output.buffer_Buffer_From_Buffer->GetDesc().Width - 0;
            int copySize = min(srcCopyableBytes, destCopyableBytes);
            commandList->CopyBufferRegion(context->m_output.buffer_Buffer_From_Buffer, 0, context->m_input.buffer_Source_Buffer, 0, copySize);
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.texture_Source_Texture_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.texture_Source_Texture_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
            ...
            if(context->m_input.buffer_Source_Buffer_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                ...
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Source_Buffer;
                barriers[barrierCount].Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_SOURCE;
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_Source_Buffer_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Texture_From_Texture
        {
            unsigned int baseSize[3] = {
                m_input.texture_Source_Texture_size[0],
                m_input.texture_Source_Texture_size[1],
                m_input.texture_Source_Texture_size[2]
            };
            ...
            DXGI_FORMAT desiredFormat = m_input.texture_Source_Texture_format;

            if(!m_output.texture_Texture_From_Texture ||
               m_output.texture_Texture_From_Texture_size[0] != desiredSize[0] ||
               m_output.texture_Texture_From_Texture_size[1] != desiredSize[1] ||
               m_output.texture_Texture_From_Texture_size[2] != desiredSize[2] ||
               m_output.texture_Texture_From_Texture_numMips != desiredNumMips ||
               m_output.texture_Texture_From_Texture_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_Texture_From_Texture)
                    s_delayedRelease.Add(m_output.texture_Texture_From_Texture);

                m_output.texture_Texture_From_Texture = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Texture_From_Texture_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Texture_From_Texture" : nullptr), Context::LogFn);
                m_output.texture_Texture_From_Texture_size[0] = desiredSize[0];
                m_output.texture_Texture_From_Texture_size[1] = desiredSize[1];
                m_output.texture_Texture_From_Texture_size[2] = desiredSize[2];
                m_output.texture_Texture_From_Texture_numMips = desiredNumMips;
        ...
        // Texture_From_Buffer
        {
            ...
            if(!m_output.texture_Texture_From_Buffer ||
               m_output.texture_Texture_From_Buffer_size[0] != desiredSize[0] ||
               m_output.texture_Texture_From_Buffer_size[1] != desiredSize[1] ||
               m_output.texture_Texture_From_Buffer_size[2] != desiredSize[2] ||
               m_output.texture_Texture_From_Buffer_numMips != desiredNumMips ||
               m_output.texture_Texture_From_Buffer_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_Texture_From_Buffer)
                    s_delayedRelease.Add(m_output.texture_Texture_From_Buffer);

                m_output.texture_Texture_From_Buffer = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_Texture_From_Buffer_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Texture_From_Buffer" : nullptr), Context::LogFn);
                m_output.texture_Texture_From_Buffer_size[0] = desiredSize[0];
                m_output.texture_Texture_From_Buffer_size[1] = desiredSize[1];
                m_output.texture_Texture_From_Buffer_size[2] = desiredSize[2];
                m_output.texture_Texture_From_Buffer_numMips = desiredNumMips;
        ...
        // Buffer_From_Texture
        {
            unsigned int baseCount = m_input.buffer_Source_Buffer_count;
            unsigned int desiredCount = ((baseCount + 0 ) * 1) / 1 + 0;
            DXGI_FORMAT desiredFormat = m_input.buffer_Source_Buffer_format;
            unsigned int desiredStride = m_input.buffer_Source_Buffer_stride;

            if(!m_output.buffer_Buffer_From_Texture ||
               m_output.buffer_Buffer_From_Texture_count != desiredCount ||
               m_output.buffer_Buffer_From_Texture_format != desiredFormat ||
               m_output.buffer_Buffer_From_Texture_stride != desiredStride)
            {
                dirty = true;
                if(m_output.buffer_Buffer_From_Texture)
                    s_delayedRelease.Add(m_output.buffer_Buffer_From_Texture);
                ...
                m_output.buffer_Buffer_From_Texture = DX12Utils::CreateBuffer(device, desiredSize, m_output.c_buffer_Buffer_From_Texture_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Buffer_From_Texture" : nullptr), Context::LogFn);
                m_output.buffer_Buffer_From_Texture_count = desiredCount;
                m_output.buffer_Buffer_From_Texture_format = desiredFormat;
                m_output.buffer_Buffer_From_Texture_stride = desiredStride;
            }
        ...
        // Buffer_From_Buffer
        {
            ...
            if(!m_output.buffer_Buffer_From_Buffer ||
               m_output.buffer_Buffer_From_Buffer_count != desiredCount ||
               m_output.buffer_Buffer_From_Buffer_format != desiredFormat ||
               m_output.buffer_Buffer_From_Buffer_stride != desiredStride)
            {
                dirty = true;
                if(m_output.buffer_Buffer_From_Buffer)
                    s_delayedRelease.Add(m_output.buffer_Buffer_From_Buffer);
                ...
                m_output.buffer_Buffer_From_Buffer = DX12Utils::CreateBuffer(device, desiredSize, m_output.c_buffer_Buffer_From_Buffer_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"Buffer_From_Buffer" : nullptr), Context::LogFn);
                m_output.buffer_Buffer_From_Buffer_count = desiredCount;
                m_output.buffer_Buffer_From_Buffer_format = desiredFormat;
                m_output.buffer_Buffer_From_Buffer_stride = desiredStride;
            }
