...
namespace GPUWrite
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_ComputeShader_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_ComputeShader_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: ComputeShader
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 5, samplers, 0, &ContextInternal::computeShader_ComputeShader_rootSig, (c_debugNames ? L"ComputeShader" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/GPUWrite.hlsl", "csmain", "cs_6_1", defines,
               ContextInternal::computeShader_ComputeShader_rootSig, &ContextInternal::computeShader_ComputeShader_pso, c_debugShaders, (c_debugNames ? L"ComputeShader" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_ComputeShader_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_ComputeShader_pso);
            ContextInternal::computeShader_ComputeShader_pso = nullptr;
        }

        if(ContextInternal::computeShader_ComputeShader_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_ComputeShader_rootSig);
            ContextInternal::computeShader_ComputeShader_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: ComputeShader
    ...
    Context::~Context()
    {
        ...
        if(m_internal.texture_Texture2D)
        {
            s_delayedRelease.Add(m_internal.texture_Texture2D);
            m_internal.texture_Texture2D = nullptr;
        }

        if(m_internal.texture_Texture2DArray)
        {
            s_delayedRelease.Add(m_internal.texture_Texture2DArray);
            m_internal.texture_Texture2DArray = nullptr;
        }

        if(m_internal.texture_Texture3D)
        {
            s_delayedRelease.Add(m_internal.texture_Texture3D);
            m_internal.texture_Texture3D = nullptr;
        }

        if(m_internal.buffer_FloatBuffer)
        {
            s_delayedRelease.Add(m_internal.buffer_FloatBuffer);
            m_internal.buffer_FloatBuffer = nullptr;
        }

        if(m_internal.buffer_StructBuffer)
        {
            s_delayedRelease.Add(m_internal.buffer_StructBuffer);
            m_internal.buffer_StructBuffer = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("GPUWrite", commandList, 6);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"GPUWrite Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"GPUWrite Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].UAV.pResource = context->m_internal.texture_Texture2D;

            barriers[1].Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
            barriers[1].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[1].UAV.pResource = context->m_internal.texture_Texture2DArray;

            barriers[2].Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
            barriers[2].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[2].UAV.pResource = context->m_internal.texture_Texture3D;

            barriers[3].Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
            barriers[3].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[3].UAV.pResource = context->m_internal.buffer_FloatBuffer;

            barriers[4].Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
            barriers[4].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[4].UAV.pResource = context->m_internal.buffer_StructBuffer;
        ...
        // Compute Shader: ComputeShader
        {
            ScopedPerfEvent scopedPerf("Compute Shader: ComputeShader", commandList, 5);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_ComputeShader_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_ComputeShader_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_internal.texture_Texture2D, context->m_internal.texture_Texture2D_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 },
                { context->m_internal.texture_Texture2DArray, context->m_internal.texture_Texture2DArray_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2DArray, false, 0, context->m_internal.texture_Texture2DArray_size[2] },
                { context->m_internal.texture_Texture3D, context->m_internal.texture_Texture3D_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture3D, false, 0, context->m_internal.texture_Texture3D_size[2] },
                { context->m_internal.buffer_FloatBuffer, context->m_internal.buffer_FloatBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_internal.buffer_FloatBuffer_stride, context->m_internal.buffer_FloatBuffer_count },
                { context->m_internal.buffer_StructBuffer, context->m_internal.buffer_StructBuffer_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Buffer, false, context->m_internal.buffer_StructBuffer_stride, context->m_internal.buffer_StructBuffer_count }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "ComputeShader";
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Texture2D
        {
            ...
            if(!m_internal.texture_Texture2D ||
               m_internal.texture_Texture2D_size[0] != desiredSize[0] ||
               m_internal.texture_Texture2D_size[1] != desiredSize[1] ||
               m_internal.texture_Texture2D_size[2] != desiredSize[2] ||
               m_internal.texture_Texture2D_format != desiredFormat)
            {
                dirty = true;
                if(m_internal.texture_Texture2D)
                    s_delayedRelease.Add(m_internal.texture_Texture2D);

                m_internal.texture_Texture2D = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_internal.texture_Texture2D_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Texture2D" : nullptr), Context::LogFn);
                m_internal.texture_Texture2D_size[0] = desiredSize[0];
                m_internal.texture_Texture2D_size[1] = desiredSize[1];
                m_internal.texture_Texture2D_size[2] = desiredSize[2];
                m_internal.texture_Texture2D_format = desiredFormat;
        ...
        // Texture2DArray
        {
            ...
            DXGI_FORMAT desiredFormat = m_internal.texture_Texture2D_format;

            if(!m_internal.texture_Texture2DArray ||
               m_internal.texture_Texture2DArray_size[0] != desiredSize[0] ||
               m_internal.texture_Texture2DArray_size[1] != desiredSize[1] ||
               m_internal.texture_Texture2DArray_size[2] != desiredSize[2] ||
               m_internal.texture_Texture2DArray_format != desiredFormat)
            {
                dirty = true;
                if(m_internal.texture_Texture2DArray)
                    s_delayedRelease.Add(m_internal.texture_Texture2DArray);

                m_internal.texture_Texture2DArray = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_internal.texture_Texture2DArray_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture2DArray, (c_debugNames ? L"Texture2DArray" : nullptr), Context::LogFn);
                m_internal.texture_Texture2DArray_size[0] = desiredSize[0];
                m_internal.texture_Texture2DArray_size[1] = desiredSize[1];
                m_internal.texture_Texture2DArray_size[2] = desiredSize[2];
                m_internal.texture_Texture2DArray_format = desiredFormat;
        ...
        // Texture3D
        {
            ...
            if(!m_internal.texture_Texture3D ||
               m_internal.texture_Texture3D_size[0] != desiredSize[0] ||
               m_internal.texture_Texture3D_size[1] != desiredSize[1] ||
               m_internal.texture_Texture3D_size[2] != desiredSize[2] ||
               m_internal.texture_Texture3D_format != desiredFormat)
            {
                dirty = true;
                if(m_internal.texture_Texture3D)
                    s_delayedRelease.Add(m_internal.texture_Texture3D);

                m_internal.texture_Texture3D = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_internal.texture_Texture3D_flags, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, DX12Utils::ResourceType::Texture3D, (c_debugNames ? L"Texture3D" : nullptr), Context::LogFn);
                m_internal.texture_Texture3D_size[0] = desiredSize[0];
                m_internal.texture_Texture3D_size[1] = desiredSize[1];
                m_internal.texture_Texture3D_size[2] = desiredSize[2];
                m_internal.texture_Texture3D_format = desiredFormat;
        ...
        // FloatBuffer
        {
            ...
            unsigned int desiredCount = ((baseCount + 0 ) * 8) / 1 + 0;
            DXGI_FORMAT desiredFormat = DXGI_FORMAT_R32_FLOAT;
            unsigned int desiredStride = 0;

            if(!m_internal.buffer_FloatBuffer ||
               m_internal.buffer_FloatBuffer_count != desiredCount ||
               m_internal.buffer_FloatBuffer_format != desiredFormat ||
               m_internal.buffer_FloatBuffer_stride != desiredStride)
            {
                dirty = true;
                if(m_internal.buffer_FloatBuffer)
                    s_delayedRelease.Add(m_internal.buffer_FloatBuffer);
                ...
                m_internal.buffer_FloatBuffer = DX12Utils::CreateBuffer(device, desiredSize, m_internal.c_buffer_FloatBuffer_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"FloatBuffer" : nullptr), Context::LogFn);
                m_internal.buffer_FloatBuffer_count = desiredCount;
                m_internal.buffer_FloatBuffer_format = desiredFormat;
                m_internal.buffer_FloatBuffer_stride = desiredStride;
            }
        ...
        // StructBuffer
        {
            ...
            if(!m_internal.buffer_StructBuffer ||
               m_internal.buffer_StructBuffer_count != desiredCount ||
               m_internal.buffer_StructBuffer_format != desiredFormat ||
               m_internal.buffer_StructBuffer_stride != desiredStride)
            {
                dirty = true;
                if(m_internal.buffer_StructBuffer)
                    s_delayedRelease.Add(m_internal.buffer_StructBuffer);
                ...
                m_internal.buffer_StructBuffer = DX12Utils::CreateBuffer(device, desiredSize, m_internal.c_buffer_StructBuffer_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"StructBuffer" : nullptr), Context::LogFn);
                m_internal.buffer_StructBuffer_count = desiredCount;
                m_internal.buffer_StructBuffer_format = desiredFormat;
                m_internal.buffer_StructBuffer_stride = desiredStride;
            }
