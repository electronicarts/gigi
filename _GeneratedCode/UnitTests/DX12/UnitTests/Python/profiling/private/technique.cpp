...
namespace profiling
{
    ...
    // Runs the shader
    ID3D12PipelineState* ContextInternal::computeShader_DoSimpleCS_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_DoSimpleCS_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Runs the shader
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_DoSimpleCS_rootSig, (c_debugNames ? L"DoSimpleCS" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/profiling.hlsl", "Main", "cs_6_1", defines,
               ContextInternal::computeShader_DoSimpleCS_rootSig, &ContextInternal::computeShader_DoSimpleCS_pso, c_debugShaders, (c_debugNames ? L"DoSimpleCS" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        // Runs the shader
        if(ContextInternal::computeShader_DoSimpleCS_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_DoSimpleCS_pso);
            ContextInternal::computeShader_DoSimpleCS_pso = nullptr;
        }

        if(ContextInternal::computeShader_DoSimpleCS_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_DoSimpleCS_rootSig);
            ContextInternal::computeShader_DoSimpleCS_rootSig = nullptr;
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("profiling", commandList, 2);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"profiling Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"profiling Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "profiling: Imported texture \"Input\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.texture_Input_state != D3D12_RESOURCE_STATE_UNORDERED_ACCESS)
            {
            ...
            else
            {
                ...
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].UAV.pResource = context->m_input.texture_Input;
                barrierCount++;
            }
        ...
        // Runs the shader
        {
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_DoSimpleCS_rootSig);
            commandList->SetPipelineState(ContextInternal::computeShader_DoSimpleCS_pso);

            DX12Utils::ResourceDescriptor descriptors[] = {
                { context->m_input.texture_Input, context->m_input.texture_Input_format, DX12Utils::AccessType::UAV, DX12Utils::ResourceType::Texture2D, false, 0, 0 }
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DoSimpleCS";
