...
namespace Stencil
{
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_Color_rtv != -1)
        {
            s_heapAllocationTrackerRTV.Free(m_output.texture_Color_rtv);
            m_output.texture_Color_rtv = -1;
        }
        ...
        if(m_output.texture_Depth_Stencil_dsv != -1)
        {
            s_heapAllocationTrackerRTV.Free(m_output.texture_Depth_Stencil_dsv);
            m_output.texture_Depth_Stencil_dsv = -1;
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Draw Call: Draw_1
        {
            ...
            {
                { context->m_internal.constantBuffer__Draw1VSCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1 },
            };
            ...
            // Clear Color
            {
                 ...
                 commandList->ClearRenderTargetView(s_heapAllocationTrackerRTV.GetCPUHandle(context->m_output.texture_Color_rtv), clearValues, 0, nullptr);
            }

            // Clear Depth_Stencil
            {
                D3D12_CLEAR_FLAGS clearFlags = D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL;
                commandList->ClearDepthStencilView(s_heapAllocationTrackerDSV.GetCPUHandle(context->m_output.texture_Depth_Stencil_dsv), clearFlags, 0.000000f, 0, 0, nullptr);
            }
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->m_output.texture_Color_rtv),
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->m_output.texture_Depth_Stencil_dsv);
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Color
        {
               ...
               m_output.texture_Color_format != desiredFormat)
            {
                ...
                m_output.texture_Color = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Color_flags, D3D12_RESOURCE_STATE_RENDER_TARGET, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Color" : nullptr), Context::LogFn);
                m_output.texture_Color_size[0] = desiredSize[0];
                m_output.texture_Color_size[1] = desiredSize[1];
                m_output.texture_Color_size[2] = desiredSize[2];
                m_output.texture_Color_format = desiredFormat;
                ...
                if(m_output.texture_Color_rtv == -1 && !s_heapAllocationTrackerRTV.Allocate(m_output.texture_Color_rtv, "Color"))
                    Context::LogFn(LogLevel::Error, "Ran out of RTV descriptors, please increase c_numRTVDescriptors");

                // Create RTV
                D3D12_RENDER_TARGET_VIEW_DESC rtvDesc;
                rtvDesc.Format = m_output.texture_Color_format;
                rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
                rtvDesc.Texture2D.MipSlice = 0;
                rtvDesc.Texture2D.PlaneSlice = 0;
                device->CreateRenderTargetView(m_output.texture_Color, &rtvDesc, s_heapAllocationTrackerRTV.GetCPUHandle(m_output.texture_Color_rtv));
            }
        }
        ...
        // Depth_Stencil
        {
               ...
               m_output.texture_Depth_Stencil_format != desiredFormat)
            {
                ...
                m_output.texture_Depth_Stencil = DX12Utils::CreateTexture(device, desiredSize, desiredFormat, m_output.texture_Depth_Stencil_flags, D3D12_RESOURCE_STATE_DEPTH_WRITE, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"Depth_Stencil" : nullptr), Context::LogFn);
                m_output.texture_Depth_Stencil_size[0] = desiredSize[0];
                m_output.texture_Depth_Stencil_size[1] = desiredSize[1];
                m_output.texture_Depth_Stencil_size[2] = desiredSize[2];
                m_output.texture_Depth_Stencil_format = desiredFormat;
                ...
                if(m_output.texture_Depth_Stencil_dsv == -1 && !s_heapAllocationTrackerDSV.Allocate(m_output.texture_Depth_Stencil_dsv, "Depth_Stencil"))
                    Context::LogFn(LogLevel::Error, "Ran out of DSV descriptors, please increase c_numDSVDescriptors");

                // Create DSV
                D3D12_DEPTH_STENCIL_VIEW_DESC dsvDesc;
                dsvDesc.Format = DX12Utils::DSV_Safe_DXGI_FORMAT(m_output.texture_Depth_Stencil_format);
                dsvDesc.Flags = D3D12_DSV_FLAG_NONE;
                dsvDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
                dsvDesc.Texture2D.MipSlice = 0;
                device->CreateDepthStencilView(m_output.texture_Depth_Stencil, &dsvDesc, s_heapAllocationTrackerDSV.GetCPUHandle(m_output.texture_Depth_Stencil_dsv));
            }
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Draw_1_pso || !m_internal.drawCall_Draw_1_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/StencilDraw1VS.hlsl", "Draw1VS", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/StencilDraw1PS.hlsl", "Draw1PS", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
        ...
        if(!m_internal.drawCall_Draw_2_pso || !m_internal.drawCall_Draw_2_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/StencilDraw2VS.hlsl", "Draw2VS", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/StencilDraw2PS.hlsl", "Draw2PS", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            if (byteCodePS.size() == 0)
                return false;
