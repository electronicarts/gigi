...
namespace YesVertexStruct_YesIndex_NoInstance
{
    ...
    struct Context
    {
        ...
        {
            return "YesVertexStruct_YesIndex_NoInstance";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"YesVertexStruct_YesIndex_NoInstance";
        }
        ...
        struct ContextInput
        {
            ...
            ViewMode variable_viewMode = ViewMode::UV;
            ...
            ID3D12Resource* buffer_Index_Buffer = nullptr;
            DXGI_FORMAT buffer_Index_Buffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Index_Buffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Index_Buffer_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_Index_Buffer_state = D3D12_RESOURCE_STATE_COMMON;

            static const D3D12_RESOURCE_FLAGS c_buffer_Index_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
