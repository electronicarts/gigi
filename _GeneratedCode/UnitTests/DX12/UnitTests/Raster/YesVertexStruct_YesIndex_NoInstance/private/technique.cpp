...
namespace YesVertexStruct_YesIndex_NoInstance
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("YesVertexStruct_YesIndex_NoInstance", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"YesVertexStruct_YesIndex_NoInstance Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"YesVertexStruct_YesIndex_NoInstance Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_YesIndex_NoInstance: Imported buffer \"Vertex_Buffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_Vertex_Buffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_YesIndex_NoInstance: Imported buffer \"Vertex_Buffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }

        if (!context->m_input.buffer_Index_Buffer)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_YesIndex_NoInstance: Imported buffer \"Index_Buffer\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_Index_Buffer_state != D3D12_RESOURCE_STATE_INDEX_BUFFER)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Index_Buffer;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_Index_Buffer_state;
                barriers[barrierCount].Transition.StateAfter = D3D12_RESOURCE_STATE_INDEX_BUFFER;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
        ...
        // Draw Call: Rasterize
        {
            ...
            // Index Buffer
            int indexCountPerInstance = context->m_input.buffer_Index_Buffer_count;

            D3D12_INDEX_BUFFER_VIEW ibView;
            ibView.BufferLocation = context->m_input.buffer_Index_Buffer->GetGPUVirtualAddress();
            ibView.SizeInBytes = DX12Utils::Get_DXGI_FORMAT_Info(context->m_input.buffer_Index_Buffer_format, Context::LogFn).bytesPerPixel * context->m_input.buffer_Index_Buffer_count;
            ibView.Format = context->m_input.buffer_Index_Buffer_format;
            ...
            // Clear Color_Buffer
            {
                 ...
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_YesIndex_NoInstance.Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for YesVertexStruct_YesIndex_NoInstance.Color_Buffer");
            ...
            // Clear Depth_Buffer
            {
                 ...
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_YesIndex_NoInstance.Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for YesVertexStruct_YesIndex_NoInstance.Depth_Buffer");
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_YesIndex_NoInstance.Color_Buffer"))
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_YesIndex_NoInstance.Depth_Buffer"));
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_Index_Buffer_state != D3D12_RESOURCE_STATE_INDEX_BUFFER)
            {
                ...
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Index_Buffer;
                barriers[barrierCount].Transition.StateBefore = D3D12_RESOURCE_STATE_INDEX_BUFFER;
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_Index_Buffer_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_YesIndex_NoInstance_VS.hlsl", "VSMain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_YesIndex_NoInstance_PS.hlsl", "PSMain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
