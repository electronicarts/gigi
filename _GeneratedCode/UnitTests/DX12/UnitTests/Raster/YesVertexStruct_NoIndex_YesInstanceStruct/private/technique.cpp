...
namespace YesVertexStruct_NoIndex_YesInstanceStruct
{
    ...
    Context::~Context()
    {
        ...
        // _PixelShaderCB
        if (m_internal.constantBuffer__PixelShaderCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__PixelShaderCB);
            m_internal.constantBuffer__PixelShaderCB = nullptr;
        }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("YesVertexStruct_NoIndex_YesInstanceStruct", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"YesVertexStruct_NoIndex_YesInstanceStruct Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"YesVertexStruct_NoIndex_YesInstanceStruct Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceStruct: Imported buffer \"Vertex_Buffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_Vertex_Buffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceStruct: Imported buffer \"Vertex_Buffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }

        if (!context->m_input.buffer_Instance_Buffer)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceStruct: Imported buffer \"Instance_Buffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_Instance_Buffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceStruct: Imported buffer \"Instance_Buffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_Instance_Buffer_state != D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Instance_Buffer;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_Instance_Buffer_state;
                barriers[barrierCount].Transition.StateAfter = D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
        ...
        {
            context->m_internal.constantBuffer__PixelShaderCB_cpu.viewMode = (int)context->m_input.variable_viewMode;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__PixelShaderCB, context->m_internal.constantBuffer__PixelShaderCB_cpu, Context::LogFn);
        }
        ...
        // Draw Call: Rasterize
        {
            ...
            // Instance Buffer
            int instanceCount = context->m_input.buffer_Instance_Buffer_count;

            D3D12_VERTEX_BUFFER_VIEW ivbView;
            ivbView.BufferLocation = context->m_input.buffer_Instance_Buffer->GetGPUVirtualAddress();
            ivbView.StrideInBytes = (context->m_input.buffer_Instance_Buffer_format == DXGI_FORMAT_UNKNOWN)
                ? context->m_input.buffer_Instance_Buffer_stride
                : DX12Utils::Get_DXGI_FORMAT_Info(context->m_input.buffer_Instance_Buffer_format, Context::LogFn).bytesPerPixel;
            ivbView.SizeInBytes = ivbView.StrideInBytes * context->m_input.buffer_Instance_Buffer_count;
            ...
            // Clear Color_Buffer
            {
                 ...
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_NoIndex_YesInstanceStruct.Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for YesVertexStruct_NoIndex_YesInstanceStruct.Color_Buffer");
            ...
            // Clear Depth_Buffer
            {
                 ...
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_NoIndex_YesInstanceStruct.Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for YesVertexStruct_NoIndex_YesInstanceStruct.Depth_Buffer");
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_NoIndex_YesInstanceStruct.Color_Buffer"))
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "YesVertexStruct_NoIndex_YesInstanceStruct.Depth_Buffer"));
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_Instance_Buffer_state != D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER)
            {
                ...
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_Instance_Buffer;
                barriers[barrierCount].Transition.StateBefore = D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_Instance_Buffer_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // _PixelShaderCB
        if (m_internal.constantBuffer__PixelShaderCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__PixelShaderCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_PixelShaderCB" : nullptr), Context::LogFn);
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            // _PixelShaderCB
            rangesPixel[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_NoIndex_YesInstanceStruct_VS.hlsl", "VSMain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_NoIndex_YesInstanceStruct_PS.hlsl", "PSMain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            ...
            vertexInputLayout.insert(vertexInputLayout.end(), m_input.buffer_Instance_Buffer_vertexInputLayout.begin(), m_input.buffer_Instance_Buffer_vertexInputLayout.end());
