...
namespace YesVertexStruct_NoIndex_YesInstanceStruct
{
    ...
    struct Context
    {
        ...
        {
            return "YesVertexStruct_NoIndex_YesInstanceStruct";
        }

        static const wchar_t* GetTechniqueNameW()
        {
            return L"YesVertexStruct_NoIndex_YesInstanceStruct";
        }
        ...
        struct ContextInput
        {
            ...
            DXGI_FORMAT buffer_Vertex_Buffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Vertex_Buffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Vertex_Buffer_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_Vertex_Buffer_state = D3D12_RESOURCE_STATE_COMMON;
            std::vector<D3D12_INPUT_ELEMENT_DESC> buffer_Vertex_Buffer_vertexInputLayout;

            static const D3D12_RESOURCE_FLAGS c_buffer_Vertex_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with

            ID3D12Resource* buffer_Instance_Buffer = nullptr;
            DXGI_FORMAT buffer_Instance_Buffer_format = DXGI_FORMAT_UNKNOWN; // For typed buffers, the type of the buffer
            unsigned int buffer_Instance_Buffer_stride = 0; // For structured buffers, the size of the structure
            unsigned int buffer_Instance_Buffer_count = 0; // How many items there are
            D3D12_RESOURCE_STATES buffer_Instance_Buffer_state = D3D12_RESOURCE_STATE_COMMON;
            std::vector<D3D12_INPUT_ELEMENT_DESC> buffer_Instance_Buffer_vertexInputLayout;

            static const D3D12_RESOURCE_FLAGS c_buffer_Instance_Buffer_flags =  D3D12_RESOURCE_FLAG_NONE; // Flags the buffer needs to have been created with
        };
        ...
        struct ContextOutput
        {
            ...
            int texture_Color_Buffer_rtv = -1;
            ...
            int texture_Depth_Buffer_dsv = -1;
