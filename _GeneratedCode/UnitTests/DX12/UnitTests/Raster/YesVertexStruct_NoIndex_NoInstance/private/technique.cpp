...
namespace YesVertexStruct_NoIndex_NoInstance
{
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            shaderCompilationInfoVS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "YesVertexStruct_NoIndex_NoInstance_VS.hlsl";
            ...
            shaderCompilationInfoPS.fileName = std::filesystem::path(Context::s_techniqueLocation) / "shaders" / "YesVertexStruct_NoIndex_NoInstance_PS.hlsl";
