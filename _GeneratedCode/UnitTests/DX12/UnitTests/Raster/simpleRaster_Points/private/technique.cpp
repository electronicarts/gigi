...
namespace simpleRaster_Points
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("simpleRaster_Points", commandList, 5);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"simpleRaster_Points Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"simpleRaster_Points Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "simpleRaster_Points: Imported buffer \"VertexBuffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_VertexBuffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "simpleRaster_Points: Imported buffer \"VertexBuffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }
        ...
        // Draw Call: Rasterize
        {
            ...
            // Clear Color_Buffer
            {
                 ...
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "simpleRaster_Points.Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for simpleRaster_Points.Color_Buffer");
            ...
            // Clear Depth_Buffer
            {
                 ...
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "simpleRaster_Points.Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for simpleRaster_Points.Depth_Buffer");
            ...
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "simpleRaster_Points.Color_Buffer"))
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "simpleRaster_Points.Depth_Buffer"));
