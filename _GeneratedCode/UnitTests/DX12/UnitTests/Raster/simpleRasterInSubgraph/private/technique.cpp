...
namespace simpleRasterInSubgraph
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Copy Resource: DoSimpleRaster_CopyVBs
        {
            ...
            int srcCopyableBytes = context->m_input.buffer_VB->GetDesc().Width - 0;
            int destCopyableBytes = context->m_internal.buffer_DoSimpleRaster_VBCopy->GetDesc().Width - 0;
            int copySize = min(srcCopyableBytes, destCopyableBytes);
            commandList->CopyBufferRegion(context->m_internal.buffer_DoSimpleRaster_VBCopy, 0, context->m_input.buffer_VB, 0, copySize);
        ...
        // Draw Call: DoSimpleRaster_Rasterize
        {
            ...
            {
                { context->m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0, 0, 0, false },
            };
