...
namespace simpleRasterInSubgraph
{
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // copy resource: DoSimpleRaster_CopyVBs
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // Draw Call: DoSimpleRaster_Rasterize
    ...
    Context::~Context()
    {
        ...
        if(m_output.texture_DoSimpleRaster_Color_Buffer)
        {
            s_delayedRelease.Add(m_output.texture_DoSimpleRaster_Color_Buffer);
            m_output.texture_DoSimpleRaster_Color_Buffer = nullptr;
        }

        if(m_output.texture_DoSimpleRaster_Depth_Buffer)
        {
            s_delayedRelease.Add(m_output.texture_DoSimpleRaster_Depth_Buffer);
            m_output.texture_DoSimpleRaster_Depth_Buffer = nullptr;
        }

        // This is here for the benefit of simpleRasterInSubgraph. Need a reference to a struct type in a subgraph.
        if(m_internal.buffer_DoSimpleRaster_VBCopy)
        {
            s_delayedRelease.Add(m_internal.buffer_DoSimpleRaster_VBCopy);
            m_internal.buffer_DoSimpleRaster_VBCopy = nullptr;
        }

        // _DoSimpleRaster_VertexShaderCB
        if (m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB);
            m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB = nullptr;
        }

        if(m_internal.drawCall_DoSimpleRaster_Rasterize_pso)
        {
            s_delayedRelease.Add(m_internal.drawCall_DoSimpleRaster_Rasterize_pso);
            m_internal.drawCall_DoSimpleRaster_Rasterize_pso = nullptr;
        }

        if(m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig)
        {
            s_delayedRelease.Add(m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig);
            m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("simpleRasterInSubgraph", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"simpleRasterInSubgraph Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (2+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"simpleRasterInSubgraph Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        if (!context->m_input.buffer_VB)
        {
            Context::LogFn(LogLevel::Error, "simpleRasterInSubgraph: Imported buffer \"VB\" is null.\n");
            return;
        }
        ...
        // Make sure imported resources are in the correct state
        {
            ...
            if(context->m_input.buffer_VB_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                barriers[barrierCount].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
                barriers[barrierCount].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                barriers[barrierCount].Transition.pResource = context->m_input.buffer_VB;
                barriers[barrierCount].Transition.StateBefore = context->m_input.buffer_VB_state;
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_internal.buffer_DoSimpleRaster_VBCopy;
        ...
        // Copy Resource: DoSimpleRaster_CopyVBs
        {
            ScopedPerfEvent scopedPerf("Copy Resource: DoSimpleRaster_CopyVBs", commandList, 5);
            ...
            unsigned int srcSize = context->m_input.buffer_VB->GetDesc().Width;
            unsigned int destSize = context->m_internal.buffer_DoSimpleRaster_VBCopy->GetDesc().Width;
            if (srcSize == destSize)
                commandList->CopyResource(context->m_internal.buffer_DoSimpleRaster_VBCopy, context->m_input.buffer_VB);
            else
                commandList->CopyBufferRegion(context->m_internal.buffer_DoSimpleRaster_VBCopy, 0, context->m_input.buffer_VB, 0, min(srcSize, destSize));

            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DoSimpleRaster_CopyVBs";
        ...
        // Shader Constants: _DoSimpleRaster_VertexShaderCB
        {
            context->m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB_cpu.DoSimpleRaster_ViewProjMtx = context->m_input.variable_DoSimpleRaster_ViewProjMtx;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB, context->m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB_cpu, Context::LogFn);
        }
        ...
        // Draw Call: DoSimpleRaster_Rasterize
        {
            ScopedPerfEvent scopedPerf("Draw Call: DoSimpleRaster_Rasterize", commandList, 1);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetGraphicsRootSignature(context->m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig);
            commandList->SetPipelineState(context->m_internal.drawCall_DoSimpleRaster_Rasterize_pso);

            DX12Utils::ResourceDescriptor descriptorsVS[] =
            {
                { context->m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB, DXGI_FORMAT_UNKNOWN, DX12Utils::AccessType::CBV, DX12Utils::ResourceType::Buffer, false, 256, 1, 0 },
            };
            ...
            // Vertex Buffer
            int vertexCountPerInstance = context->m_internal.buffer_DoSimpleRaster_VBCopy_count;

            D3D12_VERTEX_BUFFER_VIEW vbView;
            vbView.BufferLocation = context->m_internal.buffer_DoSimpleRaster_VBCopy->GetGPUVirtualAddress();
            vbView.StrideInBytes = (context->m_internal.buffer_DoSimpleRaster_VBCopy_format == DXGI_FORMAT_UNKNOWN)
                ? context->m_internal.buffer_DoSimpleRaster_VBCopy_stride
                : DX12Utils::Get_DXGI_FORMAT_Info(context->m_internal.buffer_DoSimpleRaster_VBCopy_format, Context::LogFn).bytesPerPixel;
            vbView.SizeInBytes = vbView.StrideInBytes * context->m_internal.buffer_DoSimpleRaster_VBCopy_count;
            ...
            // Clear DoSimpleRaster_Color_Buffer
            {
                 float clearValues[4] = { 0.200000f, 0.200000f, 0.200000f, 1.000000f };
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_DoSimpleRaster_Color_Buffer, context->m_output.texture_DoSimpleRaster_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "simpleRasterInSubgraph.DoSimpleRaster_Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for simpleRasterInSubgraph.DoSimpleRaster_Color_Buffer");
                 commandList->ClearRenderTargetView(s_heapAllocationTrackerRTV.GetCPUHandle(rtvIndex), clearValues, 0, nullptr);
            }

            // Clear DoSimpleRaster_Depth_Buffer
            {
                 D3D12_CLEAR_FLAGS clearFlags = D3D12_CLEAR_FLAG_DEPTH;
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_DoSimpleRaster_Depth_Buffer, context->m_output.texture_DoSimpleRaster_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "simpleRasterInSubgraph.DoSimpleRaster_Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for simpleRasterInSubgraph.DoSimpleRaster_Depth_Buffer");
                commandList->ClearDepthStencilView(s_heapAllocationTrackerDSV.GetCPUHandle(dsvIndex), clearFlags, 0.000000f, 0, 0, nullptr);
            }

            int renderWidth = context->m_output.texture_DoSimpleRaster_Color_Buffer_size[0];
            int renderHeight = context->m_output.texture_DoSimpleRaster_Color_Buffer_size[1];

            D3D12_CPU_DESCRIPTOR_HANDLE colorTargetHandles[] =
            {
                s_heapAllocationTrackerRTV.GetCPUHandle(context->GetRTV(device, context->m_output.texture_DoSimpleRaster_Color_Buffer, context->m_output.texture_DoSimpleRaster_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "simpleRasterInSubgraph.DoSimpleRaster_Color_Buffer"))
            };

            int colorTargetHandleCount = _countof(colorTargetHandles);

            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_DoSimpleRaster_Depth_Buffer, context->m_output.texture_DoSimpleRaster_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "simpleRasterInSubgraph.DoSimpleRaster_Depth_Buffer"));
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DoSimpleRaster_Rasterize";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
        ...
        // Make sure imported resources are put back in the state they were given to us in
        {
            ...
            if(context->m_input.buffer_VB_state != D3D12_RESOURCE_STATE_COPY_SOURCE)
            {
                ...
                barriers[barrierCount].Transition.StateAfter = context->m_input.buffer_VB_state;
                barriers[barrierCount].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
                barrierCount++;
            }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // DoSimpleRaster_Color_Buffer
        {
            ...
            if(!m_output.texture_DoSimpleRaster_Color_Buffer ||
               m_output.texture_DoSimpleRaster_Color_Buffer_size[0] != desiredSize[0] ||
               m_output.texture_DoSimpleRaster_Color_Buffer_size[1] != desiredSize[1] ||
               m_output.texture_DoSimpleRaster_Color_Buffer_size[2] != desiredSize[2] ||
               m_output.texture_DoSimpleRaster_Color_Buffer_numMips != desiredNumMips ||
               m_output.texture_DoSimpleRaster_Color_Buffer_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_DoSimpleRaster_Color_Buffer)
                    s_delayedRelease.Add(m_output.texture_DoSimpleRaster_Color_Buffer);

                m_output.texture_DoSimpleRaster_Color_Buffer = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_DoSimpleRaster_Color_Buffer_flags, D3D12_RESOURCE_STATE_RENDER_TARGET, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"DoSimpleRaster_Color_Buffer" : nullptr), Context::LogFn);
                m_output.texture_DoSimpleRaster_Color_Buffer_size[0] = desiredSize[0];
                m_output.texture_DoSimpleRaster_Color_Buffer_size[1] = desiredSize[1];
                m_output.texture_DoSimpleRaster_Color_Buffer_size[2] = desiredSize[2];
                m_output.texture_DoSimpleRaster_Color_Buffer_numMips = desiredNumMips;
        ...
        // DoSimpleRaster_Depth_Buffer
        {
            unsigned int baseSize[3] = {
                m_output.texture_DoSimpleRaster_Color_Buffer_size[0],
                m_output.texture_DoSimpleRaster_Color_Buffer_size[1],
                m_output.texture_DoSimpleRaster_Color_Buffer_size[2]
            };
            ...
            if(!m_output.texture_DoSimpleRaster_Depth_Buffer ||
               m_output.texture_DoSimpleRaster_Depth_Buffer_size[0] != desiredSize[0] ||
               m_output.texture_DoSimpleRaster_Depth_Buffer_size[1] != desiredSize[1] ||
               m_output.texture_DoSimpleRaster_Depth_Buffer_size[2] != desiredSize[2] ||
               m_output.texture_DoSimpleRaster_Depth_Buffer_numMips != desiredNumMips ||
               m_output.texture_DoSimpleRaster_Depth_Buffer_format != desiredFormat)
            {
                dirty = true;
                if(m_output.texture_DoSimpleRaster_Depth_Buffer)
                    s_delayedRelease.Add(m_output.texture_DoSimpleRaster_Depth_Buffer);

                m_output.texture_DoSimpleRaster_Depth_Buffer = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_output.texture_DoSimpleRaster_Depth_Buffer_flags, D3D12_RESOURCE_STATE_DEPTH_WRITE, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"DoSimpleRaster_Depth_Buffer" : nullptr), Context::LogFn);
                m_output.texture_DoSimpleRaster_Depth_Buffer_size[0] = desiredSize[0];
                m_output.texture_DoSimpleRaster_Depth_Buffer_size[1] = desiredSize[1];
                m_output.texture_DoSimpleRaster_Depth_Buffer_size[2] = desiredSize[2];
                m_output.texture_DoSimpleRaster_Depth_Buffer_numMips = desiredNumMips;
        ...
        // DoSimpleRaster_VBCopy
        // This is here for the benefit of simpleRasterInSubgraph. Need a reference to a struct type in a subgraph.
        {
            unsigned int baseCount = m_input.buffer_VB_count;
            unsigned int desiredCount = ((baseCount + 0 ) * 1) / 1 + 0;
            DXGI_FORMAT desiredFormat = DXGI_FORMAT_UNKNOWN;
            unsigned int desiredStride = 24;

            if(!m_internal.buffer_DoSimpleRaster_VBCopy ||
               m_internal.buffer_DoSimpleRaster_VBCopy_count != desiredCount ||
               m_internal.buffer_DoSimpleRaster_VBCopy_format != desiredFormat ||
               m_internal.buffer_DoSimpleRaster_VBCopy_stride != desiredStride)
            {
                dirty = true;
                if(m_internal.buffer_DoSimpleRaster_VBCopy)
                    s_delayedRelease.Add(m_internal.buffer_DoSimpleRaster_VBCopy);
                ...
                m_internal.buffer_DoSimpleRaster_VBCopy = DX12Utils::CreateBuffer(device, desiredSize, m_internal.c_buffer_DoSimpleRaster_VBCopy_flags, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"DoSimpleRaster_VBCopy" : nullptr), Context::LogFn);
                m_internal.buffer_DoSimpleRaster_VBCopy_count = desiredCount;
                m_internal.buffer_DoSimpleRaster_VBCopy_format = desiredFormat;
                m_internal.buffer_DoSimpleRaster_VBCopy_stride = desiredStride;
            }
        ...
        // _DoSimpleRaster_VertexShaderCB
        if (m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__DoSimpleRaster_VertexShaderCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_DoSimpleRaster_VertexShaderCB" : nullptr), Context::LogFn);
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_DoSimpleRaster_Rasterize_pso || !m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig)
        {
            ...
            hr = device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig));
            if (FAILED(hr))
            {
                const char* errorMsg = (error ? (const char*)error->GetBufferPointer() : nullptr);
                if (errorMsg)
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/simpleRaster/simpleRaster_VS.hlsl", "VSMain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/simpleRaster/simpleRaster_PS.hlsl", "PSMain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
            ...
            psoDesc.pRootSignature = m_internal.drawCall_DoSimpleRaster_Rasterize_rootSig;
            ...
            psoDesc.RTVFormats[0] = m_output.texture_DoSimpleRaster_Color_Buffer_format;
            psoDesc.NumRenderTargets = 1;

            psoDesc.DepthStencilState.DepthEnable = TRUE;
            psoDesc.DSVFormat = DX12Utils::DSV_Safe_DXGI_FORMAT(m_output.texture_DoSimpleRaster_Depth_Buffer_format);
            ...
            hr = device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_internal.drawCall_DoSimpleRaster_Rasterize_pso));

            if (FAILED(hr))
            {
                Context::LogFn(LogLevel::Error, "Could not create PSO for DoSimpleRaster_Rasterize");
                return false;
            }

            // name the PSO for debuggers
            if (c_debugNames)
                m_internal.drawCall_DoSimpleRaster_Rasterize_pso->SetName(L"DoSimpleRaster_Rasterize");
        }
        return true;
    }
};
