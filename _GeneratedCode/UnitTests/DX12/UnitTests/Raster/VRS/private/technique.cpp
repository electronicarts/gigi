...
namespace VRS
{
    ...
    ID3D12PipelineState* ContextInternal::computeShader_MakeShadingRate_pso = nullptr;
    ID3D12RootSignature* ContextInternal::computeShader_MakeShadingRate_rootSig = nullptr;
    ...
    bool CreateShared(ID3D12Device* device)
    {
        ...
        // Compute Shader: MakeShadingRate
        {
            ...
            if(!DX12Utils::MakeRootSig(device, ranges, 1, samplers, 0, &ContextInternal::computeShader_MakeShadingRate_rootSig, (c_debugNames ? L"MakeShadingRate" : nullptr), Context::LogFn))
                return false;
            ...
            if(!DX12Utils::MakeComputePSO_DXC(device, Context::s_techniqueLocation.c_str(), L"shaders/VRS_ShadingRateCS.hlsl", "ShadingRateCSMain", "cs_6_4", defines,
               ContextInternal::computeShader_MakeShadingRate_rootSig, &ContextInternal::computeShader_MakeShadingRate_pso, c_debugShaders, (c_debugNames ? L"MakeShadingRate" : nullptr), Context::LogFn))
                return false;
        }
    ...
    void DestroyShared()
    {
        ...
        if(ContextInternal::computeShader_MakeShadingRate_pso)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_MakeShadingRate_pso);
            ContextInternal::computeShader_MakeShadingRate_pso = nullptr;
        }

        if(ContextInternal::computeShader_MakeShadingRate_rootSig)
        {
            s_delayedRelease.Add(ContextInternal::computeShader_MakeShadingRate_rootSig);
            ContextInternal::computeShader_MakeShadingRate_rootSig = nullptr;
        }
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // compute shader: MakeShadingRate
        ...
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // Draw Call: Vignette
        m_profileData[numItems].m_gpu = float(GPUTickDelta * double(timeStampBuffer[numItems*2+2] - timeStampBuffer[numItems*2+1])); numItems++; // Draw Call: DrawQuad
    ...
    Context::~Context()
    {
        ...
        if(m_internal.texture_ShadingRate)
        {
            s_delayedRelease.Add(m_internal.texture_ShadingRate);
            m_internal.texture_ShadingRate = nullptr;
        }
        ...
        if(m_internal.drawCall_Vignette_pso)
        {
            s_delayedRelease.Add(m_internal.drawCall_Vignette_pso);
            m_internal.drawCall_Vignette_pso = nullptr;
        }

        if(m_internal.drawCall_Vignette_rootSig)
        {
            s_delayedRelease.Add(m_internal.drawCall_Vignette_rootSig);
            m_internal.drawCall_Vignette_rootSig = nullptr;
        }

        // _QuadVSCB
        if (m_internal.constantBuffer__QuadVSCB)
        {
            s_delayedRelease.Add(m_internal.constantBuffer__QuadVSCB);
            m_internal.constantBuffer__QuadVSCB = nullptr;
        }

        if(m_internal.drawCall_DrawQuad_pso)
        {
            s_delayedRelease.Add(m_internal.drawCall_DrawQuad_pso);
            m_internal.drawCall_DrawQuad_pso = nullptr;
        }

        if(m_internal.drawCall_DrawQuad_rootSig)
        {
            s_delayedRelease.Add(m_internal.drawCall_DrawQuad_rootSig);
            m_internal.drawCall_DrawQuad_rootSig = nullptr;
        }
    }
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("VRS", commandList, 10);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"VRS Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (4+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"VRS Time Stamp Query Heap" : nullptr), nullptr);
            }
            commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
        }

        // Set variables
        context->m_internal.variable_ShadingRateTextureSizeXY[0] = 512 / context->m_input.variable_ShadingRateImageTileSize;
        context->m_internal.variable_ShadingRateTextureSizeXY[1] = context->m_internal.variable_ShadingRateTextureSizeXY[0];

        if (!context->m_input.buffer_VertexBuffer)
        {
            Context::LogFn(LogLevel::Error, "VRS: Imported buffer \"VertexBuffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_VertexBuffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "VRS: Imported buffer \"VertexBuffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Transition.pResource = context->m_internal.texture_ShadingRate;
            barriers[0].Transition.StateBefore = D3D12_RESOURCE_STATE_SHADING_RATE_SOURCE;
        ...
        // Compute Shader: MakeShadingRate
        {
            ScopedPerfEvent scopedPerf("Compute Shader: MakeShadingRate", commandList, 6);
            ...
            commandList->SetComputeRootSignature(ContextInternal::computeShader_MakeShadingRate_rootSig);
            ...
            unsigned int baseDispatchSize[3] = {
                context->m_internal.texture_ShadingRate_size[0],
                context->m_internal.texture_ShadingRate_size[1],
                context->m_internal.texture_ShadingRate_size[2]
            };
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "MakeShadingRate";
        ...
        // Draw Call: Rasterize
        {
            ...
            // Clear Color_Buffer
            {
                 ...
                 int rtvIndex = context->GetRTV(device, context->m_output.texture_Color_Buffer, context->m_output.texture_Color_Buffer_format, D3D12_RTV_DIMENSION_TEXTURE2D, 0, 0, "VRS.Color_Buffer");
                 if (rtvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get RTV for VRS.Color_Buffer");
            ...
            // Clear Depth_Buffer
            {
                 ...
                 int dsvIndex = context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "VRS.Depth_Buffer");
                 if (dsvIndex == -1)
                     Context::LogFn(LogLevel::Error, "Could not get DSV for VRS.Depth_Buffer");
            ...
            D3D12_CPU_DESCRIPTOR_HANDLE depthTargetHandle = s_heapAllocationTrackerDSV.GetCPUHandle(context->GetDSV(device, context->m_output.texture_Depth_Buffer, context->m_output.texture_Depth_Buffer_format, D3D12_DSV_DIMENSION_TEXTURE2D, 0, 0, "VRS.Depth_Buffer"));
            ...
            // Set the shading rate
            {
                ...
                else
                {
                    ...
                    commandList5->RSSetShadingRate(D3D12_SHADING_RATE_4X4, combiners);
        ...
        // Transition resources for the next action
        {
            ...
            barriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
            barriers[0].Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
            barriers[0].Transition.pResource = context->m_output.texture_Depth_Buffer;
            ...
            barriers[1].Transition.pResource = context->m_internal.texture_ShadingRate;
            barriers[1].Transition.StateBefore = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
            barriers[1].Transition.StateAfter = D3D12_RESOURCE_STATE_SHADING_RATE_SOURCE;
        ...
        // Draw Call: Vignette
        {
            ScopedPerfEvent scopedPerf("Draw Call: Vignette", commandList, 7);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetGraphicsRootSignature(context->m_internal.drawCall_Vignette_rootSig);
            commandList->SetPipelineState(context->m_internal.drawCall_Vignette_pso);
            ...
            // Set the shading rate
            {
                ...
                {
                    D3D12_SHADING_RATE_COMBINER combiners[2] = { D3D12_SHADING_RATE_COMBINER_PASSTHROUGH, D3D12_SHADING_RATE_COMBINER_MAX };
                    commandList5->RSSetShadingRate(D3D12_SHADING_RATE_1X1, combiners);
                    commandList5->RSSetShadingRateImage(context->m_internal.texture_ShadingRate);
                    commandList5->Release();
                }
            }
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "Vignette";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }

        // Shader Constants: _QuadVSCB
        {
            context->m_internal.constantBuffer__QuadVSCB_cpu.ViewProjMtx = context->m_input.variable_ViewProjMtx;
            DX12Utils::CopyConstantsCPUToGPU(s_ubTracker, device, commandList, context->m_internal.constantBuffer__QuadVSCB, context->m_internal.constantBuffer__QuadVSCB_cpu, Context::LogFn);
        }
        ...
        // Draw Call: DrawQuad
        {
            ScopedPerfEvent scopedPerf("Draw Call: DrawQuad", commandList, 4);
            std::chrono::high_resolution_clock::time_point startPointCPU;
            if(context->m_profile)
            {
                startPointCPU = std::chrono::high_resolution_clock::now();
            ...
            commandList->SetGraphicsRootSignature(context->m_internal.drawCall_DrawQuad_rootSig);
            commandList->SetPipelineState(context->m_internal.drawCall_DrawQuad_pso);
            ...
            // Set the shading rate
            {
                ...
                {
                    D3D12_SHADING_RATE_COMBINER combiners[2] = { D3D12_SHADING_RATE_COMBINER_MAX, D3D12_SHADING_RATE_COMBINER_PASSTHROUGH };
                    commandList5->RSSetShadingRate(D3D12_SHADING_RATE_1X1, combiners);
                    commandList5->Release();
                }
            }
            ...
            if(context->m_profile)
            {
                context->m_profileData[(s_timerIndex-1)/2].m_label = "DrawQuad";
                context->m_profileData[(s_timerIndex-1)/2].m_cpu = (float)std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::high_resolution_clock::now() - startPointCPU).count();
                commandList->EndQuery(context->m_internal.m_TimestampQueryHeap, D3D12_QUERY_TYPE_TIMESTAMP, s_timerIndex++);
            }
        }
    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // ShadingRate
        {
            ...
            unsigned int baseSize[3] = { (unsigned int)m_internal.variable_ShadingRateTextureSizeXY[0], (unsigned int)m_internal.variable_ShadingRateTextureSizeXY[1], 1 };
            ...
            DXGI_FORMAT desiredFormat = DXGI_FORMAT_R8_UINT;

            if(!m_internal.texture_ShadingRate ||
               m_internal.texture_ShadingRate_size[0] != desiredSize[0] ||
               m_internal.texture_ShadingRate_size[1] != desiredSize[1] ||
               m_internal.texture_ShadingRate_size[2] != desiredSize[2] ||
               m_internal.texture_ShadingRate_numMips != desiredNumMips ||
               m_internal.texture_ShadingRate_format != desiredFormat)
            {
                dirty = true;
                if(m_internal.texture_ShadingRate)
                    s_delayedRelease.Add(m_internal.texture_ShadingRate);

                m_internal.texture_ShadingRate = DX12Utils::CreateTexture(device, desiredSize, desiredNumMips, desiredFormat, m_internal.texture_ShadingRate_flags, D3D12_RESOURCE_STATE_SHADING_RATE_SOURCE, DX12Utils::ResourceType::Texture2D, (c_debugNames ? L"ShadingRate" : nullptr), Context::LogFn);
                m_internal.texture_ShadingRate_size[0] = desiredSize[0];
                m_internal.texture_ShadingRate_size[1] = desiredSize[1];
                m_internal.texture_ShadingRate_size[2] = desiredSize[2];
                m_internal.texture_ShadingRate_numMips = desiredNumMips;
        ...
        // _QuadVSCB
        if (m_internal.constantBuffer__QuadVSCB == nullptr)
        {
            dirty = true;
            m_internal.constantBuffer__QuadVSCB = DX12Utils::CreateBuffer(device, 256, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT, (c_debugNames ? L"_QuadVSCB" : nullptr), Context::LogFn);
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_VS.hlsl", "VSMain", "vs_6_4", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_PS.hlsl", "PSMain", "ps_6_4", definesPS, c_debugShaders, Context::LogFn);
        ...
        if(!m_internal.drawCall_Vignette_pso || !m_internal.drawCall_Vignette_rootSig)
        {
            ...
            hr = device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&m_internal.drawCall_Vignette_rootSig));
            if (FAILED(hr))
            {
                const char* errorMsg = (error ? (const char*)error->GetBufferPointer() : nullptr);
                if (errorMsg)
            ...
            // name the root signature for debuggers
            if (c_debugNames)
                m_internal.drawCall_Vignette_rootSig->SetName(L"Vignette");

            D3D_SHADER_MACRO* definesVS = nullptr;

            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_Vignette_VS.hlsl", "VSMain", "vs_6_4", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_Vignette_PS.hlsl", "PSMain", "ps_6_4", definesPS, c_debugShaders, Context::LogFn);
            if (byteCodePS.size() == 0)
                return false;
            ...
            psoDesc.PS.BytecodeLength = byteCodePS.size();
            psoDesc.SampleDesc.Count = 1;
            psoDesc.pRootSignature = m_internal.drawCall_Vignette_rootSig;
            psoDesc.RasterizerState.FillMode = D3D12_FILL_MODE_SOLID;
            psoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
            psoDesc.RasterizerState.FrontCounterClockwise = FALSE;
            psoDesc.RasterizerState.DepthBias = D3D12_DEFAULT_DEPTH_BIAS;
            ...
            hr = device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_internal.drawCall_Vignette_pso));

            if (FAILED(hr))
            {
                Context::LogFn(LogLevel::Error, "Could not create PSO for Vignette");
                return false;
            }

            // name the PSO for debuggers
            if (c_debugNames)
                m_internal.drawCall_Vignette_pso->SetName(L"Vignette");
        }
        ...
        if(!m_internal.drawCall_DrawQuad_pso || !m_internal.drawCall_DrawQuad_rootSig)
        {
            ...
            hr = device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(&m_internal.drawCall_DrawQuad_rootSig));
            if (FAILED(hr))
            {
                const char* errorMsg = (error ? (const char*)error->GetBufferPointer() : nullptr);
                if (errorMsg)
            ...
            // name the root signature for debuggers
            if (c_debugNames)
                m_internal.drawCall_DrawQuad_rootSig->SetName(L"DrawQuad");

            D3D_SHADER_MACRO* definesVS = nullptr;

            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_Quad_VS.hlsl", "VSMain", "vs_6_4", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/VRS_Quad_PS.hlsl", "PSMain", "ps_6_4", definesPS, c_debugShaders, Context::LogFn);
            if (byteCodePS.size() == 0)
                return false;
            ...
            psoDesc.PS.BytecodeLength = byteCodePS.size();
            psoDesc.SampleDesc.Count = 1;
            psoDesc.pRootSignature = m_internal.drawCall_DrawQuad_rootSig;
            psoDesc.RasterizerState.FillMode = D3D12_FILL_MODE_SOLID;
            psoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
            psoDesc.RasterizerState.FrontCounterClockwise = FALSE;
            psoDesc.RasterizerState.DepthBias = D3D12_DEFAULT_DEPTH_BIAS;
            ...
            hr = device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_internal.drawCall_DrawQuad_pso));

            if (FAILED(hr))
            {
                Context::LogFn(LogLevel::Error, "Could not create PSO for DrawQuad");
                return false;
            }

            // name the PSO for debuggers
            if (c_debugNames)
                m_internal.drawCall_DrawQuad_pso->SetName(L"DrawQuad");
        }
        return true;
    }
};
