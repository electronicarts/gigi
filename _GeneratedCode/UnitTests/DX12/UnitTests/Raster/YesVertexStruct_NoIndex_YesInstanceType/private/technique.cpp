...
namespace YesVertexStruct_NoIndex_YesInstanceType
{
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("YesVertexStruct_NoIndex_YesInstanceType", commandList, 7);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"YesVertexStruct_NoIndex_YesInstanceType Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (1+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"YesVertexStruct_NoIndex_YesInstanceType Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceType: Imported buffer \"Vertex_Buffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_Vertex_Buffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceType: Imported buffer \"Vertex_Buffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }

        if (!context->m_input.buffer_Instance_Buffer)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceType: Imported buffer \"Instance_Buffer\" is null.\n");
            return;
        }

        if (context->m_input.buffer_Instance_Buffer_vertexInputLayout.size() == 0)
        {
            Context::LogFn(LogLevel::Error, "YesVertexStruct_NoIndex_YesInstanceType: Imported buffer \"Instance_Buffer\" is used as a vertex buffer but no vertexInputLayout was given.\n");
            return;
        }
    ...
    bool Context::EnsureDrawCallPSOsCreated(ID3D12Device* device, bool dirty)
    {
        ...
        if(!m_internal.drawCall_Rasterize_pso || !m_internal.drawCall_Rasterize_rootSig)
        {
            ...
            std::vector<unsigned char> byteCodeVS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_NoIndex_YesInstanceType_VS.hlsl", "VSMain", "vs_6_1", definesVS, c_debugShaders, Context::LogFn);
            if (byteCodeVS.size() == 0)
                return false;

            D3D_SHADER_MACRO* definesPS = nullptr;

            std::vector<unsigned char> byteCodePS = DX12Utils::CompileShaderToByteCode_DXC(Context::s_techniqueLocation.c_str(), L"shaders/YesVertexStruct_NoIndex_YesInstanceType_PS.hlsl", "PSMain", "ps_6_1", definesPS, c_debugShaders, Context::LogFn);
