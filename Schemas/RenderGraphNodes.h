...
STRUCT_BEGIN(TextureSizeDesc, "size = (inputSize + preAdd) * multiply / divide + postAdd.  inputSize is (1,1,1) if nothing given.")
    STRUCT_FIELD(TextureNodeReference, node, {}, "If a texture is specified, the size will be based on the size of the texture. Padded with 1s to make it 3 dimensional.", 0)
...
STRUCT_INHERIT_BEGIN(RenderGraphNode_Action_DrawCall, RenderGraphNode_ActionBase, "Rasterization")
    ...
    STRUCT_FIELD(NodePinReferenceOptional, indirectBuffer, {}, "Indirect buffer to make this draw use ExecuteIndirect", SCHEMA_FLAG_NO_UI)
