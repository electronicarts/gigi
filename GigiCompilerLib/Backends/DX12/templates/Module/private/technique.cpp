...
/*$(if:DX12.AgilitySDKRequired:true)*/
#include "d3dx12/d3dx12.h"/*$(endif)*/
#include <vector>
#include <chrono>
...
namespace /*$(Name)*/
{
    ...
    ID3D12CommandSignature* ContextInternal::s_commandSignatureDispatch = nullptr;/*$(StaticVariables)*/
    ...
    const ProfileEntry* Context::ReadbackProfileData(ID3D12CommandQueue* commandQueue, int& numItems)
    {
        ...
        D3D12_RANGE range;
        range.Begin = 0;
        range.End = ((/*$(NumActionNodes)*/ + 1) * 2) * sizeof(uint64_t);
...
/*$(ReadbackProfileData)*/
    ...
    void Execute(Context* context, ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        ScopedPerfEvent scopedPerf("/*$(Name)*/", commandList, /*$(NumNodes)*/);
        ...
        if(context->m_profile)
        {
            ...
            if(context->m_internal.m_TimestampQueryHeap == nullptr)
            {
                ...
                QueryHeapDesc.Count = (/*$(NumActionNodes)*/+1) * 2;
                QueryHeapDesc.NodeMask = 1;
                QueryHeapDesc.Type = D3D12_QUERY_HEAP_TYPE_TIMESTAMP;
                device->CreateQueryHeap(&QueryHeapDesc, IID_PPV_ARGS(&context->m_internal.m_TimestampQueryHeap));
                if (c_debugNames)
                    context->m_internal.m_TimestampQueryHeap->SetName(L"/*$(Name)*/ Time Stamp Query Heap");

                context->m_internal.m_TimestampReadbackBuffer = DX12Utils::CreateBuffer(device, sizeof(uint64_t) * (/*$(NumActionNodes)*/+1) * 2, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_HEAP_TYPE_READBACK, (c_debugNames ? L"/*$(Name)*/ Time Stamp Query Heap" : nullptr), nullptr);
            }
        ...
        commandList->SetDescriptorHeaps(_countof(heaps), heaps);/*$(Execute)*//*$(ExecuteEnd)*/
    }

    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        bool dirty = false;/*$(EnsureResourcesCreated)*/
