...
def DoTest():
	...
	outDirName = "Techniques/UnitTests/_GoldImages/Compute/IndirectDispatchExtended/"
	os.makedirs(outDirName, exist_ok=True)

	# Load the technique
	if not Host.LoadGG("Techniques/UnitTests/Compute/IndirectDispatchExtended.gg"):
		return False

	# Specify the resources we want to readback
	Host.SetWantReadback("Test_Indirect_Dispatch_Extended_Merged.Render_Target: Render_Target (UAV - After)")
	...
	Host.RunTechnique()
	Host.WaitOnGPU()
	lastReadback, success = Host.Readback("Test_Indirect_Dispatch_Extended_Merged.Render_Target: Render_Target (UAV - After)")
