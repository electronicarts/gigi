...
{
    float3 loadedTexturePx = /*$(Image3D:ImageB%i.png:RGBA8_Unorm:float4:false)*/[uint3(input.position.xy, 0)].rgb;
    float3 importedTexturePx = /*$(RWTextureR:importedTexture)*/[uint3(input.position.xy, 1)].rgb;
    float3 importedColorPx = /*$(RWTextureR:importedColor)*/[uint3(input.position.xy, 2)].rgb;
