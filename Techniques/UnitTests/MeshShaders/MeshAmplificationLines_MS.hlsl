...
/*$(_mesh:MSMain:line)*/(uint3 dtid : SV_DispatchThreadID, uint gtid : SV_GroupIndex, uint3 gid : SV_GroupID, in payload ASPayload payload, out vertices ProcessedVertex verts[MESHLET_NUM_VERTS], out indices uint2 lines[MESHLET_NUM_LINES])
{
    int numVerts = MESHLET_NUM_VERTS;
    int numLines = MESHLET_NUM_LINES;

    uint instanceId = dtid.z;
    float3 offset = float3(float(instanceId) * /*$(Variable:AmplificationSpacing)*/, 0.0f, 0.0f);

    SetMeshOutputCounts(numVerts, numLines);
    ...
    if (gtid < MESHLET_NUM_LINES) 
    {
        int triIndex = gtid / 3;
        int lineOffset1 = (gtid+0) % 3;
        int lineOffset2 = (gtid+1) % 3;

        lines[gtid] = uint2(triIndex*3 + lineOffset1, triIndex*3 + lineOffset2 );
    }
}
