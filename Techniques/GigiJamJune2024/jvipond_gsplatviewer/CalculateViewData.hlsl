// Unnamed technique, shader CalculateViewData
/*$(ShaderResources)*/
...
float3 CalcCovariance2D(float3 worldPos, float3 cov3d0, float3 cov3d1, float4x4 matrixV, float4x4 matrixP, uint2 screenParams)
{
...
SplatData LoadSplatData(uint idx)
{
    ...
    uint scaleFmt = (/*$(Variable:_SplatFormat)*/ >> 8) & 0xFF;
    uint shFormat = (/*$(Variable:_SplatFormat)*/ >> 16) & 0xFF;
...
/*$(_compute:CSCalcViewData)*/(uint3 DTid : SV_DispatchThreadID)
{
	uint idx = DTid.x;
	...
	Struct_GaussianSplatViewData view = (Struct_GaussianSplatViewData) 0;

	float3 centerWorldPos = mul(float4(splat.pos, 1), /*$(Variable:_MatrixObjectToWorld)*/).xyz;
	float4 centerClipPos = mul(float4(centerWorldPos, 1), /*$(Variable:ViewProjMtx)*/);
	half opacityScale = /*$(Variable:_SplatOpacityScale)*/;
	float splatScale = /*$(Variable:_SplatScale)*/;
	...
	if (!behindCam)
	{
        ...
        float3 cov2d = CalcCovariance2D(splat.pos, cov3d0, cov3d1, mv, /*$(Variable:ProjMtx)*/, /*$(Variable:RenderSize)*/);
    
		DecomposeCovariance(cov2d, view.axis1, view.axis2);

		float3 worldViewDir = /*$(Variable:CameraPos)*/ - centerWorldPos;
		float3 objViewDir = mul(worldViewDir, (float3x3) /*$(Variable:_MatrixWorldToObject)*/);
		objViewDir = normalize(objViewDir);

		half4 col;
		col.rgb = ShadeSH(splat.sh, objViewDir, /*$(Variable:_SHOrder)*/, /*$(Variable:_SHOnly)*/ != 0);
		col.a = min(splat.opacity * opacityScale, 65000);
		view.color1 = (f32tof16(col.r) << 16) | f32tof16(col.g);
		view.color2 = (f32tof16(col.b) << 16) | f32tof16(col.a);
	}
...
/*
Shader Resources:
	Buffer _SplatViewData (as UAV)
	Buffer _SplatOther (as SRV)
	Buffer _SplatSH (as SRV)
	Buffer _SplatPos (as SRV)
	Texture _SplatColor (as SRV)
