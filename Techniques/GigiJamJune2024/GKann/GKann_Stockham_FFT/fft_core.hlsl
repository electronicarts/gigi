...
// threadgroup (256, 1)
void fft(uint local_index, uint workgroup_index, uint axis, bool inverse)
{
    for (uint ch = 0; ch < N_CHANNELS; ch++)
    {
        ...
        for (uint i = 0; i < N / WORKGROUP_SIZE; i++)
        {
            ...
            uint2 p = set_component(uint2(workgroup_index, workgroup_index), axis, j);
            X[0][j] = FFTBuffer[uint3(p.x, p.y, ch)];
        }
        ...
        for (uint it = 0; RADIX == 4 && it < LOG4_N; it++) {
            ...
            for (uint i = 0; i < N / WORKGROUP_SIZE / 4; i++) {
                ...
                float2  amc =    a - c;
                float2  bpd =    b + d;
                float2 jbmd = inv * mul(float2(0, 1), (b - d));
                X[1 - flip][q + s*(4*p + 0)] =      apc +  bpd;
                X[1 - flip][q + s*(4*p + 1)] = mul(w1p, (amc + jbmd));
                X[1 - flip][q + s*(4*p + 2)] = mul(w2p, (apc -  bpd));
                X[1 - flip][q + s*(4*p + 3)] = mul(w3p, (amc - jbmd));
            }
        ...
        for (uint i = 0; i < N / WORKGROUP_SIZE; i++) {
            uint j = local_index + i * WORKGROUP_SIZE;
            uint2 p = set_component(uint2(workgroup_index, workgroup_index), axis, j);
            FFTBuffer[uint3(p.x, p.y, ch)] = X[flip][j] / (axis == 0 ? N : 1.0);
        }
