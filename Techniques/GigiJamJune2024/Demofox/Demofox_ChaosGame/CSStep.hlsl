...
/*$(_compute:main)*/(uint3 DTid : SV_DispatchThreadID)
{
	...
	float2 target = float2(0.0f, 0.0f);
	...
	{
		float2 flippedPos = float2(pos.x, 1.0f - pos.y);
		uint2 posPx = uint2(flippedPos * float2(/*$(Variable:RenderSize)*/));
		Screen[posPx] = float4(0.0f, 1.0f, 0.0f, 1.0f);
	}
