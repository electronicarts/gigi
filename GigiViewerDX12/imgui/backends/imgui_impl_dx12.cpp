...
static void ImGui_ImplDX12_SetupRenderState(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, ImGui_ImplDX12_RenderBuffers* fr)
{
    ...
    ctx->SetGraphicsRoot32BitConstants(2, 1, &flags, 0);
    float histogramMinMax[2] = { 0.0f, 1.0f };
    ctx->SetGraphicsRoot32BitConstants(2, 2, histogramMinMax, 1);
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ImGui_ImplDX12_SetupRenderState(draw_data, ctx, fr);
                else if (pcmd->UserCallback == ImDrawCallback_SetShaderFlags)
                {
                    unsigned int flags = (unsigned int)((unsigned long long)pcmd->UserCallbackData);
                    ctx->SetGraphicsRoot32BitConstants(2, 1, &flags, 0);
                }
                else if (pcmd->UserCallback == ImDrawCallback_SetHistogramMin)
                {
                    float value = *((float*)&pcmd->UserCallbackData);
                    ctx->SetGraphicsRoot32BitConstants(2, 1, &value, 1);
                }
                else if (pcmd->UserCallback == ImDrawCallback_SetHistogramMax)
                {
                    float value = *((float*)&pcmd->UserCallbackData);
                    ctx->SetGraphicsRoot32BitConstants(2, 1, &value, 2);
                }
                else
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    {
        #include "imgui_impl_dx12.ps.hlsl.h"

        ID3DBlob* error = nullptr;
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main", "ps_5_0", D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &pixelShaderBlob, &error)))
        {
            if (error)
            {
                const char* errorMsg = (const char*)error->GetBufferPointer();
                OutputDebugStringA("Could not compile shader:\n");
    ...
    bd->pPipelineState->SetName(L"ImGui Texture Drawing PSO");
