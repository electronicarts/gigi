...
bool GigiInterpreterPreviewWindowDX12::OnNodeAction(const RenderGraphNode_Action_ComputeShader& node, RuntimeTypes::RenderGraphNode_Action_ComputeShader& runtimeData, NodeAction nodeAction)
{
    ...
    if (nodeAction == NodeAction::Execute)
    {
        ...
        else
        {
            ...
            if (node.dispatchSize.indirectCountBuffer.resourceNodeIndex != -1)
            {
                // Get the indirect count buffer and publish it as a viewable resource if the resource differ form the indirect buffer
                const std::string& indirectCountBufferName = m_renderGraph.nodes[node.dispatchSize.indirectCountBuffer.resourceNodeIndex].resourceBuffer.name;
            ...
            if (executionConditionMet)
            {
                ...
                UINT64 argumentCountBufferOffset = node.dispatchSize.indirectCountOffsetValue;
                if (node.dispatchSize.indirectCountOffsetVariable.variableIndex != -1)
                {
                    GigiInterpreterPreviewWindowDX12::RuntimeVariable rtVar = GetRuntimeVariable(node.dispatchSize.indirectCountOffsetVariable.variableIndex);
                    switch (rtVar.variable->type)
                    {
                    case DataFieldType::Int: argumentCountBufferOffset = static_cast<UINT64>(*(int*)rtVar.storage.value); break;
                    default:
                    {
